...
uint64_t get_ref_poc(PictureDecisionContext *context,
                     uint64_t curr_picture_number, int32_t delta_poc) {
    uint64_t ref_poc;

    if ((int64_t)curr_picture_number - (int64_t)delta_poc <
        (int64_t)context->key_poc)
...
void eb_av1_setup_skip_mode_allowed(PictureParentControlSet *parent_pcs_ptr) {
    ...
    for (uint8_t i = 0; i < 7; ++i) {
        ref_frame_arr_single[i].poc =
            parent_pcs_ptr->av1_ref_signal.ref_poc_array[i] %
            (uint64_t)(1 << (parent_pcs_ptr->sequence_control_set_ptr
    ...
    const OrderHintInfo *const order_hint_info =
        &order_hint_info_st;  // cm->seq_params.order_hint_info;
    SkipModeInfo *const skip_mode_info =
        &frm_hdr->skip_mode_params;  // cm->current_frame.skip_mode_info;

    skip_mode_info->skip_mode_allowed = 0;
    skip_mode_info->ref_frame_idx_0 = INVALID_IDX;
    skip_mode_info->ref_frame_idx_1 = INVALID_IDX;
    parent_pcs_ptr->cur_order_hint =
        parent_pcs_ptr->picture_number %
        (uint64_t)(1 << (parent_pcs_ptr->sequence_control_set_ptr->seq_header
                             .order_hint_info.order_hint_bits));
    for (uint8_t i = 0; i < 7; ++i)
        parent_pcs_ptr->ref_order_hint[i] =
            (uint32_t)ref_frame_arr_single[i].poc;
    if (/*!order_hint_info->enable_order_hint ||*/ parent_pcs_ptr->slice_type ==
            I_SLICE /*frame_is_intra_only(cm)*/
        || frm_hdr->reference_mode == SINGLE_REFERENCE)
        return;

    const int cur_order_hint =
        parent_pcs_ptr->picture_number %
        (uint64_t)(1 << (parent_pcs_ptr->sequence_control_set_ptr->seq_header
                             .order_hint_info.order_hint_bits));
    ...
    for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
        // const RefCntBuffer *const buf = get_ref_frame_buf(cm, LAST_FRAME +
        // i); if (buf == NULL) continue;

        if (ref_frame_arr_single[i].used == 0)
            continue;

        const int ref_order_hint =
            (const int)ref_frame_arr_single[i].poc;  // buf->order_hint;
        if (get_relative_dist(order_hint_info, ref_order_hint, cur_order_hint) <
            0) {
            // Forward reference
            if (ref_order_hints[0] == -1 ||
                get_relative_dist(
                    order_hint_info, ref_order_hint, ref_order_hints[0]) > 0) {
                ref_order_hints[0] = ref_order_hint;
                ref_idx[0] = i;
            }
        } else if (get_relative_dist(
                       order_hint_info, ref_order_hint, cur_order_hint) > 0) {
            // Backward reference
            if (ref_order_hints[1] == INT_MAX ||
                get_relative_dist(
                    order_hint_info, ref_order_hint, ref_order_hints[1]) < 0) {
    ...
    if (ref_idx[0] != INVALID_IDX && ref_idx[1] != INVALID_IDX) {
        ...
        for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
            ...
            const int ref_order_hint =
                (const int)ref_frame_arr_single[i].poc;  // buf->order_hint;
            if ((ref_order_hints[0] != -1 &&
                 get_relative_dist(order_hint_info,
                                   ref_order_hint,
                                   ref_order_hints[0]) < 0) &&
                (ref_order_hints[1] == -1 ||
                 get_relative_dist(order_hint_info,
...
#define POC_CIRCULAR_ADD(base, offset /*, bits*/)                                \
    (/*(((int32_t) (base)) + ((int32_t) (offset)) > ((int32_t) (1 << (bits))))   \
    ? ((base) + (offset) - (1 << (bits))) :                                      \
    (((int32_t) (base)) + ((int32_t) (offset)) < 0) ? ((base) + (offset) + (1                                                       \
    << (bits))) :                                                                \
                                                                              */ \
     ((base) + (offset)))
...
#define QUEUE_GET_PREVIOUS_SPOT(h) \
    ((h == 0) ? PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH - 1 : h - 1)
#define QUEUE_GET_NEXT_SPOT(h, off)                           \
    (((h + off) >= PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH)  \
         ? h + off - PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH \
         : h + off)
...
EbErrorType picture_decision_context_ctor(
    ...
    context_ptr->picture_analysis_results_input_fifo_ptr =
        picture_analysis_results_input_fifo_ptr;
    context_ptr->picture_decision_results_output_fifo_ptr =
        picture_decision_results_output_fifo_ptr;

    EB_MALLOC_2D(context_ptr->ahd_running_avg_cb,
                 MAX_NUMBER_OF_REGIONS_IN_WIDTH,
                 MAX_NUMBER_OF_REGIONS_IN_HEIGHT);
    EB_MALLOC_2D(context_ptr->ahd_running_avg_cr,
                 MAX_NUMBER_OF_REGIONS_IN_WIDTH,
                 MAX_NUMBER_OF_REGIONS_IN_HEIGHT);
    EB_MALLOC_2D(context_ptr->ahd_running_avg,
                 MAX_NUMBER_OF_REGIONS_IN_WIDTH,
                 MAX_NUMBER_OF_REGIONS_IN_HEIGHT);

    for (arrayRow = 0; arrayRow < MAX_NUMBER_OF_REGIONS_IN_HEIGHT; arrayRow++) {
        for (arrowColumn = 0; arrowColumn < MAX_NUMBER_OF_REGIONS_IN_WIDTH;
             arrowColumn++) {
...
EbBool SceneTransitionDetector(PictureDecisionContext *context_ptr,
    ...
    // calculating the frame threshold based on the number of 64x64 blocks in
    // the frame
    uint32_t regionThreshHold;
    uint32_t regionThreshHoldChroma;
    // this variable determines whether the running average should be reset to
    // equal the ahd or not after detecting a scene change.
    // EbBool reset_running_avg = context_ptr->reset_running_avg;

    EbBool isAbruptChange;   // this variable signals an abrubt change (scene
                             // change or flash)
    EbBool is_scene_change;  // this variable signals a frame representing a
                             // scene change
    ...
    EbBool gradualChange;  // this signals the detection of a light scene change
                           // a small/localized flash or the start of a fade

    uint32_t ahd;  // accumulative histogram (absolute) differences between the
    ...
    uint32_t ahdError = 0;  // the difference between the ahd and the running
                            // average at the current frame.

    uint8_t aidFuturePast =
        0;  // this variable denotes the average intensity difference between
            // the next and the past frames
    uint8_t aidFuturePresent = 0;
    uint8_t aidPresentPast = 0;

    uint32_t bin =
        0;  // variable used to iterate through the bins of the histograms
    ...
    uint32_t regionCountThreshold =
        (sequence_control_set_ptr->scd_mode == SCD_MODE_2)
            ? (uint32_t)(
                  ((float)((sequence_control_set_ptr
                                ->picture_analysis_number_of_regions_per_width *
                            sequence_control_set_ptr
                                ->picture_analysis_number_of_regions_per_height) *
                           75) /
                   100) +
                  0.5)
            : (uint32_t)(
    ...
    regionWidth =
        ParentPcsWindow[1]->enhanced_picture_ptr->width /
        sequence_control_set_ptr->picture_analysis_number_of_regions_per_width;
    regionHeight =
        ParentPcsWindow[1]->enhanced_picture_ptr->height /
        sequence_control_set_ptr->picture_analysis_number_of_regions_per_height;

    // Loop over regions inside the picture
    for (regionInPictureWidthIndex = 0;
         regionInPictureWidthIndex <
         sequence_control_set_ptr->picture_analysis_number_of_regions_per_width;
         regionInPictureWidthIndex++) {  // loop over horizontal regions
        for (regionInPictureHeightIndex = 0;
             regionInPictureHeightIndex <
             sequence_control_set_ptr
                 ->picture_analysis_number_of_regions_per_height;
             regionInPictureHeightIndex++) {  // loop over vertical regions
            ...
            // Reset accumulative histogram (absolute) differences between the
            // past and current frame
            ahd = 0;
            ahdCb = 0;
            ahdCr = 0;
            ...
            regionThreshHold =
                (
                    ...
                    ((ABS((int64_t)
                              currentPictureControlSetPtr->pic_avg_variance -
                          (int64_t)previousPictureControlSetPtr
                              ->pic_avg_variance)) > NOISE_VARIANCE_TH) &&
                    (currentPictureControlSetPtr->pic_avg_variance >
                         HIGH_PICTURE_VARIANCE_TH ||
                     previousPictureControlSetPtr->pic_avg_variance >
                         HIGH_PICTURE_VARIANCE_TH))
                    ? NOISY_SCENE_TH * NUM64x64INPIC(regionWidth, regionHeight)
            ...
            for (bin = 0; bin < HISTOGRAM_NUMBER_OF_BINS; ++bin) {
                ahd += ABS((int32_t)currentPictureControlSetPtr
                               ->picture_histogram[regionInPictureWidthIndex]
                                                  [regionInPictureHeightIndex]
                                                  [0][bin] -
                           (int32_t)previousPictureControlSetPtr
                               ->picture_histogram[regionInPictureWidthIndex]
                                                  [regionInPictureHeightIndex]
                                                  [0][bin]);
                ahdCb += ABS((int32_t)currentPictureControlSetPtr
                                 ->picture_histogram[regionInPictureWidthIndex]
                                                    [regionInPictureHeightIndex]
                                                    [1][bin] -
                             (int32_t)previousPictureControlSetPtr
                                 ->picture_histogram[regionInPictureWidthIndex]
                                                    [regionInPictureHeightIndex]
                                                    [1][bin]);
                ahdCr += ABS((int32_t)currentPictureControlSetPtr
                                 ->picture_histogram[regionInPictureWidthIndex]
                                                    [regionInPictureHeightIndex]
                                                    [2][bin] -
                             (int32_t)previousPictureControlSetPtr
            ...
            if (context_ptr->reset_running_avg) {
                ahd_running_avg[regionInPictureWidthIndex]
                               [regionInPictureHeightIndex] = ahd;
                ahd_running_avg_cb[regionInPictureWidthIndex]
                                  [regionInPictureHeightIndex] = ahdCb;
                ahd_running_avg_cr[regionInPictureWidthIndex]
                                  [regionInPictureHeightIndex] = ahdCr;
            }

            ahdError =
                ABS((int32_t)ahd_running_avg[regionInPictureWidthIndex]
                                            [regionInPictureHeightIndex] -
                    (int32_t)ahd);
            ahdErrorCb =
                ABS((int32_t)ahd_running_avg_cb[regionInPictureWidthIndex]
                                               [regionInPictureHeightIndex] -
                    (int32_t)ahdCb);
            ahdErrorCr =
                ABS((int32_t)ahd_running_avg_cr[regionInPictureWidthIndex]
                                               [regionInPictureHeightIndex] -
                    (int32_t)ahdCr);
            ...
            if (isAbruptChange) {
                aidFuturePast = (uint8_t)ABS(
                    (int16_t)
                        futurePictureControlSetPtr->average_intensity_per_region
                            [regionInPictureWidthIndex]
                            [regionInPictureHeightIndex][0] -
                    (int16_t)previousPictureControlSetPtr
                        ->average_intensity_per_region
                            [regionInPictureWidthIndex]
                            [regionInPictureHeightIndex][0]);
                aidFuturePresent = (uint8_t)ABS(
                    (int16_t)
                        futurePictureControlSetPtr->average_intensity_per_region
                            [regionInPictureWidthIndex]
                            [regionInPictureHeightIndex][0] -
                    (int16_t)currentPictureControlSetPtr
                        ->average_intensity_per_region
                            [regionInPictureWidthIndex]
                            [regionInPictureHeightIndex][0]);
                aidPresentPast =
                    (uint8_t)ABS((int16_t)currentPictureControlSetPtr
                                     ->average_intensity_per_region
                                         [regionInPictureWidthIndex]
                                         [regionInPictureHeightIndex][0] -
                                 (int16_t)previousPictureControlSetPtr
                ...
                if (aidFuturePast < FLASH_TH && aidFuturePresent >= FLASH_TH &&
                    aidPresentPast >= FLASH_TH) {
                    isFlash = EB_TRUE;
                    // printf ("\nFlash in frame# %i , %i\n",
                    // currentPictureControlSetPtr->picture_number,aidFuturePast);
                } else if (aidFuturePresent < FADE_TH &&
                           aidPresentPast < FADE_TH) {
                    isFade = EB_TRUE;
                    // printf ("\nFlash in frame# %i , %i\n",
                    // currentPictureControlSetPtr->picture_number,aidFuturePast);
                } else {
                ...
                if (aidFuturePast < FLASH_TH) {
                    ...
                    ahd_running_avg
                        [regionInPictureWidthIndex]
                        [regionInPictureHeightIndex] =
                            (3 * ahd_running_avg[regionInPictureWidthIndex]
                                                [regionInPictureHeightIndex] +
                             ahd) /
                            4;
                } else {
                    // proper action to be signalled
                    // printf ("\nLight Scene Change / fade detected in frame#
                    // %i , %i\n",
                    // currentPictureControlSetPtr->picture_number,aidFuturePast);
                    ahd_running_avg
                        [regionInPictureWidthIndex]
    ...
    if ((isSceneChangeCount >= regionCountThreshold) &&
        ((!ParentPcsWindow[1]->fade_in_to_black) &&
         (!ParentPcsWindow[1]->fade_out_from_black)))
        return (EB_TRUE);
    else
        return (EB_FALSE);
}
...
EbErrorType ReleasePrevPictureFromReorderQueue(
    ...
    // Get the previous entry from the Picture Decision Reordering Queue (Entry
    // N-1) P.S. The previous entry in display order is needed for Scene Change
    // Detection
    previousEntryIndex =
        (encode_context_ptr->picture_decision_reorder_queue_head_index == 0)
            ? PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH - 1
            : encode_context_ptr->picture_decision_reorder_queue_head_index - 1;
    queuePreviousEntryPtr =
        encode_context_ptr->picture_decision_reorder_queue[previousEntryIndex];

    // SB activity classification based on (0,0) SAD & picture activity
    // derivation
    if (queuePreviousEntryPtr->parent_pcs_wrapper_ptr) {
        // Reset the Picture Decision Reordering Queue Entry
        // P.S. The reset of the Picture Decision Reordering Queue Entry could
        // not be done before running the Scene Change Detector
        queuePreviousEntryPtr->picture_number +=
            PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH;
        queuePreviousEntryPtr->parent_pcs_wrapper_ptr =
...
EbErrorType initialize_mini_gop_activity_array(
    ...
    for (MiniGopIndex = 0; MiniGopIndex < MINI_GOP_MAX_COUNT; ++MiniGopIndex) {
        context_ptr->mini_gop_activity_array[MiniGopIndex] =
            (get_mini_gop_stats(MiniGopIndex)->hierarchical_levels ==
 ...
 *
 ***************************************************************************************************/
EbErrorType generate_picture_window_split(PictureDecisionContext *context_ptr,
                                          EncodeContext *encode_context_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    while (MiniGopIndex < MINI_GOP_MAX_COUNT) {
        ...
        if (get_mini_gop_stats(MiniGopIndex)->end_index <
                encode_context_ptr->pre_assignment_buffer_count &&
            context_ptr->mini_gop_activity_array[MiniGopIndex] == EB_FALSE) {
            context_ptr
                ->mini_gop_start_index[context_ptr->total_number_of_mini_gops] =
                get_mini_gop_stats(MiniGopIndex)->start_index;
            context_ptr
                ->mini_gop_end_index[context_ptr->total_number_of_mini_gops] =
                get_mini_gop_stats(MiniGopIndex)->end_index;
            context_ptr
                ->mini_gop_length[context_ptr->total_number_of_mini_gops] =
                get_mini_gop_stats(MiniGopIndex)->lenght;
            context_ptr->mini_gop_hierarchical_levels
                [context_ptr->total_number_of_mini_gops] =
                get_mini_gop_stats(MiniGopIndex)->hierarchical_levels;
            context_ptr
                ->mini_gop_intra_count[context_ptr->total_number_of_mini_gops] =
                0;
            context_ptr
                ->mini_gop_idr_count[context_ptr->total_number_of_mini_gops] =
                0;
        ...
        MiniGopIndex += context_ptr->mini_gop_activity_array[MiniGopIndex]
                            ? 1
                            : mini_gop_offset[get_mini_gop_stats(MiniGopIndex)
                                                  ->hierarchical_levels -
                                              MIN_HIERARCHICAL_LEVEL];
    }
    ...
    if (context_ptr->total_number_of_mini_gops != 0) {
        context_ptr
            ->mini_gop_intra_count[context_ptr->total_number_of_mini_gops - 1] =
            encode_context_ptr->pre_assignment_buffer_intra_count;
        context_ptr
            ->mini_gop_idr_count[context_ptr->total_number_of_mini_gops - 1] =
            encode_context_ptr->pre_assignment_buffer_idr_count;
    }
...
EbErrorType handle_incomplete_picture_window_map(
    PictureDecisionContext *context_ptr, EncodeContext *encode_context_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    if (context_ptr->total_number_of_mini_gops == 0) {
        context_ptr
            ->mini_gop_start_index[context_ptr->total_number_of_mini_gops] = 0;
        context_ptr
            ->mini_gop_end_index[context_ptr->total_number_of_mini_gops] =
            encode_context_ptr->pre_assignment_buffer_count - 1;
        context_ptr->mini_gop_length[context_ptr->total_number_of_mini_gops] =
            encode_context_ptr->pre_assignment_buffer_count -
            context_ptr
                ->mini_gop_start_index[context_ptr->total_number_of_mini_gops];
        context_ptr->mini_gop_hierarchical_levels
            [context_ptr->total_number_of_mini_gops] =
            3;  // MIN_HIERARCHICAL_LEVEL; // AMIR to be updated after other
 ...
 *the 2 different prediction structures Clean up the reference queue dependant
 *counts of the base layer frame separating the 2 different prediction
 *structures
 *
 ***************************************************************************************************/
EbErrorType update_base_layer_reference_queue_dependent_count(
    PictureDecisionContext *context_ptr, EncodeContext *encode_context_ptr,
    SequenceControlSet *sequence_control_set_ptr, uint32_t MiniGopIndex) {
    ...
    picture_control_set_ptr =
        (PictureParentControlSet *)encode_context_ptr
            ->pre_assignment_buffer[context_ptr
                                        ->mini_gop_start_index[MiniGopIndex]]
            ->object_ptr;

    // Derive the temporal layer difference between the current mini GOP and the
    // previous mini GOP
    picture_control_set_ptr->hierarchical_layers_diff =
        (uint8_t)(encode_context_ptr->previous_mini_gop_hierarchical_levels -
    ...
    picture_control_set_ptr->init_pred_struct_position_flag =
        (picture_control_set_ptr->hierarchical_layers_diff != 0) ? EB_TRUE
                                                                 : EB_FALSE;

    // If the current mini GOP is different than the previous mini GOP update
    // then update the positive dependant counts of the reference entry
    // separating the 2 mini GOPs
    if (picture_control_set_ptr->hierarchical_layers_diff != 0) {
        input_queue_index =
            encode_context_ptr->picture_decision_pa_reference_queue_head_index;

        while (input_queue_index !=
               encode_context_ptr
                   ->picture_decision_pa_reference_queue_tail_index) {
            input_entry_ptr =
                encode_context_ptr
                    ->picture_decision_pa_reference_queue[input_queue_index];

            // Find the reference entry separating the 2 mini GOPs
            // (picture_control_set_ptr->picture_number is the POC of the first
            // isput in the mini GOP)
            if (input_entry_ptr->picture_number ==
                (picture_control_set_ptr->picture_number - 1)) {
                // Update the positive dependant counts

                // 1st step: remove all positive entries from the dependant
                // list0 and dependant list1
                dependant_list_positive_entries = 0;
                for (dep_idx = 0; dep_idx < input_entry_ptr->list0.list_count;
                     ++dep_idx) {
                    if (input_entry_ptr->list0.list[dep_idx] >= 0)
                        dependant_list_positive_entries++;
                }
                input_entry_ptr->list0.list_count =
                    input_entry_ptr->list0.list_count -
                    dependant_list_positive_entries;
                dependant_list_positive_entries = 0;
                for (dep_idx = 0; dep_idx < input_entry_ptr->list1.list_count;
                     ++dep_idx) {
                    if (input_entry_ptr->list1.list[dep_idx] >= 0)
                        dependant_list_positive_entries++;
                }
                input_entry_ptr->list1.list_count =
                    input_entry_ptr->list1.list_count -
                    dependant_list_positive_entries;

                // 2nd step: inherit the positive dependant counts of the
                // current mini GOP Get the RPS set of the current mini GOP
                next_pred_struct_ptr = get_prediction_structure(
                    ...
                    picture_control_set_ptr
                        ->hierarchical_levels);  // Number of temporal layer in
                                                 // the current mini GOP
                ...
                for (dep_idx = 0;
                     dep_idx <
                     next_base_layer_pred_position_ptr->dep_list0.list_count;
                     ++dep_idx) {
                    if (next_base_layer_pred_position_ptr->dep_list0
                            .list[dep_idx] >= 0)
                        input_entry_ptr->list0
                            .list[input_entry_ptr->list0.list_count++] =
                            next_base_layer_pred_position_ptr->dep_list0
                                .list[dep_idx];
                }

                for (dep_idx = 0;
                     dep_idx <
                     next_base_layer_pred_position_ptr->dep_list1.list_count;
                     ++dep_idx) {
                    if (next_base_layer_pred_position_ptr->dep_list1
                            .list[dep_idx] >= 0)
                        input_entry_ptr->list1
                            .list[input_entry_ptr->list1.list_count++] =
                            next_base_layer_pred_position_ptr->dep_list1
                                .list[dep_idx];
                }

                // 3rd step: update the dependant count
                dependant_list_removed_entries =
                    input_entry_ptr->dep_list0_count +
                    input_entry_ptr->dep_list1_count -
                    input_entry_ptr->dependent_count;
                input_entry_ptr->dep_list0_count =
                    (input_entry_ptr->is_alt_ref)
                        ? input_entry_ptr->list0.list_count + 1
                        : input_entry_ptr->list0.list_count;
                input_entry_ptr->dep_list1_count =
                    input_entry_ptr->list1.list_count;
                input_entry_ptr->dependent_count =
                ...
                for (dep_idx = 0; dep_idx < dep_list_count; ++dep_idx) {
                    // Adjust the latest currentInputPoc in case we're in a POC
                    // rollover scenario currentInputPoc += (currentInputPoc <
                    // input_entry_ptr->pocNumber) ? (1 <<
                    // sequence_control_set_ptr->bitsForPictureOrderCount) : 0;
            ...
            input_queue_index =
                (input_queue_index ==
                 PICTURE_DECISION_PA_REFERENCE_QUEUE_MAX_DEPTH - 1)
                    ? 0
                    : input_queue_index + 1;
        }
    }
...
EbBool is_supposedly_4L_reference_frame(PictureDecisionContext *context_ptr,
                                        uint32_t MiniGopIndex,
                                        uint32_t picture_index) {
    if ((context_ptr->mini_gop_hierarchical_levels[MiniGopIndex] == 4 &&
         context_ptr->mini_gop_length[MiniGopIndex] == 16 &&
         (picture_index == 7 ||
          picture_index == 23)) ||  // supposedly a 4L reference frame for 5L
                                    // prediction structure
        (context_ptr->mini_gop_hierarchical_levels[MiniGopIndex] == 5 &&
         context_ptr->mini_gop_length[MiniGopIndex] == 32 &&
         (picture_index == 7 ||
          picture_index == 23))) {  // supposedly a 4L reference frame for 6L
                                    // prediction structure
        return (EB_TRUE);
    } else
        return (EB_FALSE);
 ...
 *
 ***************************************************************************************************/
EbErrorType GenerateMiniGopRps(PictureDecisionContext *context_ptr,
                               EncodeContext *encode_context_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    SequenceControlSet *sequence_control_set_ptr;
    ...
    // Loop over all mini GOPs
    for (mini_gop_index = 0;
         mini_gop_index < context_ptr->total_number_of_mini_gops;
         ++mini_gop_index) {
        // Loop over picture within the mini GOP
        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index];
             pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index];
             pictureIndex++) {
            picture_control_set_ptr =
                (PictureParentControlSet *)encode_context_ptr
                    ->pre_assignment_buffer[pictureIndex]
                    ->object_ptr;
            sequence_control_set_ptr =
                (SequenceControlSet *)picture_control_set_ptr
                    ->sequence_control_set_wrapper_ptr->object_ptr;
            picture_control_set_ptr->pred_structure =
                sequence_control_set_ptr->static_config.pred_structure;
            picture_control_set_ptr->hierarchical_levels =
                (uint8_t)
...
EbErrorType signal_derivation_multi_processes_oq(
    ...
    uint8_t enc_mode_hme = sequence_control_set_ptr->use_output_stat_file
                               ? picture_control_set_ptr->snd_pass_enc_mode
                               : picture_control_set_ptr->enc_mode;
    picture_control_set_ptr->enable_hme_flag =
        enable_hme_flag[picture_control_set_ptr->sc_content_detected]
                       [sequence_control_set_ptr->input_resolution]
                       [enc_mode_hme];

    picture_control_set_ptr->enable_hme_level0_flag =
        enable_hme_level0_flag[picture_control_set_ptr->sc_content_detected]
                              [sequence_control_set_ptr->input_resolution]
                              [enc_mode_hme];
    picture_control_set_ptr->enable_hme_level1_flag =
        enable_hme_level1_flag[picture_control_set_ptr->sc_content_detected]
                              [sequence_control_set_ptr->input_resolution]
                              [enc_mode_hme];
    picture_control_set_ptr->enable_hme_level2_flag =
        enable_hme_level2_flag[picture_control_set_ptr->sc_content_detected]
                              [sequence_control_set_ptr->input_resolution]
                              [enc_mode_hme];

    picture_control_set_ptr->tf_enable_hme_flag =
        tf_enable_hme_flag[picture_control_set_ptr->sc_content_detected]
                          [sequence_control_set_ptr->input_resolution]
                          [enc_mode_hme];
    picture_control_set_ptr->tf_enable_hme_level0_flag =
        tf_enable_hme_level0_flag[picture_control_set_ptr->sc_content_detected]
                                 [sequence_control_set_ptr->input_resolution]
                                 [enc_mode_hme];
    picture_control_set_ptr->tf_enable_hme_level1_flag =
        tf_enable_hme_level1_flag[picture_control_set_ptr->sc_content_detected]
                                 [sequence_control_set_ptr->input_resolution]
                                 [enc_mode_hme];
    picture_control_set_ptr->tf_enable_hme_level2_flag =
        tf_enable_hme_level2_flag[picture_control_set_ptr->sc_content_detected]
                                 [sequence_control_set_ptr->input_resolution]
                                 [enc_mode_hme];
#else
    picture_control_set_ptr->enable_hme_flag =
    ...
    if (sc_content_detected)
        ...
        if (picture_control_set_ptr->slice_type == I_SLICE)
            ...
            picture_control_set_ptr->pic_depth_mode = PIC_ALL_C_DEPTH_MODE;
            // Jing: TODO:
            // In M3, sb_sz may be 128x128, and init_sq_non4_block only works
            // for 64x64 sb size
#endif
...
// These are the default settings to use
    ...
    // Jing: TODO:
    // In M3, sb_sz may be 128x128, and init_sq_non4_block only works for 64x64
    // sb size
    else if (picture_control_set_ptr->enc_mode <= ENC_M3)
        if (picture_control_set_ptr->slice_type == I_SLICE)
            picture_control_set_ptr->pic_depth_mode = PIC_ALL_C_DEPTH_MODE;
        else if (sequence_control_set_ptr->seq_header.sb_size == BLOCK_128X128)
            picture_control_set_ptr->pic_depth_mode = PIC_SQ_DEPTH_MODE;
        else
            picture_control_set_ptr->pic_depth_mode = PIC_SQ_NON4_DEPTH_MODE;
    ...
    if (picture_control_set_ptr->pic_depth_mode < PIC_SQ_DEPTH_MODE)
        assert(sequence_control_set_ptr->nsq_present == 1 &&
               "use nsq_present 1");
    ...
    if (sequence_control_set_ptr->static_config.olpd_refinement ==
        -1) {  // auto mode; if not set by cfg
        ...
        picture_control_set_ptr->enable_adaptive_ol_partitioning =
            sequence_control_set_ptr->static_config.olpd_refinement;
    }
#else
    ...
    if (MR_MODE || sc_content_detected)
        ...
        picture_control_set_ptr->mdc_depth_level =
            (sequence_control_set_ptr->input_resolution ==
    ...
    // NSQ_SEARCH_OFF                                 OFF
    // NSQ_SEARCH_LEVEL1                              Allow only NSQ
    // Inter-NEAREST/NEAR/GLOBAL if parent SQ has no coeff + reordering
    // nsq_table number and testing only 1 NSQ SHAPE NSQ_SEARCH_LEVEL2 Allow
    // only NSQ Inter-NEAREST/NEAR/GLOBAL if parent SQ has no coeff + reordering
    // nsq_table number and testing only 2 NSQ SHAPE NSQ_SEARCH_LEVEL3 Allow
    // only NSQ Inter-NEAREST/NEAR/GLOBAL if parent SQ has no coeff + reordering
    // nsq_table number and testing only 3 NSQ SHAPE NSQ_SEARCH_LEVEL4 Allow
    // only NSQ Inter-NEAREST/NEAR/GLOBAL if parent SQ has no coeff + reordering
    // nsq_table number and testing only 4 NSQ SHAPE NSQ_SEARCH_LEVEL5 Allow
    // only NSQ Inter-NEAREST/NEAR/GLOBAL if parent SQ has no coeff + reordering
    // nsq_table number and testing only 5 NSQ SHAPE NSQ_SEARCH_LEVEL6 Allow
    // only NSQ Inter-NEAREST/NEAR/GLOBAL if parent SQ has no coeff + reordering
    // nsq_table number and testing only 6 NSQ SHAPE NSQ_SEARCH_FULL Allow NSQ
    // Intra-FULL and Inter-FULL

    if (MR_MODE)
        picture_control_set_ptr->nsq_search_level = NSQ_SEARCH_FULL;
#if MULTI_PASS_PD
    else if (picture_control_set_ptr->pic_depth_mode ==
...
#endif
            picture_control_set_ptr->nsq_search_level =
    ...
    case NSQ_SEARCH_OFF: picture_control_set_ptr->nsq_max_shapes_md = 0; break;
    ...
    case NSQ_SEARCH_LEVEL7:
        ...
        break;
#endif
    case NSQ_SEARCH_FULL: picture_control_set_ptr->nsq_max_shapes_md = 6; break;
    default: printf("nsq_search_level is not supported\n"); break;
    }
    ...
    // blind interpolation search at fast loop 4 Interpolation search at fast
    // loop

    if (MR_MODE)
        picture_control_set_ptr->interpolation_search_level =
            IT_SEARCH_FAST_LOOP;
    else if (sc_content_detected)
...
#if PAL_SUP
        // this will enable sc tools for P frames. hence change bitstream even
        // if palette mode is OFF
        frm_hdr->allow_screen_content_tools =
            picture_control_set_ptr->sc_content_detected;
#else
    ...
    if (frm_hdr->allow_screen_content_tools)
        if (sequence_control_set_ptr->static_config.enable_palette ==
            -1)  // auto mode; if not set by cfg
                ...
                (sequence_control_set_ptr->static_config.encoder_bit_depth ==
                 ...
                 (sequence_control_set_ptr->static_config.encoder_bit_depth >
                  ...
                  sequence_control_set_ptr->static_config
                          .enable_hbd_mode_decision == 0)) &&
                        picture_control_set_ptr->enc_mode == ENC_M0
                    ? 6
                    : 0;
        else
            picture_control_set_ptr->palette_mode =
                sequence_control_set_ptr->static_config.enable_palette;
    else
        picture_control_set_ptr->palette_mode = 0;

    assert(picture_control_set_ptr->palette_mode < 7);
#endif
    if (!picture_control_set_ptr->sequence_control_set_ptr->static_config
             .disable_dlf_flag &&
        frm_hdr->allow_intrabc == 0) {
        if (sc_content_detected)
#if M0_OPT
...
#endif
                picture_control_set_ptr->loop_filter_mode =
                    picture_control_set_ptr->is_used_as_reference_flag ? 3 : 0;
            else
            ...
            picture_control_set_ptr->loop_filter_mode =
                picture_control_set_ptr->is_used_as_reference_flag ? 1 : 0;
    } else
        picture_control_set_ptr->loop_filter_mode = 0;
    // CDEF Level                                   Settings
    // 0                                            OFF
    ...
    if (sequence_control_set_ptr->seq_header.enable_cdef &&
        frm_hdr->allow_intrabc == 0) {
        if (sc_content_detected)
#if PRESETS_TUNE
            if (picture_control_set_ptr->enc_mode <= ENC_M5)
...
#else
            picture_control_set_ptr->cdef_filter_mode =
                (picture_control_set_ptr->enc_mode <= ENC_M1) ? 5 : 4;
#endif
#else
            picture_control_set_ptr->cdef_filter_mode =
                (picture_control_set_ptr->enc_mode <= ENC_M0) ? 5 : 4;
#endif
#else
            picture_control_set_ptr->cdef_filter_mode = 4;
#endif
    ...
    // Set tx search skip weights (MAX_MODE_COST: no skipping; 0: always
    // skipping)
    ...
    // 0                                            FULL
    // 1                                            LIGHT per block :
    // disable_z2_prediction && disable_angle_refinement  for 64/32/4 2 OFF per
    // block : disable_angle_prediction for 64/32/4 3 OFF :
    // disable_angle_prediction 4                                            OIS
    // based Intra 5                                            Light OIS based
    // Intra
...
#if PRESETS_TUNE
            if ((picture_control_set_ptr->enc_mode <= ENC_M1) ||
                (picture_control_set_ptr->enc_mode <= ENC_M2 &&
    ...
    if (picture_control_set_ptr->sc_content_detected)
        picture_control_set_ptr->cu8x8_mode =
            (picture_control_set_ptr->temporal_layer_index > 0) ? CU_8x8_MODE_1
                                                                : CU_8x8_MODE_0;
    ...
    if (picture_control_set_ptr->enc_mode <= ENC_M1 &&
        ...
        picture_control_set_ptr->atb_mode =
            (MR_MODE || picture_control_set_ptr->temporal_layer_index == 0) ? 1
                                                                            : 0;
#else
    ...
    // 0                 FULL: Full search
    // 1                 Fast: If two predictors are very similar, skip wedge
    // compound mode search 2                 Fast: estimate Wedge sign 3 Fast:
    // Mode 1 & Mode 2
    ...
    if (sequence_control_set_ptr->static_config.inter_intra_compound == DEFAULT)
        picture_control_set_ptr->enable_inter_intra =
            picture_control_set_ptr->slice_type != I_SLICE
        ...
        picture_control_set_ptr->enable_inter_intra =
            sequence_control_set_ptr->static_config.inter_intra_compound;
#endif
    ...
    if (sequence_control_set_ptr->static_config.compound_level == DEFAULT) {
        if (sequence_control_set_ptr->compound_mode)
            ...
            if (picture_control_set_ptr->sc_content_detected)
                picture_control_set_ptr->compound_mode =
                    (picture_control_set_ptr->enc_mode <= ENC_M0) ? 2 : 0;
            else
                picture_control_set_ptr->compound_mode =
        ...
        if (picture_control_set_ptr->compound_mode)
            picture_control_set_ptr->compound_types_to_try =
                picture_control_set_ptr->compound_mode == 1 ? MD_COMP_DIFF0
                                                            : MD_COMP_WEDGE;
        else
        ...
        picture_control_set_ptr->compound_mode =
            sequence_control_set_ptr->static_config.compound_level;
    ...
    if (sequence_control_set_ptr->static_config.frame_end_cdf_update == DEFAULT)
        ...
        picture_control_set_ptr->frame_end_cdf_update_mode =
            sequence_control_set_ptr->static_config.frame_end_cdf_update;
    ...
    if (sequence_control_set_ptr->static_config.prune_unipred_me == DEFAULT)
        ...
        picture_control_set_ptr->prune_unipred_at_me =
            sequence_control_set_ptr->static_config.prune_unipred_me;
    ...
    if (picture_control_set_ptr->slice_type == I_SLICE)
        ...
        picture_control_set_ptr->frm_hdr.use_ref_frame_mvs =
            sequence_control_set_ptr->mfmv_enabled;
...
static void set_all_ref_frame_type(SequenceControlSet *sequence_control_set_ptr,
                                   PictureParentControlSet *parent_pcs_ptr,
                                   MvReferenceFrame ref_frame_arr[],
                                   uint8_t *tot_ref_frames) {
    MvReferenceFrame rf[2];
    *tot_ref_frames = 0;

    // printf("POC %i  totRef L0:%i   totRef L1: %i\n",
    // parent_pcs_ptr->picture_number, parent_pcs_ptr->ref_list0_count,
    // parent_pcs_ptr->ref_list1_count);

    // single ref - List0
    for (uint8_t ref_idx0 = 0; ref_idx0 < parent_pcs_ptr->ref_list0_count;
         ++ref_idx0) {
    ...
    // single ref - List1
    for (uint8_t ref_idx1 = 0; ref_idx1 < parent_pcs_ptr->ref_list1_count;
         ++ref_idx1) {
    ...
    if (sequence_control_set_ptr->mrp_mode == 0 &&
        parent_pcs_ptr->slice_type == B_SLICE) {
        // compound Uni-Dir
...
static void prune_refs(PredictionStructureEntry *pred_position_ptr,
                       Av1RpsNode *av1_rps) {
...
static EbBool set_frame_display_params(
    PictureParentControlSet *picture_control_set_ptr,
    PictureDecisionContext *context_ptr, uint32_t mini_gop_index) {
    Av1RpsNode *av1_rps = &picture_control_set_ptr->av1_ref_signal;
    FrameHeader *frm_hdr = &picture_control_set_ptr->frm_hdr;

    if (picture_control_set_ptr->pred_struct_ptr->pred_type ==
            EB_PRED_LOW_DELAY_P ||
    ...
    } else if (picture_control_set_ptr->pred_struct_ptr->pred_type ==
               EB_PRED_LOW_DELAY_B) {
        ...
        if (picture_control_set_ptr->slice_type == I_SLICE) {
            // 3 cases for I slice:  1:Key Frame treated above.  2: broken
            // MiniGop due to sc or intra refresh  3: complete miniGop due to sc
            // or intra refresh
            if (context_ptr->mini_gop_length[mini_gop_index] <
                picture_control_set_ptr->pred_struct_ptr->pred_struct_period) {
                // Scene Change that breaks the mini gop and switch to LDP (if I
                // scene change happens to be aligned with a complete miniGop,
            ...
            if (context_ptr->mini_gop_length[0] !=
                picture_control_set_ptr->pred_struct_ptr->pred_struct_period) {
...
static void set_key_frame_rps(PictureParentControlSet *pcs,
                              PictureDecisionContext *context_ptr) {
...
* 1) first Key frame
* 2) part of a complete RA MiniGop where the last frame could be a regular I for
open GOP
* 3) part of complete LDP MiniGop where the last frame could be Key frame for
closed GOP
* 4) part of non-complete LDP MiniGop where the last frame is a
regularI+SceneChange. This miniGOP has P frames with predStruct=LDP, and the
last frame=I with pred struct=RA.
* 5) part of non-complete LDP MiniGop at the end of the stream.This miniGOP has
P frames with predStruct=LDP, and the last frame=I with pred struct=RA.
*
*Note: the  SceneChange I has pred_type = EB_PRED_RANDOM_ACCESS. if SChange is
aligned on the miniGop, we do not break the GOP.
*************************************************/
static void Av1GenerateRpsInfo(PictureParentControlSet *picture_control_set_ptr,
                               EncodeContext *encode_context_ptr,
                               PictureDecisionContext *context_ptr,
                               uint32_t picture_index,
                               uint32_t mini_gop_index) {
    ...
    picture_control_set_ptr->intra_only =
    ...
    if (picture_control_set_ptr->hierarchical_levels == 0) {
        ...
        const uint8_t base0_idx = context_ptr->lay0_toggle == 0
                                      ? 1
                                      : context_ptr->lay0_toggle == 1 ? 2 : 0;
        const uint8_t base1_idx = context_ptr->lay0_toggle == 0
                                      ? 2
                                      : context_ptr->lay0_toggle == 1 ? 0 : 1;
        ...
        av1_rps->ref_dpb_index[ALT] = av1_rps->ref_dpb_index[LAST];
        gop_i = 0;
        av1_rps->ref_poc_array[LAST] =
            get_ref_poc(context_ptr,
                        picture_control_set_ptr->picture_number,
                        flat_pred_struct[gop_i].ref_list0[0]);
        av1_rps->ref_poc_array[LAST2] =
            get_ref_poc(context_ptr,
                        picture_control_set_ptr->picture_number,
                        flat_pred_struct[gop_i].ref_list0[1]);
        av1_rps->ref_poc_array[LAST3] = av1_rps->ref_poc_array[LAST];
        av1_rps->ref_poc_array[GOLD] = av1_rps->ref_poc_array[LAST];

        av1_rps->ref_poc_array[BWD] =
            get_ref_poc(context_ptr,
                        picture_control_set_ptr->picture_number,
                        flat_pred_struct[gop_i].ref_list1[0]);
        av1_rps->ref_poc_array[ALT2] =
            get_ref_poc(context_ptr,
                        picture_control_set_ptr->picture_number,
                        flat_pred_struct[gop_i].ref_list1[1]);
        av1_rps->ref_poc_array[ALT] = av1_rps->ref_poc_array[BWD];
        ...
        set_frame_display_params(
            picture_control_set_ptr, context_ptr, mini_gop_index);
        frm_hdr->show_frame = EB_TRUE;
        picture_control_set_ptr->has_show_existing = EB_FALSE;
        ...
        const uint8_t base1_idx = context_ptr->lay0_toggle == 0
                                      ? 2
                                      : context_ptr->lay0_toggle == 1 ? 0 : 1;
        const uint8_t base2_idx = context_ptr->lay0_toggle == 0
                                      ? 0
                                      : context_ptr->lay0_toggle == 1 ? 1 : 2;
        ...
        case 0:
            ...
            av1_rps->ref_dpb_index[ALT] = av1_rps->ref_dpb_index[LAST];
            gop_i = 0;
            av1_rps->ref_poc_array[LAST] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                two_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
            av1_rps->ref_poc_array[LAST2] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                two_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1_rps->ref_poc_array[LAST3] = av1_rps->ref_poc_array[LAST];
            av1_rps->ref_poc_array[GOLD] = av1_rps->ref_poc_array[LAST];

            av1_rps->ref_poc_array[BWD] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                two_level_hierarchical_pred_struct[gop_i].ref_list1[0]);
            av1_rps->ref_poc_array[ALT2] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                two_level_hierarchical_pred_struct[gop_i].ref_list1[1]);
            av1_rps->ref_poc_array[ALT] = av1_rps->ref_poc_array[BWD];
        ...
        default: printf("Error: unexpected picture mini Gop number\n"); break;
        }

        prune_refs(pred_position_ptr, av1_rps);

        if (!set_frame_display_params(
                picture_control_set_ptr, context_ptr, mini_gop_index)) {
            if (picture_control_set_ptr->is_used_as_reference_flag &&
                picture_index != 0) {
                frm_hdr->show_frame = EB_FALSE;
        ...
        const uint8_t lay1_0_idx =
            context_ptr->lay1_toggle == 0 ? LAY1_OFF + 1 : LAY1_OFF + 0;
        const uint8_t lay1_1_idx =
            context_ptr->lay1_toggle == 0 ? LAY1_OFF + 0 : LAY1_OFF + 1;
        ...
        case 0:
            ...
            av1_rps->ref_poc_array[LAST] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                three_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
            av1_rps->ref_poc_array[LAST2] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                three_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1_rps->ref_poc_array[LAST3] = av1_rps->ref_poc_array[LAST];
            av1_rps->ref_poc_array[GOLD] = av1_rps->ref_poc_array[LAST];
            ...
            av1_rps->ref_poc_array[BWD] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                three_level_hierarchical_pred_struct[gop_i].ref_list1[0]);
            av1_rps->ref_poc_array[ALT2] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                three_level_hierarchical_pred_struct[gop_i].ref_list1[1]);
            av1_rps->ref_poc_array[ALT] = av1_rps->ref_poc_array[BWD];
        ...
        case 1:
            ...
            av1_rps->ref_poc_array[LAST2] = get_ref_poc(
                ...
                picture_control_set_ptr->picture_number,
                three_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1_rps->ref_poc_array[LAST3] = get_ref_poc(
                context_ptr,
                picture_control_set_ptr->picture_number,
                three_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
            av1_rps->ref_poc_array[GOLD] = av1_rps->ref_poc_array[LAST];
            ...
            av1_rps->refresh_frame_mask =
                1 << (LAY1_OFF + context_ptr->lay1_toggle);
        ...
        case 2:
            ...
            if (picture_control_set_ptr->is_overlay) {
                ...
                av1_rps->ref_poc_array[LAST] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
                av1_rps->ref_poc_array[LAST2] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
                av1_rps->ref_poc_array[LAST3] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
                av1_rps->ref_poc_array[GOLD] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
                av1_rps->ref_poc_array[BWD] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
                av1_rps->ref_poc_array[ALT2] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
                av1_rps->ref_poc_array[ALT] = get_ref_poc(
                    context_ptr, picture_control_set_ptr->picture_number, 0);
            } else {
                if (picture_index == 0) {
                    ...
                    av1_rps->ref_poc_array[LAST] =
                        get_ref_poc(context_ptr,
                                    picture_control_set_ptr->picture_number,
                                    three_level_hierarchical_pred_struct[gop_i]
                                        .ref_list0[0]);
                    av1_rps->ref_poc_array[LAST2] =
                        get_ref_poc(context_ptr,
                                    picture_control_set_ptr->picture_number,
        ...
        if (picture_control_set_ptr->pred_struct_ptr->pred_type ==
            ...
            picture_control_set_ptr->sequence_control_set_ptr->static_config
                    .pred_structure == EB_PRED_RANDOM_ACCESS) {
            is_trailing_frames = EB_TRUE;
        }

        const uint8_t base0_idx =
            context_ptr->lay0_toggle == 0
                ? 1
                : context_ptr->lay0_toggle == 1
                      ? 2
                      : 0;  // the oldest L0 picture in the DPB
        const uint8_t base1_idx =
            context_ptr->lay0_toggle == 0
                ? 2
                : context_ptr->lay0_toggle == 1
                      ? 0
                      : 1;  // the middle L0 picture in the DPB
        const uint8_t base2_idx =
            context_ptr->lay0_toggle == 0
                ? 0
                : context_ptr->lay0_toggle == 1
                      ? 1
                      : 2;  // the newest L0 picture in the DPB

        const uint8_t lay1_0_idx =
            context_ptr->lay1_toggle == 0
                ? LAY1_OFF + 1
                : LAY1_OFF + 0;  // the oldest L1 picture in the DPB
        const uint8_t lay1_1_idx =
            context_ptr->lay1_toggle == 0
                ? LAY1_OFF + 0
                : LAY1_OFF + 1;  // the newest L1 picture in the DPB

        const uint8_t lay2_0_idx =
            picture_index < 4
                ? LAY2_OFF + 1
                : LAY2_OFF + 0;  // the oldest L2 picture in the DPB
        const uint8_t lay2_1_idx =
            picture_index < 4
                ? LAY2_OFF + 0
                : LAY2_OFF + 1;  // the newest L2 picture in the DPB
#if PRED_CHANGE
        const uint8_t lay3_idx = 7;  // the newest L3 picture in the DPB
#endif
        /*in 5L struct, we switich to  4L at the end of the seq.
        the current pred struct is reset, and the previous 5L minGop is out of
        reach! four_level_hierarchical_pred_struct will be set to follow 4L
        order, but this will generate some RPS mismatch for some frames.
        ...
        case 2:
            ...
            if (picture_index == 1) {
                ...
                av1_rps->ref_dpb_index[LAST2] = lay2_0_idx;
                av1_rps->ref_dpb_index[LAST3] = base1_idx;
                av1_rps->ref_dpb_index[GOLD] =
                    lay1_0_idx;  // av1_rps->ref_dpb_index[LAST];
            ...
            av1_rps->refresh_frame_mask =
                1 << (LAY2_OFF + context_ptr->lay2_toggle);
            // toggle 3->4
            if (!is_trailing_frames ||
                (context_ptr->mini_gop_length[mini_gop_index] >= 7 &&
                 is_trailing_frames &&
                 picture_control_set_ptr->sequence_control_set_ptr
                         ->static_config.pred_structure ==
                     EB_PRED_RANDOM_ACCESS)) {
                // For trailing frames, Only toggle it if we are sure we have 2
                // layer 2 frames in trailing frames
        ...
        // a regular I keeps the toggling process and does not reset the toggle.
        // K-0-1-0-1-0-K-0-1-0-1-K-0-1..... whoever needs a miniGOP Level
        // toggling, this is the time
        if (((picture_index ==
                  (context_ptr->mini_gop_end_index[mini_gop_index] % 8) &&
              !picture_control_set_ptr->is_overlay &&
              sequence_control_set_ptr->static_config.pred_structure ==
                  EB_PRED_RANDOM_ACCESS)) ||
            ((sequence_control_set_ptr->static_config.pred_structure ==
            ...
            if ((sequence_control_set_ptr->static_config.pred_structure ==
                     EB_PRED_LOW_DELAY_P ||
                 sequence_control_set_ptr->static_config.pred_structure ==
                     EB_PRED_LOW_DELAY_B) &&
                sequence_control_set_ptr->intra_period_length != -1 &&
                picture_control_set_ptr->cra_flag) {
                int32_t trailing_count =
                    (sequence_control_set_ptr->intra_period_length % 8);
        ...
        // Reset miniGop Toggling. The first miniGop after a KEY frame has
        // toggle=0
        if (frm_hdr->frame_type == KEY_FRAME) {
            set_key_frame_rps(picture_control_set_ptr, context_ptr);
            return;
        ...
        const uint8_t lay1_1_idx =
            context_ptr->lay1_toggle == 0
                ...
                : LAY1_OFF + 1;  // the newest L1 picture in the DPB
#if PRED_CHANGE_5L
        const uint8_t lay2_1_idx = LAY2_OFF;  // the oldest L2 picture in the
                                              // DPB
...
***************************************************************************************************/
void perform_simple_picture_analysis_for_overlay(
    PictureParentControlSet *picture_control_set_ptr) {
    ...
    paReferenceObject = (EbPaReferenceObject *)picture_control_set_ptr
                            ->pa_reference_picture_wrapper_ptr->object_ptr;
    input_padded_picture_ptr =
        (EbPictureBufferDesc *)paReferenceObject->input_padded_picture_ptr;
    picture_width_in_sb =
        (sequence_control_set_ptr->seq_header.max_frame_width +
         sequence_control_set_ptr->sb_sz - 1) /
        sequence_control_set_ptr->sb_sz;
    pictureHeighInLcu = (sequence_control_set_ptr->seq_header.max_frame_height +
                         sequence_control_set_ptr->sb_sz - 1) /
                        sequence_control_set_ptr->sb_sz;
    sb_total_count = picture_width_in_sb * pictureHeighInLcu;

    // Pad pictures to multiple min cu size
    PadPictureToMultipleOfMinCuSizeDimensions(sequence_control_set_ptr,
                                              input_picture_ptr);

    // Pre processing operations performed on the input picture
    PicturePreProcessingOperations(
        picture_control_set_ptr, sequence_control_set_ptr, sb_total_count);
    if (input_picture_ptr->color_format >= EB_YUV422) {
        // Jing: Do the conversion of 422/444=>420 here since it's
        // multi-threaded kernel
        //       Reuse the Y, only add cb/cr in the newly created buffer desc
        //       NOTE: since denoise may change the src, so this part is after
        //       PicturePreProcessingOperations()
        picture_control_set_ptr->chroma_downsampled_picture_ptr->buffer_y =
            input_picture_ptr->buffer_y;
        DownSampleChroma(
            input_picture_ptr,
            picture_control_set_ptr->chroma_downsampled_picture_ptr);
    } else
        picture_control_set_ptr->chroma_downsampled_picture_ptr =
            input_picture_ptr;
    // Pad input picture to complete border LCUs
    PadPictureToMultipleOfLcuDimensions(input_padded_picture_ptr);
    ...
    if (sequence_control_set_ptr->down_sampling_method_me_search ==
    ...
    // Gathering statistics of input picture, including Variance Calculation,
    // Histogram Bins
    GatheringPictureStatistics(
        sequence_control_set_ptr,
        picture_control_set_ptr,
        picture_control_set_ptr
            ->chroma_downsampled_picture_ptr,  // 420 input_picture_ptr
        input_padded_picture_ptr,
        paReferenceObject
            ->sixteenth_decimated_picture_ptr,  // Hsan: always use decimated
                                                // until studying the trade offs
        sb_total_count);

    picture_control_set_ptr->sc_content_detected =
        picture_control_set_ptr->alt_ref_ppcs_ptr->sc_content_detected;
}
/***************************************************************************************************
 * Initialize the overlay frame
 ***************************************************************************************************/
void initialize_overlay_frame(
    PictureParentControlSet *picture_control_set_ptr) {
    picture_control_set_ptr->fade_out_from_black =
        picture_control_set_ptr->alt_ref_ppcs_ptr->fade_out_from_black;
    picture_control_set_ptr->fade_in_to_black =
        picture_control_set_ptr->alt_ref_ppcs_ptr->fade_in_to_black;
    ...
    picture_control_set_ptr->target_bit_rate =
        picture_control_set_ptr->alt_ref_ppcs_ptr->target_bit_rate;
    picture_control_set_ptr->last_idr_picture =
        picture_control_set_ptr->alt_ref_ppcs_ptr->last_idr_picture;
    picture_control_set_ptr->use_rps_in_sps = EB_FALSE;
    picture_control_set_ptr->open_gop_cra_flag = EB_FALSE;
    picture_control_set_ptr->pred_structure =
        picture_control_set_ptr->alt_ref_ppcs_ptr->pred_structure;
    picture_control_set_ptr->pred_struct_ptr =
        picture_control_set_ptr->alt_ref_ppcs_ptr->pred_struct_ptr;
    picture_control_set_ptr->hierarchical_levels =
        picture_control_set_ptr->alt_ref_ppcs_ptr->hierarchical_levels;
    ...
    picture_control_set_ptr->pre_assignment_buffer_count =
        picture_control_set_ptr->alt_ref_ppcs_ptr->pre_assignment_buffer_count;

    perform_simple_picture_analysis_for_overlay(picture_control_set_ptr);
}

/***************************************************************************************************
 * Helper function. Compare two frames: center frame and target frame. Return
 *the summation of absolute difference between the two frames from a histogram
 *of luma values
 ***************************************************************************************************/

static __inline uint32_t compute_luma_sad_between_center_and_target_frame(
    int center_index, int target_frame_index,
    PictureParentControlSet *picture_control_set_ptr,
    SequenceControlSet *sequence_control_set_ptr) {
    ...
    for (int bin = 0; bin < HISTOGRAM_NUMBER_OF_BINS; ++bin) {
        ...
        for (uint32_t regionInPictureWidthIndex = 0;
            ...
            for (uint32_t regionInPictureHeightIndex = 0;
                 ...
                 regionInPictureHeightIndex++) {
                center_sum +=
                    picture_control_set_ptr->temp_filt_pcs_list[center_index]
                        ->picture_histogram[regionInPictureWidthIndex]
                                           [regionInPictureHeightIndex][0][bin];
                altref_sum +=
                    picture_control_set_ptr
                        ->temp_filt_pcs_list[target_frame_index]
                        ->picture_histogram[regionInPictureWidthIndex]
                                           [regionInPictureHeightIndex][0][bin];
            }
        }
 ...
 *including setting of the prediction structure, setting the picture type and
 ...
 *
 *  For Low Delay Sequences, pictures are started into the encoder pipeline
 *immediately.
 *
 *  For Random Access Sequences, pictures are held for up to a
 *PredictionStructurePeriod in order to determine if a Scene Change or Intra
 *Frame is forthcoming. Either of those events (and additionally a End of
 *Sequence Flag) will change the expected prediction structure.
 *
 *  Below is an example worksheet for how Intra Flags and Scene Change Flags
 *interact together to affect the prediction structure.
 *
 *  The base prediction structure for this example is a 3-Level Hierarchical
 *Random Access, Single Reference Prediction Structure:
 *
 *        b   b
 *       / \ / \
 *      /   B   \
 ...
 *            a
 *
 *  The RPS is composed of Leading Picture [L2-L0], Intra (CRA), Base/Normal
 *Pictures, and Trailing Pictures [T0-T2]. Generally speaking, Leading Pictures
 *are useful for handling scene changes without adding extraneous I-pictures and
 *the Trailing pictures are useful for terminating GOPs.
 *
 *  Here is a table of possible combinations of pictures needed to handle intra
 *and scene changes happening in quick succession.
 *
 ...
 *
 *   If there is an intra in the PredPeriod, then one of the above combinations
 *of Leading and Trailing pictures is used.  If there is no scene change, the
 *last valid column consisting of Trailing Pictures only is used.  However, if
 *there is an upcoming scene change before the next intra, then one of the above
 *patterns is used. In the case of End of Sequence flags, only the last valid
 *column of Trailing Pictures is used. The intention here is that any
 *combination of Intra Flag and Scene Change flag can be coded.
 *
 ***************************************************************************************************/
void *picture_decision_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_get_full_object(context_ptr->picture_analysis_results_input_fifo_ptr,
                           &inputResultsWrapperPtr);

        inputResultsPtr =
            (PictureAnalysisResults *)inputResultsWrapperPtr->object_ptr;
        picture_control_set_ptr =
        ...
        encode_context_ptr =
            (EncodeContext *)sequence_control_set_ptr->encode_context_ptr;
        loopCount++;

        // Input Picture Analysis Results into the Picture Decision Reordering
        // Queue P.S. Since the prior Picture Analysis processes stage is
        // multithreaded, inputs to the Picture Decision Process can arrive
        // out-of-display-order, so a the Picture Decision Reordering Queue is
        // used to enforce processing of pictures in display order
        if (!picture_control_set_ptr->is_overlay) {
            queueEntryIndex = (int32_t)(
                picture_control_set_ptr->picture_number -
                encode_context_ptr
                    ->picture_decision_reorder_queue
                        [encode_context_ptr
                             ->picture_decision_reorder_queue_head_index]
                    ->picture_number);
            queueEntryIndex +=
                encode_context_ptr->picture_decision_reorder_queue_head_index;
            queueEntryIndex =
                (queueEntryIndex > PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH - 1)
                    ? queueEntryIndex - PICTURE_DECISION_REORDER_QUEUE_MAX_DEPTH
                    : queueEntryIndex;
            queueEntryPtr =
                encode_context_ptr
                    ->picture_decision_reorder_queue[queueEntryIndex];
            if (queueEntryPtr->parent_pcs_wrapper_ptr != NULL) {
                CHECK_REPORT_ERROR_NC(encode_context_ptr->app_callback_ptr,
                                      EB_ENC_PD_ERROR8);
            } else {
                queueEntryPtr->parent_pcs_wrapper_ptr =
                    inputResultsPtr->picture_control_set_wrapper_ptr;
                queueEntryPtr->picture_number =
                    picture_control_set_ptr->picture_number;
            }

            picture_control_set_ptr->pic_decision_reorder_queue_idx =
                queueEntryIndex;
        }
        ...
        queueEntryPtr =
            encode_context_ptr->picture_decision_reorder_queue
                [encode_context_ptr->picture_decision_reorder_queue_head_index];

        while (queueEntryPtr->parent_pcs_wrapper_ptr != EB_NULL) {
            if (((PictureParentControlSet
            ...
            windowAvail = EB_TRUE;
            previousEntryIndex = QUEUE_GET_PREVIOUS_SPOT(
                encode_context_ptr->picture_decision_reorder_queue_head_index);

            ParentPcsWindow[0] = ParentPcsWindow[1] = ParentPcsWindow[2] =
                ParentPcsWindow[3] = ParentPcsWindow[4] = ParentPcsWindow[5] =
                    NULL;
            // for poc 0, ignore previous frame check
            if (queueEntryPtr->picture_number > 0 &&
                encode_context_ptr
                        ->picture_decision_reorder_queue[previousEntryIndex]
                        ->parent_pcs_wrapper_ptr == NULL)
                windowAvail = EB_FALSE;
            else {
                ParentPcsWindow[0] =
                    queueEntryPtr->picture_number > 0
                        ? (PictureParentControlSet *)encode_context_ptr
                              ->picture_decision_reorder_queue
                                  [previousEntryIndex]
...
#if SERIAL_MODE
                for (windowIndex = 0;
                     windowIndex < sequence_control_set_ptr->scd_delay;
                     windowIndex++) {
#else
                for (windowIndex = 0; windowIndex < FUTURE_WINDOW_WIDTH;
                     windowIndex++) {
#endif
                    entryIndex = QUEUE_GET_NEXT_SPOT(
                        encode_context_ptr
                            ->picture_decision_reorder_queue_head_index,
                        windowIndex + 1);
                    if (encode_context_ptr
                            ->picture_decision_reorder_queue[entryIndex]
                            ->parent_pcs_wrapper_ptr == NULL) {
                        windowAvail = EB_FALSE;
                        break;
                    } else if (((PictureParentControlSet
                                     *)(encode_context_ptr
                                            ->picture_decision_reorder_queue
                                                [entryIndex]
            ...
            if (windowAvail == EB_TRUE && queueEntryPtr->picture_number > 0) {
                if (sequence_control_set_ptr->static_config
                        .scene_change_detection) {
                    picture_control_set_ptr->scene_change_flag =
                        SceneTransitionDetector(context_ptr,
                                                sequence_control_set_ptr,
                                                ParentPcsWindow,
                                                FUTURE_WINDOW_WIDTH);
                } else
                ...
                context_ptr->is_scene_change_detected =
                    picture_control_set_ptr->scene_change_flag;
            }

            if (windowAvail == EB_TRUE || framePasseThru == EB_TRUE) {
                // Place the PCS into the Pre-Assignment Buffer
                // P.S. The Pre-Assignment Buffer is used to store a whole
                // pre-structure
                ...
                picture_control_set_ptr =
                    (PictureParentControlSet *)encode_context_ptr
                        ->pre_assignment_buffer
                            [encode_context_ptr->pre_assignment_buffer_count]
                        ->object_ptr;
                frm_hdr = &picture_control_set_ptr->frm_hdr;
                // Set the POC Number
                picture_control_set_ptr->picture_number =
                    (encode_context_ptr->current_input_poc +
                     1) /*& ((1 <<
                           sequence_control_set_ptr->bits_for_picture_order_count)-1)*/
                    ;
                encode_context_ptr->current_input_poc =
                    picture_control_set_ptr->picture_number;
                ...
                picture_control_set_ptr->target_bit_rate =
                    sequence_control_set_ptr->static_config.target_bit_rate;

                ReleasePrevPictureFromReorderQueue(encode_context_ptr);

                // If the Intra period length is 0, then introduce an intra for
                // every picture
                if (sequence_control_set_ptr->intra_period_length == 0)
                    picture_control_set_ptr->cra_flag = EB_TRUE;
                // If an #IntraPeriodLength has passed since the last Intra,
                // then introduce a CRA or IDR based on Intra Refresh type
                else if (sequence_control_set_ptr->intra_period_length != -1) {
                    picture_control_set_ptr->cra_flag =
                        (sequence_control_set_ptr->intra_refresh_type !=
                         CRA_REFRESH)
                            ? picture_control_set_ptr->cra_flag
                            : (encode_context_ptr->intra_period_position ==
                               (uint32_t)sequence_control_set_ptr
                                   ->intra_period_length)
                                  ? EB_TRUE
                                  : picture_control_set_ptr->cra_flag;
                ...
                encode_context_ptr->pre_assignment_buffer_eos_flag =
                    (picture_control_set_ptr->end_of_sequence_flag)
                        ? (uint32_t)EB_TRUE
                        : encode_context_ptr->pre_assignment_buffer_eos_flag;
                ...
                if (sequence_control_set_ptr->static_config.rate_control_mode) {
                    ...
                    encode_context_ptr->intra_period_position =
                        (encode_context_ptr->intra_period_position ==
                         (uint32_t)
                             sequence_control_set_ptr->intra_period_length)
                            ? 0
                ...
                // Determine if Pictures can be released from the Pre-Assignment
                // Buffer
                if ((encode_context_ptr->pre_assignment_buffer_intra_count >
                     0) ||
                    (encode_context_ptr->pre_assignment_buffer_count ==
                     (uint32_t)(1 << sequence_control_set_ptr->static_config
                                         .hierarchical_levels)) ||
                    (encode_context_ptr->pre_assignment_buffer_eos_flag ==
                     EB_TRUE) ||
                    (picture_control_set_ptr->pred_structure ==
                    ...
                    context_ptr->mini_gop_end_index[0] =
                        encode_context_ptr->pre_assignment_buffer_count - 1;
                    context_ptr->mini_gop_length[0] =
                        encode_context_ptr->pre_assignment_buffer_count;

                    context_ptr->mini_gop_hierarchical_levels[0] =
                        sequence_control_set_ptr->static_config
                            .hierarchical_levels;
                    context_ptr->mini_gop_intra_count[0] =
                        encode_context_ptr->pre_assignment_buffer_intra_count;
                    context_ptr->mini_gop_idr_count[0] =
                        encode_context_ptr->pre_assignment_buffer_idr_count;
                    context_ptr->total_number_of_mini_gops = 1;

                    encode_context_ptr->previous_mini_gop_hierarchical_levels =
                        (picture_control_set_ptr->picture_number == 0)
                            ? sequence_control_set_ptr->static_config
                                  .hierarchical_levels
                            : encode_context_ptr
...

                    {
                        if (sequence_control_set_ptr->static_config
                            ...
                            // minigop 2 case
                            context_ptr->mini_gop_start_index
                                [context_ptr->total_number_of_mini_gops] = 0;
                            context_ptr->mini_gop_end_index
                                [context_ptr->total_number_of_mini_gops] =
                                encode_context_ptr
                                    ->pre_assignment_buffer_count -
                                1;
                            context_ptr->mini_gop_length
                                [context_ptr->total_number_of_mini_gops] =
                                encode_context_ptr
                                    ->pre_assignment_buffer_count -
                                context_ptr->mini_gop_start_index
                                    [context_ptr->total_number_of_mini_gops];
                            context_ptr->mini_gop_hierarchical_levels
                                [context_ptr->total_number_of_mini_gops] = 2;
                        } else {
                            // minigop 4,8,16
                            if (encode_context_ptr
                                    ->pre_assignment_buffer_count > 1) {
                                initialize_mini_gop_activity_array(context_ptr);

                                if (encode_context_ptr
                                        ->pre_assignment_buffer_count == 16)
                                    context_ptr
                                        ->mini_gop_activity_array[L5_0_INDEX] =
                                        EB_FALSE;
                                else if (sequence_control_set_ptr->static_config
                                             .hierarchical_levels >= 3) {
                                    context_ptr
                                        ->mini_gop_activity_array[L4_0_INDEX] =
                                        EB_FALSE;
                                    context_ptr
                                        ->mini_gop_activity_array[L4_1_INDEX] =
                                        EB_FALSE;
                                }

                                generate_picture_window_split(
                                    context_ptr, encode_context_ptr);
                    ...
                    GenerateMiniGopRps(context_ptr, encode_context_ptr);
                    ...
                    for (mini_gop_index = 0;
                        ...
                        preAssignmentBufferFirstPassFlag = EB_TRUE;
                        {
                            ...
                            // Keep track of the number of hierarchical levels
                            // of the latest implemented mini GOP
                            encode_context_ptr
                                ->previous_mini_gop_hierarchical_levels =
                                context_ptr->mini_gop_hierarchical_levels
                                    [mini_gop_index];
                        ...
                        for (pictureIndex =
                                 context_ptr
                                     ->mini_gop_start_index[mini_gop_index];
                             pictureIndex <=
                             context_ptr->mini_gop_end_index[mini_gop_index];
                             ++pictureIndex) {
                            picture_control_set_ptr =
                            ...
                            sequence_control_set_ptr =
                                (SequenceControlSet *)picture_control_set_ptr
                                    ...
                                    ->object_ptr;
                            frm_hdr = &picture_control_set_ptr->frm_hdr;
                            // Keep track of the mini GOP size to which the
                            // input picture belongs - needed @
                            // PictureManagerProcess()
                            picture_control_set_ptr
                                ->pre_assignment_buffer_count =
                                context_ptr->mini_gop_length[mini_gop_index];

                            // Update the Pred Structure if cutting short a
                            // Random Access period
                            if ((context_ptr->mini_gop_length[mini_gop_index] <
                                     picture_control_set_ptr->pred_struct_ptr
                                         ->pred_struct_period ||
                                 context_ptr
                                         ->mini_gop_idr_count[mini_gop_index] >
                                     0) &&
                                picture_control_set_ptr->pred_struct_ptr
                                        ->pred_type == EB_PRED_RANDOM_ACCESS &&
                                picture_control_set_ptr->idr_flag == EB_FALSE &&
                                picture_control_set_ptr->cra_flag == EB_FALSE) {
                                // Correct the Pred Index before switching
                                // structures
                                if (preAssignmentBufferFirstPassFlag == EB_TRUE)
                                    encode_context_ptr->pred_struct_position -=
                                        picture_control_set_ptr->pred_struct_ptr
                                            ->init_pic_index;
                                picture_control_set_ptr->pred_struct_ptr =
                                    get_prediction_structure(
                                        encode_context_ptr
                                            ->prediction_structure_group_ptr,
                                        EB_PRED_LOW_DELAY_P,
                                        sequence_control_set_ptr
                                            ->reference_count,
                                        picture_control_set_ptr
                                ...
                                // Set the RPS Override Flag - this current only
                                // will convert a Random Access structure to a
                                // Low Delay structure
                                picture_control_set_ptr->use_rps_in_sps =
                                    EB_FALSE;
                                picture_control_set_ptr->open_gop_cra_flag =
                                    EB_FALSE;
                            ...
                            // Open GOP CRA - adjust the RPS
                                          ...
                                          ->mini_gop_length[mini_gop_index] ==
                                      picture_control_set_ptr->pred_struct_ptr
                                          ->pred_struct_period) &&
                                                       ...
                                                       EB_PRED_LOW_DELAY_B)
                                                          ...
                                                          : (picture_control_set_ptr
                                                             ...
                                                             picture_control_set_ptr
                                                                ...
                                                                (encode_context_ptr
                                                                     ->pre_assignment_buffer_eos_flag)
                                                                    ? P_SLICE
                                                                    : B_SLICE;
                            }
                            ...
                            encode_context_ptr->pred_struct_position =
                                (picture_control_set_ptr
                                     ->init_pred_struct_position_flag)
                                    ? picture_control_set_ptr->pred_struct_ptr
                                          ->init_pic_index
                                    : encode_context_ptr->pred_struct_position;
                            ...
                            if (picture_control_set_ptr->idr_flag == EB_TRUE)
                                encode_context_ptr->pred_struct_position =
                                    picture_control_set_ptr->pred_struct_ptr
                                        ->init_pic_index;
                            else if (picture_control_set_ptr->cra_flag ==
                                         EB_TRUE &&
                                     context_ptr
                                             ->mini_gop_length[mini_gop_index] <
                                         picture_control_set_ptr
                                ...
                                encode_context_ptr->pred_struct_position =
                                    picture_control_set_ptr->pred_struct_ptr
                                        ->init_pic_index;
                            else if (encode_context_ptr
                                         ->elapsed_non_cra_count == 0) {
                                // If we are the picture directly after a CRA,
                                // we have to not use references that violate
                                // the CRA
                                encode_context_ptr->pred_struct_position =
                                    picture_control_set_ptr->pred_struct_ptr
                                        ->init_pic_index +
                            ...
                            // Elif Scene Change, determine leading and trailing
                            // pictures
                            // else if
                            // (encode_context_ptr->pre_assignment_buffer_scene_change_count
                            // > 0) {
                            //    if(bufferIndex <
                            //    encode_context_ptr->pre_assignment_buffer_scene_change_index)
                            //    {
                            //        ++encode_context_ptr->pred_struct_position;
                            //        picture_type = P_SLICE;
                            //    }
                            ...
                            // Else, Increment the position normally
                                ...
                                ++encode_context_ptr->pred_struct_position;
                            // The poc number of the latest IDR picture is
                            // stored so that last_idr_picture (present in PCS)
                            // for the incoming pictures can be updated. The
                            // last_idr_picture is used in reseting the poc (in
                            // entropy coding) whenever IDR is encountered. Note
                            // IMP: This logic only works when display and
                            // decode order are the same. Currently for Random
                            // Access, IDR is inserted (similar to CRA) by using
                            // trailing P pictures (low delay fashion) and
                            // breaking prediction structure. Note: When leading
                            // P pictures are implemented, this logic has to
                            // change..
                            if (picture_control_set_ptr->idr_flag == EB_TRUE)
                                encode_context_ptr->last_idr_picture =
                                    picture_control_set_ptr->picture_number;
                            else
                                picture_control_set_ptr->last_idr_picture =
                                    encode_context_ptr->last_idr_picture;
                            ...
                            predPositionPtr =
                                picture_control_set_ptr->pred_struct_ptr
                                    ->pred_struct_entry_ptr_array
                                        [encode_context_ptr
                                             ->pred_struct_position];
                            if (sequence_control_set_ptr->static_config
                                    .enable_overlays == EB_TRUE) {
                                // At this stage we know the prediction
                                // structure and the location of ALT_REF
                                // pictures. For every ALTREF picture, there is
                                // an overlay picture. They extra pictures are
                                // released is_alt_ref flag is set for non-slice
                                // base layer pictures
                                if (predPositionPtr->temporal_layer_index ==
                                        0 &&
                                    picture_type != I_SLICE) {
                                    ...
                                    ((PictureParentControlSet
                                         ...
                                         ->pre_assignment_buffer[pictureIndex -
                                                                 1]
                                         ->object_ptr)
                                        ->has_show_existing = EB_FALSE;
                                }
                                // release the overlay PCS for non alt ref
                                // pictures. First picture does not have overlay
                                // PCS
                                else if (picture_control_set_ptr
                                             ->picture_number) {
                                    eb_release_object(
                                        picture_control_set_ptr
                                            ->overlay_ppcs_ptr
                                            ->input_picture_wrapper_ptr);
                                    // release the pa_reference_picture
                                    eb_release_object(
                                        picture_control_set_ptr
                                            ->overlay_ppcs_ptr
                                            ->pa_reference_picture_wrapper_ptr);
                                    // release the parent pcs
                                    eb_release_object(picture_control_set_ptr
                                                          ->overlay_ppcs_ptr
                                                          ->p_pcs_wrapper_ptr);
                                    picture_control_set_ptr->overlay_ppcs_ptr =
                                        EB_NULL;
                                }
                            }
                            PictureParentControlSet
                                *cur_picture_control_set_ptr =
                                    picture_control_set_ptr;
                            for (uint8_t loop_index = 0;
                                 loop_index <=
                                ...
                                case I_SLICE:
                                    ...
                                    // Reset Prediction Structure Position &
                                    // Reference Struct Position
                                    if (picture_control_set_ptr
                                            ->picture_number == 0)
                                        encode_context_ptr
                                            ->intra_period_position = 0;
                                    ...
                                    if (picture_control_set_ptr->idr_flag ==
                                        ...
                                        // Reset the pictures since last IDR
                                        // counter
                                        encode_context_ptr
                                            ->elapsed_non_idr_count = 0;
                                    ...
                                    //-------------------------------
                                        ...
                                        // Set a Random Access Point if not an
                                        // IDR
                                        picture_control_set_ptr->cra_flag =
                                            EB_TRUE;
                                    }
                                ...
                                case B_SLICE:
                                    ...
                                    if (loop_index == 0) {
                                        // Increment & Clip the elapsed Non-IDR
                                        // Counter. This is clipped rather than
                                        // allowed to free-run inorder to avoid
                                        // rollover issues.  This assumes that
                                        // any the GOP period is less than
                                        // MAX_ELAPSED_IDR_COUNT
                                        encode_context_ptr
                                            ->elapsed_non_idr_count = MIN(
                                            encode_context_ptr
                                                    ->elapsed_non_idr_count +
                                                1,
                                            MAX_ELAPSED_IDR_COUNT);
                                        encode_context_ptr
                                            ->elapsed_non_cra_count = MIN(
                                            encode_context_ptr
                                                    ->elapsed_non_cra_count +
                                                1,
                                            MAX_ELAPSED_IDR_COUNT);
                                    }
                                ...
                                picture_control_set_ptr->pred_struct_index =
                                    (uint8_t)encode_context_ptr
                                        ->pred_struct_position;
                                if (picture_control_set_ptr->is_overlay) {
                                    // set the overlay frame as non reference
                                    // frame with max temporal layer index
                                    picture_control_set_ptr
                                        ->temporal_layer_index =
                                        (uint8_t)picture_control_set_ptr
                                            ->hierarchical_levels;
                                    picture_control_set_ptr
                                        ->is_used_as_reference_flag = EB_FALSE;
                                } else {
                                    picture_control_set_ptr
                                        ->temporal_layer_index =
                                        (uint8_t)predPositionPtr
                                            ->temporal_layer_index;
                                    picture_control_set_ptr
                                        ->is_used_as_reference_flag =
                                        predPositionPtr->is_referenced;
                                }
                                ...
                                {
                                    uint16_t *fgn_random_seed_ptr =
                                        &picture_control_set_ptr
                                             ->sequence_control_set_ptr
                                             ->film_grain_random_seed;
                                    frm_hdr->film_grain_params.random_seed =
                                        *fgn_random_seed_ptr;
                                    *fgn_random_seed_ptr +=
                                        3381;  // Changing random seed for film
                                               // grain
                                    if (!(*fgn_random_seed_ptr))  // Random seed
                                                                  // should not
                                                                  // be zero
                                ...
                                if (sequence_control_set_ptr->static_config
                                    ...
                                    // For low delay P or low delay B case, get
                                    // the the picture_index by mini_gop size
                                    if (sequence_control_set_ptr->static_config
                                            .intra_period_length >= 0) {
                                        pic_index =
                                            (picture_control_set_ptr
                                                 ->picture_number == 0)
                                                ? 0
                                ...
                                Av1GenerateRpsInfo(picture_control_set_ptr,
                                                   encode_context_ptr,
                                                   context_ptr,
                                                   pic_index,
                                                   mini_gop_index);
                                picture_control_set_ptr
                                    ->allow_comp_inter_inter = 0;
                                picture_control_set_ptr->is_skip_mode_allowed =
                                    0;

                                frm_hdr->reference_mode = (ReferenceMode)0xFF;

                                if (picture_control_set_ptr->slice_type !=
                                    I_SLICE) {
                                    picture_control_set_ptr
                                        ->allow_comp_inter_inter = 1;
                                    if (picture_control_set_ptr->slice_type ==
                                        P_SLICE) {
                                        picture_control_set_ptr
                                            ->is_skip_mode_allowed = 0;
                                        frm_hdr->reference_mode =
                                            SINGLE_REFERENCE;
                                        picture_control_set_ptr
                                            ->skip_mode_flag = 0;
                                    } else if (picture_control_set_ptr
                                        ...
                                        frm_hdr->skip_mode_params
                                            .skip_mode_flag = 0;
                                    } else {
                                        frm_hdr->reference_mode =
                                            REFERENCE_MODE_SELECT;
                                        picture_control_set_ptr
                                            ->is_skip_mode_allowed = 1;
                                        picture_control_set_ptr
                                            ->skip_mode_flag = 1;
                                    }
                                }

                                picture_control_set_ptr->av1_cm->mi_cols =
                                    picture_control_set_ptr
                                        ->sequence_control_set_ptr->seq_header
                                        .max_frame_width >>
                                    MI_SIZE_LOG2;
                                picture_control_set_ptr->av1_cm->mi_rows =
                                    picture_control_set_ptr
                                        ->sequence_control_set_ptr->seq_header
                                        .max_frame_height >>
                                    MI_SIZE_LOG2;

                                // Jing: For low delay B/P case, don't alter the
                                // bias
                                memset(picture_control_set_ptr->av1_cm
                                           ->ref_frame_sign_bias,
                                       0,
                                       8 * sizeof(int32_t));
                                if (frm_hdr->reference_mode ==
                                        REFERENCE_MODE_SELECT &&
                                    picture_control_set_ptr
                                        ->temporal_layer_index &&
                                    sequence_control_set_ptr->static_config
                                            .pred_structure ==
                                        EB_PRED_RANDOM_ACCESS) {
                                    picture_control_set_ptr->av1_cm
                                ...
                                if (picture_control_set_ptr->slice_type ==
                                    I_SLICE)
                                    context_ptr->last_i_picture_sc_detection =
                                        picture_control_set_ptr
                                            ->sc_content_detected;
                                else
                                    picture_control_set_ptr
                                        ->sc_content_detected =
                                        context_ptr
                                            ->last_i_picture_sc_detection;

                                // TODO: put this in EbMotionEstimationProcess?
                                // ME Kernel Multi-Processes Signal(s)
                                // derivation
                                ...
                                picture_control_set_ptr
                                    ->limit_ois_to_dc_mode_flag = EB_FALSE;

                                // Update the Dependant List Count - If there
                                // was an I-frame or Scene Change, then cleanup
                                // the Picture Decision PA Reference Queue
                                // Dependent Counts
                                if (picture_control_set_ptr->slice_type ==
                                    I_SLICE) {
                                    inputQueueIndex =
                                        encode_context_ptr
                                            ->picture_decision_pa_reference_queue_head_index;
                                    ...
                                    while (
                                        ...
                                        // Modify Dependent List0
                                        depListCount =
                                            inputEntryPtr->list0.list_count;
                                        for (depIdx = 0; depIdx < depListCount;
                                             ++depIdx) {
                                            // Adjust the latest
                                            // current_input_poc in case we're
                                            // in a POC rollover scenario
                                            // current_input_poc +=
                                            // (current_input_poc <
                                            // inputEntryPtr->pocNumber) ? (1 <<
                                            ...
                                            // If Dependent POC is greater or
                                            // equal to the IDR POC
                                            if (depPoc >=
                                                    picture_control_set_ptr
                                                        ->picture_number &&
                                                inputEntryPtr->list0
                                        ...
                                        // Modify Dependent List1
                                        depListCount =
                                            inputEntryPtr->list1.list_count;
                                        for (depIdx = 0; depIdx < depListCount;
                                             ++depIdx) {
                                            // Adjust the latest
                                            // current_input_poc in case we're
                                ...
                                } else if (picture_control_set_ptr->idr_flag ==
                                           EB_TRUE) {
                                    // Set the Picture Decision PA Reference
                                    // Entry pointer
                                    inputEntryPtr =
                                        (PaReferenceQueueEntry *)EB_NULL;
                                }

                                // Place Picture in Picture Decision PA
                                // Reference Queue there is no need to add the
                                // overlay frames in the PA Reference Queue
                                if (!picture_control_set_ptr->is_overlay) {
                                    inputEntryPtr =
                                        encode_context_ptr
                                            ->picture_decision_pa_reference_queue
                                    ...
                                    inputEntryPtr->picture_number =
                                        picture_control_set_ptr->picture_number;
                                    inputEntryPtr->reference_entry_index =
                                        encode_context_ptr
                                            ->picture_decision_pa_reference_queue_tail_index;
                                    inputEntryPtr->is_alt_ref =
                                        picture_control_set_ptr->is_alt_ref;
                                    encode_context_ptr
                                        ->picture_decision_pa_reference_queue_tail_index =
                                        (encode_context_ptr
                                    ...
                                    // Check if the Picture Decision PA
                                    // Reference is full
                                    CHECK_REPORT_ERROR(
                                        (((encode_context_ptr
                                               ->picture_decision_pa_reference_queue_head_index !=
                                ...
                                // Copy the reference lists into the inputEntry
                                // and set the Reference Counts Based on
                                // Temporal Layer and how many frames are active
                                picture_control_set_ptr->ref_list0_count =
                                    (picture_type == I_SLICE)
                                        ...
                                        : (picture_control_set_ptr->is_overlay)
                                              ? 1
                                              : (uint8_t)
                                                    predPositionPtr->ref_list0
                                                        .reference_list_count;
                                picture_control_set_ptr->ref_list1_count =
                                    (picture_type == I_SLICE ||
                                     picture_control_set_ptr->is_overlay)
                                        ? 0
                                        : (uint8_t)predPositionPtr->ref_list1
                                              .reference_list_count;
                                if (!picture_control_set_ptr->is_overlay) {
                                    inputEntryPtr->list0_ptr =
                                        &predPositionPtr->ref_list0;
                                    inputEntryPtr->list1_ptr =
                                        &predPositionPtr->ref_list1;
                                }
                                if (!picture_control_set_ptr->is_overlay) {
                                    // Copy the Dependent Lists
                                    // *Note - we are removing any leading
                                    // picture dependencies for now
                                    inputEntryPtr->list0.list_count = 0;
                                    for (depIdx = 0;
                                         depIdx <
                                         predPositionPtr->dep_list0.list_count;
                                         ++depIdx) {
                                        if (predPositionPtr->dep_list0
                                                .list[depIdx] >= 0)
                                            inputEntryPtr->list0
                                                .list[inputEntryPtr->list0
                                                          .list_count++] =
                                                predPositionPtr->dep_list0
                                                    .list[depIdx];
                                    }
                                    inputEntryPtr->list1.list_count =
                                        predPositionPtr->dep_list1.list_count;
                                    for (depIdx = 0;
                                         depIdx <
                                         predPositionPtr->dep_list1.list_count;
                                         ++depIdx)
                                        inputEntryPtr->list1.list[depIdx] =
                                            predPositionPtr->dep_list1
                                                .list[depIdx];
                                    // add the overlay picture to the dependant
                                    // list
                                ...
                                EB_MEMSET(
                                    picture_control_set_ptr
                                        ->ref_pa_pic_ptr_array[REF_LIST_0],
                                    0,
                                    REF_LIST_MAX_DEPTH *
                                        sizeof(EbObjectWrapper *));
                                EB_MEMSET(
                                    picture_control_set_ptr
                                        ->ref_pa_pic_ptr_array[REF_LIST_1],
                                    0,
                                    REF_LIST_MAX_DEPTH *
                                        sizeof(EbObjectWrapper *));
                                ...
                                EB_MEMSET(
                                    picture_control_set_ptr
                                        ->ref_pic_poc_array[REF_LIST_0],
                                    0,
                                    REF_LIST_MAX_DEPTH * sizeof(uint64_t));
                                EB_MEMSET(
                                    picture_control_set_ptr
                                        ->ref_pic_poc_array[REF_LIST_1],
                                    0,
                                    REF_LIST_MAX_DEPTH * sizeof(uint64_t));
                            }
                            picture_control_set_ptr =
                            ...
                            if ((sequence_control_set_ptr->enable_altrefs ==
                                     EB_TRUE &&
                                 sequence_control_set_ptr->static_config
                                         .pred_structure ==
                                     EB_PRED_RANDOM_ACCESS &&
                                ...
                                int altref_nframes =
                                    picture_control_set_ptr
                                        ->sequence_control_set_ptr
                                        ->static_config.altref_nframes;
                                if (picture_control_set_ptr->idr_flag) {
                                    // initilize list
                                    for (int pic_itr = 0;
                                         pic_itr < ALTREF_MAX_NFRAMES;
                                         pic_itr++)
                                        picture_control_set_ptr
                                            ->temp_filt_pcs_list[pic_itr] =
                                            NULL;

                                    picture_control_set_ptr
                                        ->temp_filt_pcs_list[0] =
                                        picture_control_set_ptr;

                                    uint32_t num_future_pics = 6;
                                    uint32_t num_past_pics = 0;
                                    uint32_t pic_i;
                                    // search reord-queue to get the future
                                    // pictures
                                    for (pic_i = 0; pic_i < num_future_pics;
                                         pic_i++) {
                                        int32_t q_index = QUEUE_GET_NEXT_SPOT(
                                            picture_control_set_ptr
                                                ->pic_decision_reorder_queue_idx,
                                            pic_i + 1);
                                        if (encode_context_ptr
                                                ->picture_decision_reorder_queue
                                                    [q_index]
                                    ...
                                    picture_control_set_ptr
                                        ->future_altref_nframes = pic_i;
                                    int index_center = 0;
                                    uint32_t actual_future_pics =
                                        picture_control_set_ptr
                                            ->future_altref_nframes;
                                    int pic_itr, ahd;
                                    ...
                                    // Accumulative histogram absolute
                                    // differences between the central and
                                    // future frame
                                    for (pic_itr = (index_center +
                                                    actual_future_pics);
                                         pic_itr > index_center;
                                         pic_itr--) {
                                        ahd =
                                            compute_luma_sad_between_center_and_target_frame(
                                                index_center,
                                                pic_itr,
                                                picture_control_set_ptr,
                                                sequence_control_set_ptr);
                                    ...
                                    // %d\n",
                                    // picture_control_set_ptr->picture_number,
                                    // picture_control_set_ptr->past_altref_nframes,
                                    // picture_control_set_ptr->future_altref_nframes);
                                } else {
                                    int num_past_pics = altref_nframes / 2;
                                    int num_future_pics =
                                        altref_nframes - num_past_pics - 1;
                                    assert(altref_nframes <=
                                           ALTREF_MAX_NFRAMES);
                                    ...
                                    num_past_pics = MIN(
                                        num_past_pics,
                                        (int)encode_context_ptr
                                                ->pre_assignment_buffer_count -
                                            1);

                                    // get previous+current pictures from the
                                    // the pre-assign buffer
                                    for (int pic_itr = 0;
                                         pic_itr <= num_past_pics;
                                         pic_itr++) {
                                        PictureParentControlSet *pcs_itr =
                                            (PictureParentControlSet
                                                 *)encode_context_ptr
                                    ...
                                    if (pic_i < num_future_pics) {
                                        ...
                                        for (int pic_i_future = 0;
                                             pic_i_future < num_future_pics;
                                             pic_i_future++) {
                                            for (
                                                uint32_t pic_i_pa = 0;
                                                pic_i_pa <
                                    ...
                                    int pic_i;
                                    // search reorder-queue to get the future
                                    // pictures
                                    for (pic_i = num_past_pics + 1;
                                         pic_i < ALTREF_MAX_NFRAMES;
                                         pic_i++) {
                                        int32_t q_index = QUEUE_GET_NEXT_SPOT(
                                            picture_control_set_ptr
                                                ->pic_decision_reorder_queue_idx,
                                    ...
                                    // get actual number of future pictures
                                    // stored
                                    for (pic_i = num_past_pics + 1;
                                         pic_i < ALTREF_MAX_NFRAMES;
                                         pic_i++) {
                                    ...
                                    actual_past_pics +=
                                        (altref_nframes + 1) & 0x1;
#endif
#if NON_KF_INTRA_TF_FIX
                                    int index_center = actual_past_pics;
#else
                                    int index_center = (uint8_t)(
                                        picture_control_set_ptr
                                            ->sequence_control_set_ptr
                                            ->static_config.altref_nframes /
                                        2);
                                    ...
                                    // Accumulative histogram absolute
                                    // differences between the central and past
                                    // frame
#if FIX_ALTREF
                                    for (pic_itr =
                                             index_center - actual_past_pics;
                                         pic_itr < index_center;
                                         pic_itr++) {
#else
                                    for (pic_itr =
                                             index_center - actual_past_pics;
                                         pic_itr < index_center - 1;
                                         pic_itr++) {
#endif
                                        ahd =
                                            compute_luma_sad_between_center_and_target_frame(
                                    ...
                                    // adjust the temporal filtering pcs buffer
                                    // to remove unused past pictures
                                ...
                                picture_control_set_ptr->temp_filt_prep_done =
                                    0;
                                ...
                                // Start Filtering in ME processes
                                {
                                    ...
                                    // Initialize Segments
                                    picture_control_set_ptr
                                        ->tf_segments_column_count =
                                        sequence_control_set_ptr
                                            ->tf_segment_column_count;
                                    picture_control_set_ptr
                                        ->tf_segments_row_count =
                                        sequence_control_set_ptr
                                            ->tf_segment_row_count;
                                    picture_control_set_ptr
                                        ->tf_segments_total_count = (uint16_t)(
                                        picture_control_set_ptr
                                            ->tf_segments_column_count *
                                        picture_control_set_ptr
                                            ->tf_segments_row_count);

                                    picture_control_set_ptr->temp_filt_seg_acc =
                                        0;
#if TWO_PASS
                                    if (picture_control_set_ptr
                                            ->temporal_layer_index == 0)
                                        picture_control_set_ptr
                                            ->altref_strength =
                                            sequence_control_set_ptr
                                                ->static_config.altref_strength;
                                    else
                                        picture_control_set_ptr
                                            ->altref_strength = 2;
#else
                                    picture_control_set_ptr->altref_strength =
                                        sequence_control_set_ptr->static_config
                                            .altref_strength;
#endif
                                    ...
                                    for (seg_idx = 0;
                                        ...
                                        outputResultsPtr
                                            ...
                                            encode_context_ptr
                                                ->pre_assignment_buffer
                                                    [pictureIndex];
                                        outputResultsPtr->segment_index =
                                            seg_idx;
                                    ...
                                    eb_block_on_semaphore(
                                        picture_control_set_ptr
                                            ->temp_filt_done_semaphore);
                                }

                            } else
                                picture_control_set_ptr->temporal_filtering_on =
                                    EB_FALSE;  // set temporal filtering flag
                                               // OFF for current picture
                        }

                        // Add 1 to the loop for the overlay picture. If the
                        // last picture is alt ref, increase the loop by 1 to
                        // add the overlay picture
                        uint32_t has_overlay =
                            ((PictureParentControlSet *)encode_context_ptr
                                 ->pre_assignment_buffer
                                     [context_ptr
                                          ->mini_gop_end_index[mini_gop_index]]
                                 ->object_ptr)
                                    ->is_alt_ref
                                ? 1
                                : 0;
                        for (pictureIndex =
                             ...
                             context_ptr->mini_gop_end_index[mini_gop_index] +
                                 has_overlay;
                             ++pictureIndex) {
                            // 2nd Loop over Pictures in the Pre-Assignment
                            // Buffer Assign the overlay pcs. Since Overlay
                            // picture is not added to the
                            // picture_decision_pa_reference_queue, in the next
                            // stage, the loop finds the alt_ref picture. The
                            // reference for overlay frame is hardcoded later
                            if (has_overlay &&
                                pictureIndex == context_ptr->mini_gop_end_index
                                                        [mini_gop_index] +
                                                    has_overlay) {
                                picture_control_set_ptr =
                                    ((PictureParentControlSet
                            ...
                            picture_control_set_ptr->picture_number_alt =
                                encode_context_ptr->picture_number_alt++;
                            ...
                            if ((context_ptr
                                ...
                                picture_control_set_ptr->decode_order =
                                    encode_context_ptr->decode_base_number +
                                    picture_control_set_ptr->pred_struct_ptr
                                        ->pred_struct_entry_ptr_array
                                            [picture_control_set_ptr
                                                 ->pred_struct_index]
                            ...
                            encode_context_ptr->terminating_picture_number =
                                (picture_control_set_ptr
                                    ...
                                    ? picture_control_set_ptr
                                          ->picture_number_alt
                                    : encode_context_ptr
                                          ->terminating_picture_number;

                            // Find the Reference in the Picture Decision PA
                            // Reference Queue
                            inputQueueIndex =
                                encode_context_ptr
                                    ->picture_decision_pa_reference_queue_head_index;
                            ...
                            do {
                                // Setup the Picture Decision PA Reference Queue
                                // Entry
                                inputEntryPtr =
                                    encode_context_ptr
                                        ->picture_decision_pa_reference_queue
                            ...
                            EB_MEMSET(
                                ...
                                0,
                                REF_LIST_MAX_DEPTH * sizeof(EbObjectWrapper *));
                            EB_MEMSET(picture_control_set_ptr
                                          ->ref_pic_poc_array[REF_LIST_0],
                                      0,
                                      REF_LIST_MAX_DEPTH * sizeof(uint64_t));
                            EB_MEMSET(picture_control_set_ptr
                            ...
                            // Reset the PA Reference Lists
                            EB_MEMSET(
                                picture_control_set_ptr->ref_pa_pic_ptr_array,
                                0,
                                2 * sizeof(EbObjectWrapper *));

                            EB_MEMSET(
                                picture_control_set_ptr->ref_pic_poc_array,
                                0,
                                2 * sizeof(uint64_t));
                            ...
                            if ((picture_control_set_ptr->slice_type ==
                                ...
                                for (ref_pic_index = 0;
                                    ...
                                    if (picture_control_set_ptr
                                            ->ref_list0_count) {
                                        // hardcode the reference for the
                                        // overlay frame
                                        if (picture_control_set_ptr
                                                ->is_overlay) {
                                            paReferenceQueueIndex =
                                                (uint32_t)CIRCULAR_ADD(
                                                    (int32_t)inputEntryPtr
                                        ...
                                        picture_control_set_ptr
                                            ->ref_pa_pic_ptr_array
                                                [REF_LIST_0][ref_pic_index] =
                                            paReferenceEntryPtr
                                                ->input_object_ptr;
                                        picture_control_set_ptr
                                            ->ref_pic_poc_array[REF_LIST_0]
                                                               [ref_pic_index] =
                                            ref_poc;
                                        // Increment the PA Reference's
                                        // liveCount by the number of tiles in
                                        // the input picture
                            ...
                            eb_av1_setup_skip_mode_allowed(
                                picture_control_set_ptr);

                            picture_control_set_ptr->is_skip_mode_allowed =
                                frm_hdr->skip_mode_params.skip_mode_allowed;
                            picture_control_set_ptr->skip_mode_flag =
                                picture_control_set_ptr->is_skip_mode_allowed;
                            // printf("POC:%i  skip_mode_allowed:%i  REF_SKIP_0:
                            // %i   REF_SKIP_1: %i
                            // \n",picture_control_set_ptr->picture_number,
                            // picture_control_set_ptr->skip_mode_info.skip_mode_allowed,
                            // picture_control_set_ptr->skip_mode_info.ref_frame_idx_0,
                            // picture_control_set_ptr->skip_mode_info.ref_frame_idx_1);

                            {
                                picture_control_set_ptr
                                    ->intensity_transition_flag = EB_FALSE;
                                if (picture_control_set_ptr->ref_list0_count)
                                    picture_control_set_ptr
                                        ->scene_transition_flag[REF_LIST_0] =
                                        EB_FALSE;
                                if (picture_control_set_ptr->ref_list1_count)
                                    picture_control_set_ptr
                                        ->scene_transition_flag[REF_LIST_1] =
                                        EB_FALSE;
                            }

                            // set the ref frame types used for this picture,
                            set_all_ref_frame_type(
                                sequence_control_set_ptr,
                                picture_control_set_ptr,
                                picture_control_set_ptr->ref_frame_type_arr,
                                &picture_control_set_ptr->tot_ref_frame_types);
                            // Initialize Segments
                            picture_control_set_ptr->me_segments_column_count =
                                (uint8_t)(sequence_control_set_ptr
                                              ->me_segment_column_count_array
                                                  [picture_control_set_ptr
                                                       ->temporal_layer_index]);
                            picture_control_set_ptr->me_segments_row_count =
                                (uint8_t)(sequence_control_set_ptr
                                              ->me_segment_row_count_array
                                                  [picture_control_set_ptr
                                                       ->temporal_layer_index]);
                            picture_control_set_ptr->me_segments_total_count =
                                (uint16_t)(picture_control_set_ptr
                                               ->me_segments_column_count *
                                           picture_control_set_ptr
                                               ->me_segments_row_count);
                            picture_control_set_ptr
                                ->me_segments_completion_mask = 0;
                            ...
                            // Post the results to the ME processes
                            {
                                ...
                                for (segment_index = 0;
                                    ...
                                    outputResultsPtr =
                                        (PictureDecisionResults *)
                                            outputResultsWrapperPtr->object_ptr;
                                    if (picture_control_set_ptr->is_overlay)
                                        outputResultsPtr
                                            ->picture_control_set_wrapper_ptr =
                                            picture_control_set_ptr
                            ...
                            if (pictureIndex ==
                                ...
                                encode_context_ptr
                                    ->pre_assignment_buffer_count = 0;
                                encode_context_ptr
                                    ->pre_assignment_buffer_idr_count = 0;
                                encode_context_ptr
                                    ->pre_assignment_buffer_intra_count = 0;
                                encode_context_ptr
                                    ->pre_assignment_buffer_scene_change_count =
                                    0;
                                encode_context_ptr
                                    ->pre_assignment_buffer_eos_flag = EB_FALSE;
                            }
                        }
                    }  // End MINI GOPs loop
                }

                // Walk the picture_decision_pa_reference_queue and remove
                // entries that have been completely referenced.
                inputQueueIndex =
                    encode_context_ptr
                        ->picture_decision_pa_reference_queue_head_index;

                while (inputQueueIndex !=
                       encode_context_ptr
                           ->picture_decision_pa_reference_queue_tail_index) {
                    inputEntryPtr =
                        encode_context_ptr->picture_decision_pa_reference_queue
                            [inputQueueIndex];
                ...
                // Get the next entry from the Picture Decision Reordering Queue
                // (Entry N+1)
                queueEntryPtr =
                    encode_context_ptr->picture_decision_reorder_queue
                        [encode_context_ptr
