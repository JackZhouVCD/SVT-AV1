...
#define SEGMENT_ROW_COMPLETION_TEST(mask, row_index, width)             \
    ((((mask) >> ((row_index) * (width))) & ((1ull << (width)) - 1)) == \
     ((1ull << (width)) - 1))
#define SEGMENT_CONVERT_IDX_TO_XY(index, x, y, pic_width_in_lcu) \
    MULTI_LINE_MACRO_BEGIN(y) = (index) / (pic_width_in_lcu);    \
    (x) = (index) - (y) * (pic_width_in_lcu);                    \
    MULTI_LINE_MACRO_END
#define SEGMENT_START_IDX(index, pic_size_in_lcu, num_of_seg) \
    (((index) * (pic_size_in_lcu)) / (num_of_seg))
#define SEGMENT_END_IDX(index, pic_size_in_lcu, num_of_seg) \
    ((((index) + 1) * (pic_size_in_lcu)) / (num_of_seg))
...
typedef struct PictureControlSet {
    ...
    // Reference Lists
    EbObjectWrapper
        *ref_pic_ptr_array[MAX_NUM_OF_REF_PIC_LIST][REF_LIST_MAX_DEPTH];
...
#if TWO_PASS
    uint64_t ref_pic_referenced_area_avg_array[MAX_NUM_OF_REF_PIC_LIST]
                                              [REF_LIST_MAX_DEPTH];
#endif
    // GOP
    ...
    NeighborArrayUnit
        *md_intra_luma_mode_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_intra_chroma_mode_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    ...
    NeighborArrayUnit
        *md_tx_depth_1_luma_recon_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    ...
    NeighborArrayUnit
        *md_luma_recon_neighbor_array16bit[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit *md_tx_depth_1_luma_recon_neighbor_array16bit
        [NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_cb_recon_neighbor_array16bit[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_cr_recon_neighbor_array16bit[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit *md_skip_coeff_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_luma_dc_sign_level_coeff_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit *md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array
        [NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_cb_dc_sign_level_coeff_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_cr_dc_sign_level_coeff_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit *md_txfm_context_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_inter_pred_dir_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    NeighborArrayUnit
        *md_ref_frame_type_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];

    NeighborArrayUnit32
        *md_interpolation_type_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];

    NeighborArrayUnit
        *mdleaf_partition_neighbor_array[NEIGHBOR_ARRAY_TOTAL_COUNT];
    ...
    NeighborArrayUnit
        *luma_dc_sign_level_coeff_neighbor_array;  // Stored per 4x4. 8 bit:
                                                   // lower 6 bits
                                                   // (COEFF_CONTEXT_BITS),
                                                   // shows if there is at least
                                                   // one Coef. Top 2 bit store
                                                   // the sign of DC as follow:
                                                   // 0->0,1->-1,2-> 1
    NeighborArrayUnit
        *cr_dc_sign_level_coeff_neighbor_array;  // Stored per 4x4. 8 bit: lower
                                                 // 6 bits(COEFF_CONTEXT_BITS),
                                                 // shows if there is at least
                                                 // one Coef. Top 2 bit store
                                                 // the sign of DC as follow:
                                                 // 0->0,1->-1,2-> 1
    NeighborArrayUnit
        *cb_dc_sign_level_coeff_neighbor_array;  // Stored per 4x4. 8 bit: lower
                                                 // 6 bits(COEFF_CONTEXT_BITS),
                                                 // shows if there is at least
                                                 // one Coef. Top 2 bit store
                                                 // the sign of DC as follow:
...
// CHKN
// Add the concept of PictureParentControlSet which is a subset of the old
// PictureControlSet. It actually holds only high level Picture based control
// data:(GOP management,when to start a picture, when to release the PCS, ....).
// The regular PictureControlSet(Child) will be dedicated to store SB based
// encoding results and information. Parent is created before the Child, and
// continue to live more. Child PCS only lives the exact time needed to encode
// the picture: from ME to EC/ALF.
typedef struct PictureParentControlSet {
    ...
    EbPictureBufferDesc
        *chroma_downsampled_picture_ptr;  // if 422/444 input, down sample to
                                          // 420 for MD
    ...
    // Data attached to the picture. This includes data passed from the
    // application, or other data the encoder attaches to the picture.
    EbLinkedListNode *data_ll_head_ptr;
    // pointer to data to be passed back to the application when picture
    // encoding is done
    ...
    // Pre Analysis
    EbObjectWrapper
        *ref_pa_pic_ptr_array[MAX_NUM_OF_REF_PIC_LIST][REF_LIST_MAX_DEPTH];
    ...
    uint16_t
        non_moving_index_average;  // used by ModeDecisionConfigurationProcess()
    ...
    EbBool
        high_dark_area_density_flag;  // computed @ PictureAnalysisProcess() and
                                      // used @ SourceBasedOperationsProcess()
    EbBool
        high_dark_low_light_area_density_flag;  // computed @
                                                // PictureAnalysisProcess() and
                                                // used @
                                                // SourceBasedOperationsProcess()
    ...
    EbWarpedMotionParams global_motion_estimation[MAX_NUM_OF_REF_PIC_LIST]
                                                 [REF_LIST_MAX_DEPTH];
    ...
    Macroblock *av1x;
    int32_t film_grain_params_present;  // todo (AN): Do we need this flag at
                                        // picture level?
    ...
    uint64_t picture_number_alt;  // The picture number overlay includes all the
                                  // overlay frames
    uint8_t is_alt_ref;
    uint8_t is_overlay;
    ...
    uint64_t filtered_sse;  // the normalized SSE between filtered and original
                            // alt_ref with 8 bit precision. I Slice has the
                            // value of the next ALT_REF picture
...
#if TWO_PASS
    stat_struct_t *
        stat_struct_first_pass_ptr;  // pointer to stat_struct in the first pass
...
#endif
    uint32_t
        sb_size_pix;  // since we still have lot of code assuming 64x64 LCU, we
                      // add a new paramter supporting both128x128 and 64x64,
                      // ultimately the fixed code supporting 64x64 should be
                      // upgraded to use 128x128 and the above could be removed.
    uint32_t max_depth;
    // EbBool                             is16bit;
...
typedef struct Av1Comp {
    ...
    //    Yv12BufferConfig *last_source;  // NULL for first frame and alt_ref
    //    frames Yv12BufferConfig *unscaled_source; Yv12BufferConfig
    //    scaled_source; Yv12BufferConfig *unscaled_last_source;
    ...
    //    // references; Plus the currently coded frame itself, it is needed to
    //    allocate
    //    // sufficient space to the size of the maximum possible number of
    //    frames. int32_t interp_filter_selected[REF_FRAMES + 1][SWITCHABLE];
    //
    ...
    //    int32_t initial_mbs;  // Number of MBs in the full-size frame; to be
    //    used to
    //    // normalize the firstpass stats. This will differ from the
    //    // number of MBs in the current frame when the frame is
    //    // scaled.
    //
    //    // When resize is triggered through external control, the desired
    //    width/height
    //    // are stored here until use in the next frame coded. They are
    //    effective only
    //    // for
    ...
    //    //   A mapping of each reference frame from its encoder side value to
    //    the
    //    //   decoder side value obtained following the short signaling
    //    procedure. int32_t ref_conv[REF_FRAMES];
    //
 ...
 **************************************/
extern EbErrorType picture_control_set_creator(EbPtr *object_dbl_ptr,
                                               EbPtr object_init_data_ptr);

extern EbErrorType picture_parent_control_set_creator(
    EbPtr *object_dbl_ptr, EbPtr object_init_data_ptr);

extern EbErrorType me_sb_results_ctor(MeLcuResults *objectPtr,
