...
void eb_av1_cdef_search(EncDecContext *context_ptr,
...
void eb_av1_loop_restoration_save_boundary_lines(const Yv12BufferConfig *frame,
                                                 Av1Common *cm,
                                                 int32_t after_cdef);
void eb_av1_pick_filter_restoration(
...
EbErrorType enc_dec_context_ctor(
    ...
    uint8_t cfg_palette,
#endif
    EbBool is16bit, EbColorFormat color_format,
    uint8_t enable_hbd_mode_decision, uint32_t max_input_luma_width,
    ...
    EB_MALLOC(context_ptr->md_rate_estimation_ptr,
    ...
    // Scratch Coeff Buffer
    {
        ...
        EB_NEW(context_ptr->inverse_quant_buffer,
               eb_picture_buffer_desc_ctor,
               (EbPtr)&init32BitData);
        EB_NEW(context_ptr->transform_buffer,
               eb_picture_buffer_desc_ctor,
               (EbPtr)&init32BitData);
    ...
    if (enable_hbd_mode_decision)
        context_ptr->md_context->input_sample16bit_buffer =
...
static void ResetEncodePassNeighborArrays(
    ...
    EbBool is16bit = (EbBool)(
        picture_control_set_ptr->parent_pcs_ptr->sequence_control_set_ptr
            ->static_config.encoder_bit_depth > EB_8BIT);
    if (is16bit) {
        neighbor_array_unit_reset(
            picture_control_set_ptr->ep_luma_recon_neighbor_array16bit);
        neighbor_array_unit_reset(
            picture_control_set_ptr->ep_cb_recon_neighbor_array16bit);
        neighbor_array_unit_reset(
            picture_control_set_ptr->ep_cr_recon_neighbor_array16bit);
    }
    return;
}
...
static void ResetEncDec(EncDecContext *context_ptr,
    ...
    context_ptr->qp_index =
        picture_control_set_ptr->parent_pcs_ptr->frm_hdr.delta_q_params
                .delta_q_present
            ? (uint8_t)quantizer_to_qindex[context_ptr->qp]
            : (uint8_t)picture_control_set_ptr->parent_pcs_ptr->frm_hdr
    ...
    // Lambda Assignement
    context_ptr->qp_index = (uint8_t)picture_control_set_ptr->parent_pcs_ptr
    ...
    context_ptr->md_rate_estimation_ptr =
        picture_control_set_ptr->md_rate_estimation_array;
    if (segment_index == 0) {
        ResetEncodePassNeighborArrays(picture_control_set_ptr);
        reset_segmentation_map(
            picture_control_set_ptr->segmentation_neighbor_map);
    }
 ...
 ******************************************************/
static void EncDecConfigureLcu(EncDecContext *context_ptr, SuperBlock *sb_ptr,
                               PictureControlSet *picture_control_set_ptr,
                               uint8_t sb_qp) {
...
void ReconOutput(PictureControlSet *picture_control_set_ptr,
    ...
    if (!picture_control_set_ptr->parent_pcs_ptr->is_alt_ref) {
        ...
        // Copy the Reconstructed Picture to the Output Recon Buffer
        {
            ...
            if (sequence_control_set_ptr->seq_header
                ...
                if (picture_control_set_ptr->parent_pcs_ptr
                    ...
                    film_grain_ptr = &picture_control_set_ptr->parent_pcs_ptr
                                          ->frm_hdr.film_grain_params;

                eb_av1_add_film_grain(
...
void psnr_calculations(PictureControlSet *picture_control_set_ptr,
                       SequenceControlSet *sequence_control_set_ptr) {
    EbBool is16bit =
        (sequence_control_set_ptr->static_config.encoder_bit_depth > EB_8BIT);
    ...
    if (!is16bit) {
        ...
        // if current source picture was temporally filtered, use an alternative
        // buffer which stores the original source picture
        if (picture_control_set_ptr->parent_pcs_ptr->temporal_filtering_on ==
            EB_TRUE) {
            buffer_y = picture_control_set_ptr->parent_pcs_ptr
                           ->save_enhanced_picture_ptr[0];
            buffer_cb = picture_control_set_ptr->parent_pcs_ptr
                            ->save_enhanced_picture_ptr[1];
            buffer_cr = picture_control_set_ptr->parent_pcs_ptr
                            ->save_enhanced_picture_ptr[2];
        } else {
        ...
        inputBuffer = &(buffer_y[input_picture_ptr->origin_x +
        ...
        reconCoeffBuffer = &((recon_ptr->buffer_cb)[recon_ptr->origin_x / 2 +
                                                    recon_ptr->origin_y / 2 *
                                                        recon_ptr->stride_cb]);
        inputBuffer = &(buffer_cb[input_picture_ptr->origin_x / 2 +
        ...
        reconCoeffBuffer = &((recon_ptr->buffer_cr)[recon_ptr->origin_x / 2 +
                                                    recon_ptr->origin_y / 2 *
                                                        recon_ptr->stride_cr]);
        inputBuffer = &(buffer_cr[input_picture_ptr->origin_x / 2 +
                                  input_picture_ptr->origin_y / 2 *
        ...
        if (sequence_control_set_ptr->static_config.ten_bit_format == 1) {
            const uint32_t luma_width =
                sequence_control_set_ptr->seq_header.max_frame_width;
            const uint32_t luma_height =
                sequence_control_set_ptr->seq_header.max_frame_height;
            ...
            // if current source picture was temporally filtered, use an
            // alternative buffer which stores the original source picture
            EbByte buffer_y, buffer_bit_inc_y;
            EbByte buffer_cb, buffer_bit_inc_cb;
            EbByte buffer_cr, buffer_bit_inc_cr;

            if (picture_control_set_ptr->parent_pcs_ptr
                    ->temporal_filtering_on == EB_TRUE) {
                buffer_y = picture_control_set_ptr->parent_pcs_ptr
                               ->save_enhanced_picture_ptr[0];
                buffer_bit_inc_y = picture_control_set_ptr->parent_pcs_ptr
                                       ->save_enhanced_picture_bit_inc_ptr[0];
                buffer_cb = picture_control_set_ptr->parent_pcs_ptr
                                ->save_enhanced_picture_ptr[1];
                buffer_bit_inc_cb = picture_control_set_ptr->parent_pcs_ptr
                                        ->save_enhanced_picture_bit_inc_ptr[1];
                buffer_cr = picture_control_set_ptr->parent_pcs_ptr
                                ->save_enhanced_picture_ptr[2];
                buffer_bit_inc_cr = picture_control_set_ptr->parent_pcs_ptr
                                        ->save_enhanced_picture_bit_inc_ptr[2];
            } else {
                buffer_y = input_picture_ptr->buffer_y;
            ...
            inputBuffer = &((buffer_cb)[input_picture_ptr->origin_x / 2 +
                                        input_picture_ptr->origin_y / 2 *
                                            input_picture_ptr->stride_cb]);
            inputBufferBitInc = &(
                (buffer_bit_inc_cb)[input_picture_ptr->origin_x / 2 +
                                    input_picture_ptr->origin_y / 2 *
            ...
            inputBuffer = &((buffer_cr)[input_picture_ptr->origin_x / 2 +
                                        input_picture_ptr->origin_y / 2 *
                                            input_picture_ptr->stride_cr]);
            inputBufferBitInc = &(
                (buffer_bit_inc_cr)[input_picture_ptr->origin_x / 2 +
                                    input_picture_ptr->origin_y / 2 *
...
#if TWO_PASS
    referenceObject->referenced_area_avg =
...
void CopyStatisticsToRefObject(PictureControlSet *picture_control_set_ptr,
    ...
    if (sequence_control_set_ptr->mfmv_enabled) {
        ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
             ->reference_picture_wrapper_ptr->object_ptr)
            ->frame_type =
            picture_control_set_ptr->parent_pcs_ptr->frm_hdr.frame_type;
        ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
             ->reference_picture_wrapper_ptr->object_ptr)
            ->order_hint =
            picture_control_set_ptr->parent_pcs_ptr->cur_order_hint;
        memcpy(((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
                    ->reference_picture_wrapper_ptr->object_ptr)
                   ->ref_order_hint,
               picture_control_set_ptr->parent_pcs_ptr->ref_order_hint,
               7 * sizeof(uint32_t));
    }
}
...
EbErrorType signal_derivation_enc_dec_kernel_oq(
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        // at inter-depth 2 Interpolation search at full loop 3 Chroma blind
        // interpolation search at fast loop 4 Interpolation search at fast loop
#if MULTI_PASS_PD
    if (context_ptr->pd_pass == PD_PASS_0)
...
#endif
        if (sequence_control_set_ptr->static_config.set_chroma_mode ==
            DEFAULT) {
    ...
    if (sequence_control_set_ptr->static_config.enable_global_motion ==
        EB_TRUE) {
#if MULTI_PASS_PD
        if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->global_mv_injection = 0;
#else
    if (sequence_control_set_ptr->static_config.enable_global_warped_motion ==
        EB_TRUE) {
        if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected) {
            if (picture_control_set_ptr->enc_mode <= ENC_M1)
                context_ptr->global_mv_injection = 1;
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
...
#endif
        if (sequence_control_set_ptr->static_config.nx4_4xn_parent_mv_inject ==
            DEFAULT)
#if PRESETS_TUNE
        if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected)
        ...
        context_ptr->nx4_4xn_parent_mv_injection =
            sequence_control_set_ptr->static_config.nx4_4xn_parent_mv_inject;
...
#endif
        if (sequence_control_set_ptr->static_config.bipred_3x3_inject ==
            DEFAULT)
#if PRESETS_TUNE
        if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected)
    ...
    // Level                Settings
    // 0                    Level 0: OFF
    // 1                    Level 1: 7x5 full-pel search + sub-pel refinement
    // off 2                    Level 2: 7x5 full-pel search +  (H + V) sub-pel
    // refinement only = 4 half-pel + 4 quarter-pel = 8 positions +
    // pred_me_distortion to pa_me_distortion deviation on 3 Level 3: 7x5
    // full-pel search +  (H + V + D only ~ the best) sub-pel refinement = up to
    // 6 half-pel + up to 6  quarter-pel = up to 12 positions +
    // pred_me_distortion to pa_me_distortion deviation on 4 Level 4: 7x5
    // full-pel search +  (H + V + D) sub-pel refinement = 8 half-pel + 8
    // quarter-pel = 16 positions + pred_me_distortion to pa_me_distortion
    // deviation on 5                    Level 5: 7x5 full-pel search +  (H + V
    // + D) sub-pel refinement = 8 half-pel + 8 quarter-pel = 16 positions +
    // pred_me_distortion to pa_me_distortion deviation off
    if (picture_control_set_ptr->slice_type != I_SLICE) {
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected)
                if (picture_control_set_ptr->enc_mode <= ENC_M1)
                    ...
                    context_ptr->predictive_me_level =
                        (picture_control_set_ptr->enc_mode <= ENC_M0) ? 2 : 4;
#else
        ...
        // |CLASS_0 |Prediction for Luma & Chroma |T, Q, Q-1, T-1 for Luma Only
        // |T, Q, Q-1, T-1 or Luma & Chroma          | |CLASS_6 | |No RDOQ |RDOQ
        // | |        |                             |No Tx Type Search |Tx Type
        // Search                           | |        | |No Tx Size Search |Tx
        // Size Search                           | |        | | |CFL vs.
        // Independent                      |
        // |________|_____________________________|________________________________|_________________________________________|
        // |CLASS_1 |Prediction for Luma Only     |T, Q, Q-1, T-1 for Luma Only
        // |T, Q, Q-1, T-1 for Luma & Chroma         | |CLASS_2 |No
        // Interpolation Search      |No RDOQ                         |RDOQ |
        // |CLASS_3 |Bilinear Interpolation       |No Tx Type Search |Tx Type
        // Search                           | |CLASS_4 | |No Tx Size Search |Tx
        // Size Search                           | |CLASS_5 | |Interpolation
        // Search            |                                         |
        // |________|_____________________________|________________________________|_________________________________________|
#if MULTI_PASS_PD
    if (context_ptr->pd_pass == PD_PASS_0) {
    ...
    // |CLASS_0 |Prediction for Luma & Chroma |Bypassed                     |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 or Luma & Chroma          |
    // |        |No Interpolation Search      | |RDOQ |RDOQ | |        |Regular
    // Interpolation        |                             |No Tx Search |Tx
    // Search                                | |        | | |No ATB |ATB | | |
    // |                             |                                |CFL vs.
    // Independent                      |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_1 |Prediction for Chroma        |Prediction for Luma & Chroma
    // |Bypassed                        |T, Q, Q-1, T-1 for Luma & Chroma | |
    // |No Interpolation Search      |Interpolation Search         | |Tx Search
    // | |        |Bilinear Interpolation       |                             |
    // |                                         | |        | | | | |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_2 |Prediction for Chroma        |Prediction for Luma & Chroma
    // |Bypassed                        |T, Q, Q-1, T-1 for Luma & Chroma | |
    // |No Interpolation Search      |Interpolation Search         | |Tx Search
    // | |        |Bilinear Interpolation       |                             |
    // |                                         | |        | | | | |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_3 |Prediction for Chroma        |Prediction for Luma & Chroma
    // |Bypassed                        |T, Q, Q-1, T-1 for Luma & Chroma | |
    // |No Interpolation Search      |Interpolation Search         | |Tx Search
    // | |        |Bilinear Interpolation       |                             |
    // |                                         | |        | | | | |
    ...
    // |CLASS_0 |Prediction for Luma & Chroma |Bypassed                     |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 or Luma & Chroma          |
    // |        |No Interpolation Search      |                             |No
    // RDOQ                         |RDOQ                                     |
    // |        |Regular Interpolation        |                             |No
    // Tx Search                    |Tx Search                                |
    // |        |                             |                             |No
    // ATB                          |ATB                                      |
    // |        |                             |                             |
    // |CFL vs. Independent                      |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_1 |Prediction for Chroma        |Prediction for Luma & Chroma |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 for Luma & Chroma         |
    // |        |No Interpolation Search      |Interpolation Search         |No
    // RDOQ                         |Tx Search                                |
    ...
    // |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_2 |Prediction for Chroma        |Prediction for Luma & Chroma |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 for Luma & Chroma         |
    // |        |No Interpolation Search      |Interpolation Search         |No
    // RDOQ                         |Tx Search                                |
    // |        |Bilinear Interpolation       |                             |No
    ...
    // |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_3 |Prediction for Chroma        |Prediction for Luma & Chroma |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 for Luma & Chroma         |
    // |        |No Interpolation Search      |Interpolation Search         |No
    // RDOQ                         |Tx Search                                |
    // |        |Bilinear Interpolation       |                             |No
    ...
    // |CFL vs. Independent                      |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_1 |Prediction for Chroma        |Bypassed                     |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 for Luma & Chroma         |
    // |        |No Interpolation Search      |                             |No
    // RDOQ                         |Tx Search                                |
    // |        |Bilinear Interpolation       |                             |No
    ...
    // |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_2 |Prediction for Chroma        |Bypassed                     |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 for Luma & Chroma         |
    // |        |No Interpolation Search      |                             |No
    // RDOQ                         |Tx Search                                |
    // |        |Bilinear Interpolation       |                             |No
    ...
    // |
    // |________|_____________________________|_____________________________|________________________________|_________________________________________|
    // |CLASS_3 |Prediction for Chroma        |Bypassed                     |T,
    // Q, Q-1, T-1 for Luma         |T, Q, Q-1, T-1 for Luma & Chroma         |
    // |        |No Interpolation Search      |                             |No
    // RDOQ                         |Tx Search                                |
    // |        |Bilinear Interpolation       |                             |No
    ...
    if (picture_control_set_ptr->enc_mode == ENC_M0)
        ...
        context_ptr->md_staging_mode =
            MD_STAGING_MODE_0;  // Default structure = fast loop + full loop =
                                // md_stage_0 + md_stage_3
#endif

#if MULTI_PASS_PD
    // Set md staging count level
    // Level 0              minimum count = 1
    // Level 1              set towards the best possible partitioning (to
    // further optimize) Level 2              HG: breack down or look
    // up-table(s) are required !
    if (context_ptr->pd_pass == PD_PASS_0) {
    ...
    if (context_ptr->pd_pass == PD_PASS_0) {
        ...
        context_ptr->combine_class12 =
            (picture_control_set_ptr->enc_mode <= ENC_M4) ? 0 : 1;
#else
        context_ptr->combine_class12 =
            (picture_control_set_ptr->enc_mode == ENC_M0) ? 0 : 1;
#endif
    else
        context_ptr->combine_class12 =
...
#endif
            if (sequence_control_set_ptr->static_config.edge_skp_angle_intra ==
                DEFAULT) {
...
#endif
        if (sequence_control_set_ptr->static_config.prune_ref_rec_part ==
            DEFAULT)
#if PRESETS_TUNE
        if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected ||
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->md_exit_th =
            (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected) ? 10
                                                                           : 18;
    else
#endif
#if M0_OPT
...
#if ENHANCED_M0_SETTINGS  // lossless change - PD_PASS_2 and PD_PASS_1 should be
                          // completely decoupled: previous merge conflict
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->md_stage_2_cand_prune_th =
            sequence_control_set_ptr->input_resolution <= INPUT_SIZE_1080i_RANGE
                ? 5
                : 3;
    else
#endif
    ...
    if (MR_MODE || picture_control_set_ptr->parent_pcs_ptr->sc_content_detected)
        ...
        context_ptr->md_stage_2_cand_prune_th =
            sequence_control_set_ptr->static_config.md_stage_2_cand_prune_th;
#endif
    else if (picture_control_set_ptr->enc_mode <= ENC_M2)
        context_ptr->md_stage_2_cand_prune_th =
            sequence_control_set_ptr->input_resolution <= INPUT_SIZE_1080i_RANGE
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->md_stage_2_class_prune_th =
            sequence_control_set_ptr->static_config.md_stage_2_class_prune_th;
#endif
    else
#endif
#if M0_OPT
...
#endif
#if !ENHANCED_M0_SETTINGS  // lossless change - enable_auto_max_partition is
                           // properly derived using pd_pass @ the bottom:
    ...
    if (picture_control_set_ptr->enc_mode == ENC_M0 ||
        ...
        context_ptr->enable_auto_max_partition =
            sequence_control_set_ptr->static_config.enable_auto_max_partition;
#endif
#endif
#if MULTI_PASS_PD
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->compound_mode)
            context_ptr->compound_types_to_try =
                picture_control_set_ptr->parent_pcs_ptr->compound_mode == 1
                    ? MD_COMP_DIFF0
                    : MD_COMP_WEDGE;
        else
            context_ptr->compound_types_to_try = MD_COMP_AVG;
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->rdoq_quantize_fp =
            (picture_control_set_ptr->enc_mode <= ENC_M7) ? EB_TRUE : EB_FALSE;
#elif PRESETS_TUNE
        context_ptr->rdoq_quantize_fp =
            (picture_control_set_ptr->enc_mode <= ENC_M2) ? EB_TRUE : EB_FALSE;
#else
        context_ptr->rdoq_quantize_fp =
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->md_enable_inter_intra =
            picture_control_set_ptr->parent_pcs_ptr->enable_inter_intra;
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->md_filter_intra_mode =
            picture_control_set_ptr->pic_filter_intra_mode;
    ...
    if (context_ptr->pd_pass == PD_PASS_0)
        ...
        context_ptr->dc_cand_only_flag =
            (picture_control_set_ptr->slice_type == I_SLICE) ? EB_FALSE
                                                             : EB_TRUE;
    else
...
static void set_parent_to_be_considered(MdcLcuData *resultsPtr,
                                        uint32_t blk_index, int32_t sb_size,
                                        int8_t depth_step) {
    ...
    if (blk_geom->sq_size < ((sb_size == BLOCK_128X128) ? 128 : 64)) {
        ...
        const BlockGeom *parent_blk_geom =
            get_blk_geom_mds(parent_depth_idx_mds);
        uint32_t parent_tot_d1_blocks =
            parent_blk_geom->sq_size == 128
                ? 17
                : parent_blk_geom->sq_size > 8
                      ? 25
                      : parent_blk_geom->sq_size == 8 ? 5 : 1;
        for (block_1d_idx = 0; block_1d_idx < parent_tot_d1_blocks;
             block_1d_idx++) {
            resultsPtr->leaf_data_array[parent_depth_idx_mds + block_1d_idx]
                .consider_block = 1;
        }

        if (depth_step < -1)
            set_parent_to_be_considered(
                resultsPtr, parent_depth_idx_mds, sb_size, depth_step + 1);
    }
}

static void set_child_to_be_considered(MdcLcuData *resultsPtr,
                                       uint32_t blk_index, int32_t sb_size,
                                       int8_t depth_step) {
    uint32_t child_block_idx_1, child_block_idx_2, child_block_idx_3,
        child_block_idx_4;
    ...
    if (blk_geom->sq_size > 4) {
        for (block_1d_idx = 0; block_1d_idx < tot_d1_blocks; block_1d_idx++) {
            resultsPtr->leaf_data_array[blk_index + block_1d_idx]
                .consider_block = 1;
            resultsPtr->leaf_data_array[blk_index + block_1d_idx]
                .refined_split_flag = EB_TRUE;
        }
        ...
        uint32_t child1_tot_d1_blocks =
            child1_blk_geom->sq_size == 128
                ? 17
                : child1_blk_geom->sq_size > 8
                      ? 25
                      : child1_blk_geom->sq_size == 8 ? 5 : 1;

        for (block_1d_idx = 0; block_1d_idx < child1_tot_d1_blocks;
             block_1d_idx++) {
            resultsPtr->leaf_data_array[child_block_idx_1 + block_1d_idx]
                .consider_block = 1;
            resultsPtr->leaf_data_array[child_block_idx_1 + block_1d_idx]
                .refined_split_flag = EB_FALSE;
        }
        if (depth_step > 1)
            set_child_to_be_considered(
                resultsPtr, child_block_idx_1, sb_size, depth_step - 1);
        ...
        uint32_t child2_tot_d1_blocks =
            child2_blk_geom->sq_size == 128
                ? 17
                : child2_blk_geom->sq_size > 8
                      ? 25
                      : child2_blk_geom->sq_size == 8 ? 5 : 1;
        for (block_1d_idx = 0; block_1d_idx < child2_tot_d1_blocks;
             block_1d_idx++) {
            resultsPtr->leaf_data_array[child_block_idx_2 + block_1d_idx]
                .consider_block = 1;
            resultsPtr->leaf_data_array[child_block_idx_2 + block_1d_idx]
                .refined_split_flag = EB_FALSE;
        }
        if (depth_step > 1)
            set_child_to_be_considered(
                resultsPtr, child_block_idx_2, sb_size, depth_step - 1);
        ...
        uint32_t child3_tot_d1_blocks =
            child3_blk_geom->sq_size == 128
                ? 17
                : child3_blk_geom->sq_size > 8
                      ? 25
                      : child3_blk_geom->sq_size == 8 ? 5 : 1;

        for (block_1d_idx = 0; block_1d_idx < child3_tot_d1_blocks;
             block_1d_idx++) {
            resultsPtr->leaf_data_array[child_block_idx_3 + block_1d_idx]
                .consider_block = 1;
            resultsPtr->leaf_data_array[child_block_idx_3 + block_1d_idx]
                .refined_split_flag = EB_FALSE;
        }
        if (depth_step > 1)
            set_child_to_be_considered(
                resultsPtr, child_block_idx_3, sb_size, depth_step - 1);
        ...
        uint32_t child4_tot_d1_blocks =
            child4_blk_geom->sq_size == 128
                ? 17
                : child4_blk_geom->sq_size > 8
                      ? 25
                      : child4_blk_geom->sq_size == 8 ? 5 : 1;
        for (block_1d_idx = 0; block_1d_idx < child4_tot_d1_blocks;
             block_1d_idx++) {
            resultsPtr->leaf_data_array[child_block_idx_4 + block_1d_idx]
                .consider_block = 1;
            resultsPtr->leaf_data_array[child_block_idx_4 + block_1d_idx]
                .refined_split_flag = EB_FALSE;
        }
        if (depth_step > 1)
            set_child_to_be_considered(
                resultsPtr, child_block_idx_4, sb_size, depth_step - 1);
    }
}

static void build_cand_block_array(SequenceControlSet *sequence_control_set_ptr,
                                   PictureControlSet *picture_control_set_ptr,
                                   uint32_t sb_index) {
    ...
    while (blk_index < sequence_control_set_ptr->max_block_cnt) {
        ...
        uint8_t is_blk_allowed = picture_control_set_ptr->slice_type != I_SLICE
                                     ? 1
                                     : (blk_geom->sq_size < 128) ? 1 : 0;
        // init consider block flag
        if (sequence_control_set_ptr->sb_geom[sb_index]
                .block_is_inside_md_scan[blk_index] &&
            is_blk_allowed) {
            ...
            for (d1_block_idx = 0; d1_block_idx < tot_d1_blocks; d1_block_idx++)
                d1_blocks_accumlated +=
                    resultsPtr->leaf_data_array[blk_index + d1_block_idx]
                            .consider_block
                        ? 1
                        : 0;
            ...
            for (uint32_t idx = 0; idx < tot_d1_blocks; ++idx) {
                if (resultsPtr->leaf_data_array[blk_index].consider_block) {
                    resultsPtr->leaf_data_array[resultsPtr->leaf_count]
                        .tot_d1_blocks = d1_blocks_accumlated;
                    resultsPtr->leaf_data_array[resultsPtr->leaf_count]
                        .leaf_index =
                        0;  // valid only for square 85 world. will be removed.
                    resultsPtr->leaf_data_array[resultsPtr->leaf_count]
                        .mds_idx = blk_index;
                    split_flag = resultsPtr->leaf_data_array[blk_index]
                                     .refined_split_flag;
                    resultsPtr->leaf_data_array[resultsPtr->leaf_count++]
        ...
        blk_index += split_flag
                         ? d1_depth_offset[sequence_control_set_ptr->seq_header
...
void derive_start_end_depth(PictureControlSet *picture_control_set_ptr,
                            SuperBlock *sb_ptr, uint32_t sb_size,
                            int8_t *s_depth, int8_t *e_depth,
    ...
    uint64_t mth01 = pd_level_tab
        [!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&
         picture_control_set_ptr->slice_type != I_SLICE][encode_mode][0][0];
    uint64_t mth02 = pd_level_tab
        [!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&
         picture_control_set_ptr->slice_type != I_SLICE][encode_mode][0][1];
    uint64_t mth03 = pd_level_tab
        [!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&
         picture_control_set_ptr->slice_type != I_SLICE][encode_mode][0][2];
    uint64_t pth01 = pd_level_tab
        [!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&
         picture_control_set_ptr->slice_type != I_SLICE][encode_mode][1][0];
    uint64_t pth02 = pd_level_tab
        [!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&
         picture_control_set_ptr->slice_type != I_SLICE][encode_mode][1][1];
    uint64_t pth03 = pd_level_tab
        [!picture_control_set_ptr->parent_pcs_ptr->sc_content_detected &&
         picture_control_set_ptr->slice_type != I_SLICE][encode_mode][1][2];
    ...
    uint64_t dist_001 =
        sb_ptr->depth_cost[depth] == 0
            ...
            : sb_ptr->depth_cost[depthp1] <= sb_ptr->depth_cost[depth]
                  ? 0
                  : (((int64_t)sb_ptr->depth_cost[depthp1] -
                      (int64_t)sb_ptr->depth_cost[depth]) *
                     100) /
    ...
    uint64_t dist_100 =
        sb_ptr->depth_cost[depth] == 0
            ...
            : sb_ptr->depth_cost[depthm1] <= sb_ptr->depth_cost[depth]
                  ? 0
                  : (((int64_t)sb_ptr->depth_cost[depthm1] -
                      (int64_t)sb_ptr->depth_cost[depth]) *
                     100) /
                        sb_ptr->depth_cost[depth];
    ...
    uint64_t dist_002 =
        sb_ptr->depth_cost[depth] == 0
            ...
            : sb_ptr->depth_cost[depthp2] <= sb_ptr->depth_cost[depth]
                  ? 0
                  : (((int64_t)sb_ptr->depth_cost[depthp2] -
                      (int64_t)sb_ptr->depth_cost[depth]) *
                     100) /
                        sb_ptr->depth_cost[depth];
    ...
    uint64_t dist_200 =
        sb_ptr->depth_cost[depth] == 0
            ...
            : sb_ptr->depth_cost[depthm2] <= sb_ptr->depth_cost[depth]
                  ? 0
                  : (((int64_t)sb_ptr->depth_cost[depthm2] -
                      (int64_t)sb_ptr->depth_cost[depth]) *
                     100) /
                        sb_ptr->depth_cost[depth];
    ...
    uint64_t dist_003 =
        sb_ptr->depth_cost[depth] == 0
            ...
            : sb_ptr->depth_cost[depthp3] <= sb_ptr->depth_cost[depth]
                  ? 0
                  : (((int64_t)sb_ptr->depth_cost[depthp3] -
                      (int64_t)sb_ptr->depth_cost[depth]) *
                     100) /
                        sb_ptr->depth_cost[depth];
    ...
    uint64_t dist_300 =
        sb_ptr->depth_cost[depth] == 0
            ...
            : sb_ptr->depth_cost[depthm3] <= sb_ptr->depth_cost[depth]
                  ? 0
                  : (((int64_t)sb_ptr->depth_cost[depthm3] -
                      (int64_t)sb_ptr->depth_cost[depth]) *
                     100) /
                        sb_ptr->depth_cost[depth];
...
static void perform_pred_depth_refinement(
    ...
    // Reset mdc_sb_array data to defaults; it will be updated based on the
    // predicted blocks (stored in md_cu_arr_nsq)
    while (blk_index < sequence_control_set_ptr->max_block_cnt) {
        const BlockGeom *blk_geom = get_blk_geom_mds(blk_index);
        resultsPtr->leaf_data_array[blk_index].consider_block = 0;
        resultsPtr->leaf_data_array[blk_index].split_flag =
            blk_geom->sq_size > 4 ? EB_TRUE : EB_FALSE;
        resultsPtr->leaf_data_array[blk_index].refined_split_flag =
            blk_geom->sq_size > 4 ? EB_TRUE : EB_FALSE;
        blk_index++;
    }
    ...
    while (blk_index < sequence_control_set_ptr->max_block_cnt) {
        ...
        if (sequence_control_set_ptr->sb_geom[sb_index]
            ...
            if (blk_geom->shape == PART_N) {
                if (context_ptr->md_cu_arr_nsq[blk_index].split_flag ==
                    EB_FALSE) {
                    int8_t s_depth = 0;
                    int8_t e_depth = 0;
                    ...
                    if (context_ptr->pd_pass == PD_PASS_0) {
                        ...
                        if (picture_control_set_ptr->parent_pcs_ptr
                            ...
                            zero_coeff_present_flag =
                                context_ptr->md_cu_arr_nsq[blk_index]
                                    .block_has_coeff == 0;
                            ...
                            case BLOCK_128X128:
                                zero_coeff_present_flag =
                                    (context_ptr->md_local_cu_unit[blk_index]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index]
                                             .block_has_coeff == 0) ||  // SQ
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 1]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 1]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 2]
                                             .block_has_coeff == 0) ||  // H
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 3]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 3]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 4]
                                             .block_has_coeff == 0) ||  // V
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 5]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 5]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 6]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 7]
                                             .block_has_coeff == 0) ||
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 8]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 8]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 9]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 10]
                                             .block_has_coeff == 0) ||
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 11]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 11]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 12]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 13]
                                             .block_has_coeff == 0) ||
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 14]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 14]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 15]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 16]
                                             .block_has_coeff == 0);
                                break;
                                ...
                                zero_coeff_present_flag =
                                    ...
                                    (context_ptr
                                     ...
                                     context_ptr->md_cu_arr_nsq[blk_index + 16]
                                             .block_has_coeff == 0) ||
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 17]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 17]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 18]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 19]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 20]
                                             .block_has_coeff == 0) ||
                                    (context_ptr
                                         ->md_local_cu_unit[blk_index + 21]
                                         .avail_blk_flag &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 21]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 22]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 23]
                                             .block_has_coeff == 0 &&
                                     context_ptr->md_cu_arr_nsq[blk_index + 24]
                                             .block_has_coeff == 0);
                                break;
                        ...
                        if (zero_coeff_present_flag) {
                            ...
                            if (context_ptr->md_cu_arr_nsq[blk_index]
                                    .best_d1_blk == blk_index) {
                            s_depth = -1;
                            e_depth = 0;
                        } else {
                            s_depth = 0;
                    ...
                    for (block_1d_idx = 0; block_1d_idx < tot_d1_blocks;
                         block_1d_idx++) {
                        resultsPtr->leaf_data_array[blk_index + block_1d_idx]
                            .consider_block = 1;
                        resultsPtr->leaf_data_array[blk_index + block_1d_idx]
...
void *enc_dec_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        picture_width_in_sb =
            (sequence_control_set_ptr->seq_header.max_frame_width + sb_sz -
             1) >>
            lcuSizeLog2;
        ...
        while (AssignEncDecSegments(segments_ptr,
            ...
            for (y_lcu_index = yLcuStartIndex, lcuSegmentIndex = lcuStartIndex;
                ...
                for (x_lcu_index = xLcuStartIndex;
...
#if MULTI_PASS_PD
                    // Multi-Pass PD Path
                    // For each SB, all blocks are tested in PD0 (4421 blocks if
                    // 128x128 SB, and 1101 blocks if 64x64 SB). Then the PD0
                    // predicted Partitioning Structure is refined by
                    // considering up to three refinements depths away from the
                    // predicted depth, both in the direction of smaller block
                    // sizes and in the direction of larger block sizes (up to
                    // Pred - 3 / Pred + 3 refinement). The selection of the
                    // refinement depth is performed using the cost deviation
                    // between the current depth cost and candidate depth cost.
                    // The generated blocks are used as input candidates to PD1.
                    // The PD1 predicted Partitioning Structure is also refined
                    // (up to Pred - 1 / Pred + 1 refinement) using the square
                    // (SQ) vs. non-square (NSQ) decision(s) inside the
                    // predicted depth and using coefficient information. The
                    // final set of blocks is evaluated in PD2 to output the
                    // final Partitioning Structure

                    if ((picture_control_set_ptr->parent_pcs_ptr
                                 ->pic_depth_mode == PIC_MULTI_PASS_PD_MODE_0 ||
                         picture_control_set_ptr->parent_pcs_ptr
                        ...
                        // [PD_PASS_0] Mode Decision - Reduce the total number
                        // of partitions to be tested in later stages. Input :
                        // mdc_cu_ptr built @ mdc process (up to 4421) Output:
                        // md_cu_arr_nsq reduced set of block(s)

                        // PD0 MD Tool(s) : Best ME candidate only as INTER
                        // candidate(s), DC only as INTRA candidate(s), Chroma
                        // blind, Spatial SSE, no MVP table generation, no fast
                        // rate @ full cost derivation, Md-Stage 0 and Md-Stage
                        // 2 using count=1 (i.e. only best md-stage-0 candidate)
                        ...
                        // Perform Pred_0 depth refinement - Add blocks to be
                        // considered in the next stage(s) of PD based on depth
                        // cost.
                        perform_pred_depth_refinement(sequence_control_set_ptr,
                                                      picture_control_set_ptr,
                                                      context_ptr->md_context,
                                                      sb_index);

                        // Re-build mdc_cu_ptr for the 2nd PD Pass [PD_PASS_1]
                        build_cand_block_array(sequence_control_set_ptr,
                                               picture_control_set_ptr,
                                               sb_index);

                        // Reset neighnor information to current SB @ position
                        // (0,0)
                        copy_neighbour_arrays(
                            picture_control_set_ptr,
                            context_ptr->md_context,
                        ...
                        if (picture_control_set_ptr->parent_pcs_ptr
                            ...
                            // [PD_PASS_1] Mode Decision - Further reduce the
                            // number of partitions to be considered in later PD
                            // stages. This pass uses more accurate info than
                            // PD0 to give a better PD estimate. Input :
                            // mdc_cu_ptr built @ PD0 refinement Output:
                            // md_cu_arr_nsq reduced set of block(s)

                            // PD1 MD Tool(s) : ME and Predictive ME only as
                            // INTER candidate(s) but MRP blind (only reference
                            // index 0 for motion compensation), DC only as
                            // INTRA candidate(s)
                            mode_decision_sb(sequence_control_set_ptr,
                                             picture_control_set_ptr,
                                             mdcPtr,
                            ...
                            // Perform Pred_1 depth refinement - Add blocks to
                            // be considered in the next stage(s) of PD based on
                            // depth cost.
                            ...
                            // Re-build mdc_cu_ptr for the 3rd PD Pass
                            // [PD_PASS_2]
                            build_cand_block_array(sequence_control_set_ptr,
                                                   picture_control_set_ptr,
                                                   sb_index);

                            // Reset neighnor information to current SB @
                            // position (0,0)
                            copy_neighbour_arrays(
                                picture_control_set_ptr,
                                context_ptr->md_context,
                    ...
                    // [PD_PASS_2] Mode Decision - Obtain the final partitioning
                    // decision using more accurate info than previous stages.
                    // Reduce the total number of partitions to 1. Input :
                    // mdc_cu_ptr built @ PD1 refinement Output: md_cu_arr_nsq
                    // reduced set of block(s)
