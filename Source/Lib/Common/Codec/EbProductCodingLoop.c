...
int16_t eb_av1_dc_quant_QTX(int32_t qindex, int32_t delta,
...
const EbPredictionFunc ProductPredictionFunTable[3] = {
...
void mode_decision_update_neighbor_arrays(
    ...
    uint8_t chroma_mode =
        (uint8_t)context_ptr->cu_ptr->prediction_unit_array->intra_chroma_mode;
    uint8_t skip_flag = (uint8_t)context_ptr->cu_ptr->skip_flag;

    context_ptr->mv_unit.pred_direction =
        (uint8_t)(context_ptr->md_cu_arr_nsq[index_mds]
                      .prediction_unit_array[0]
                      .inter_pred_direction_index);
    context_ptr->mv_unit.mv[REF_LIST_0].mv_union =
        context_ptr->md_cu_arr_nsq[index_mds]
            .prediction_unit_array[0]
            .mv[REF_LIST_0]
            .mv_union;
    ...
    uint8_t inter_pred_direction_index =
        (uint8_t)context_ptr->cu_ptr->prediction_unit_array
            ->inter_pred_direction_index;
    uint8_t ref_frame_type =
        (uint8_t)context_ptr->cu_ptr->prediction_unit_array[0].ref_frame_type;
#if MULTI_PASS_PD
...

    {
        ...
        partition.above =
            partition_context_lookup[context_ptr->blk_geom->bsize].above;
        partition.left =
            partition_context_lookup[context_ptr->blk_geom->bsize].left;
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->skip_sub_blks)
            ...
            neighbor_array_unit_mode_write(
                ...
                (uint8_t *)&context_ptr->blk_geom
                    ->bsize,  //(uint8_t*)luma_mode,
                origin_x,
                origin_y,
                bwdith,
                bheight,
        ...
        uint16_t txb_count =
            context_ptr->blk_geom->txb_count[context_ptr->cu_ptr->tx_depth];
        for (uint8_t txb_itr = 0; txb_itr < txb_count; txb_itr++) {
            uint8_t dc_sign_level_coeff =
                (int32_t)context_ptr->cu_ptr->quantized_dc[0][txb_itr];
            ...
            neighbor_array_unit_mode_write(
                ...
                context_ptr->sb_origin_x +
                    context_ptr->blk_geom
                        ->tx_org_x[context_ptr->cu_ptr->tx_depth][txb_itr],
                context_ptr->sb_origin_y +
                    context_ptr->blk_geom
                        ->tx_org_y[context_ptr->cu_ptr->tx_depth][txb_itr],
                context_ptr->blk_geom
                    ->tx_width[context_ptr->cu_ptr->tx_depth][txb_itr],
                context_ptr->blk_geom
                    ->tx_height[context_ptr->cu_ptr->tx_depth][txb_itr],
                NEIGHBOR_ARRAY_UNIT_TOP_AND_LEFT_ONLY_MASK);

            neighbor_array_unit_mode_write(
                picture_control_set_ptr
                    ->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array
    ...
    if (context_ptr->blk_geom->has_uv &&
        ...
        {
            uint8_t dc_sign_level_coeff =
                (int32_t)context_ptr->cu_ptr->quantized_dc[1][0];
            neighbor_array_unit_mode_write(
        ...
        {
            uint8_t dc_sign_level_coeff =
                (int32_t)context_ptr->cu_ptr->quantized_dc[2][0];
            neighbor_array_unit_mode_write(
...
#if ENHANCE_ATB
    uint8_t tx_size = tx_depth_to_tx_size[context_ptr->cu_ptr->tx_depth]
    ...
    if (!context_ptr->hbd_mode_decision) {
        if (intraMdOpenLoop == EB_FALSE) {
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->atb_mode) {
                ...
                update_recon_neighbor_array(
                    picture_control_set_ptr
                        ->md_tx_depth_1_luma_recon_neighbor_array
                            [MD_NEIGHBOR_ARRAY_INDEX],
                    context_ptr->cu_ptr->neigh_top_recon[0],
                    context_ptr->cu_ptr->neigh_left_recon[0],
                    origin_x,
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->atb_mode) {
            ...
            update_recon_neighbor_array16bit(
                picture_control_set_ptr
                    ->md_tx_depth_1_luma_recon_neighbor_array16bit
                        [MD_NEIGHBOR_ARRAY_INDEX],
                context_ptr->cu_ptr->neigh_top_recon_16bit[0],
                context_ptr->cu_ptr->neigh_left_recon_16bit[0],
                origin_x,
...
void copy_neighbour_arrays(PictureControlSet *picture_control_set_ptr,
                           ModeDecisionContext *context_ptr, uint32_t src_idx,
                           uint32_t dst_idx, uint32_t blk_mds,
    ...
    if (!context_ptr->hbd_mode_decision) {
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->atb_mode) {
            ...
            copy_neigh_arr(
                picture_control_set_ptr
                    ->md_tx_depth_1_luma_recon_neighbor_array[src_idx],
                picture_control_set_ptr
                    ->md_tx_depth_1_luma_recon_neighbor_array[dst_idx],
                blk_org_x,
                blk_org_y,
                blk_geom->bwidth,
                blk_geom->bheight,
        ...
        if (picture_control_set_ptr->parent_pcs_ptr->atb_mode) {
            ...
            copy_neigh_arr(
                picture_control_set_ptr
                    ->md_tx_depth_1_luma_recon_neighbor_array16bit[src_idx],
                picture_control_set_ptr
                    ->md_tx_depth_1_luma_recon_neighbor_array16bit[dst_idx],
                blk_org_x,
                blk_org_y,
                blk_geom->bwidth,
                blk_geom->bheight,
        ...
        if (blk_geom->has_uv && context_ptr->chroma_level <= CHROMA_MODE_1) {
            copy_neigh_arr(picture_control_set_ptr
                               ->md_cb_recon_neighbor_array16bit[src_idx],
                           picture_control_set_ptr
                               ->md_cb_recon_neighbor_array16bit[dst_idx],
                           blk_org_x_uv,
                           blk_org_y_uv,
                           bwidth_uv,
                           bheight_uv,
            ...
            copy_neigh_arr(picture_control_set_ptr
                               ->md_cr_recon_neighbor_array16bit[src_idx],
                           picture_control_set_ptr
                               ->md_cr_recon_neighbor_array16bit[dst_idx],
                           blk_org_x_uv,
                           blk_org_y_uv,
                           bwidth_uv,
                           bheight_uv,
    ...
    copy_neigh_arr(picture_control_set_ptr
                       ->md_luma_dc_sign_level_coeff_neighbor_array[src_idx],
                   picture_control_set_ptr
                       ->md_luma_dc_sign_level_coeff_neighbor_array[dst_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
    ...
    copy_neigh_arr(
        picture_control_set_ptr
            ->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array[src_idx],
        picture_control_set_ptr
            ->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array[dst_idx],
        blk_org_x,
        blk_org_y,
        blk_geom->bwidth,
        blk_geom->bheight,
    ...
    if (blk_geom->has_uv && context_ptr->chroma_level <= CHROMA_MODE_1) {
        copy_neigh_arr(picture_control_set_ptr
                           ->md_cb_dc_sign_level_coeff_neighbor_array[src_idx],
                       picture_control_set_ptr
                           ->md_cb_dc_sign_level_coeff_neighbor_array[dst_idx],
                       blk_org_x_uv,
                       blk_org_y_uv,
                       bwidth_uv,
                       bheight_uv,
        ...
        copy_neigh_arr(picture_control_set_ptr
                           ->md_cr_dc_sign_level_coeff_neighbor_array[src_idx],
                       picture_control_set_ptr
                           ->md_cr_dc_sign_level_coeff_neighbor_array[dst_idx],
                       blk_org_x_uv,
                       blk_org_y_uv,
                       bwidth_uv,
                       bheight_uv,
    ...
    copy_neigh_arr(picture_control_set_ptr->md_txfm_context_array[src_idx],
...
void md_update_all_neighbour_arrays(PictureControlSet *picture_control_set_ptr,
                                    ModeDecisionContext *context_ptr,
...
void md_update_all_neighbour_arrays_multiple(
    PictureControlSet *picture_control_set_ptr,
    ModeDecisionContext *context_ptr, uint32_t blk_mds, uint32_t sb_origin_x,
    uint32_t sb_origin_y) {
    context_ptr->blk_geom = get_blk_geom_mds(blk_mds);

    uint32_t blk_it;
    for (blk_it = 0; blk_it < context_ptr->blk_geom->totns; blk_it++) {
        md_update_all_neighbour_arrays(picture_control_set_ptr,
                                       context_ptr,
                                       blk_mds + blk_it,
                                       sb_origin_x,
                                       sb_origin_y);
...
int sq_block_index[TOTAL_SQ_BLOCK_COUNT] = {
    0,    25,   50,   75,   80,   81,   82,   83,   84,   89,   90,   91,
    92,   93,   98,   99,   100,  101,  102,  107,  108,  109,  110,  111,
    136,  141,  142,  143,  144,  145,  150,  151,  152,  153,  154,  159,
    160,  161,  162,  163,  168,  169,  170,  171,  172,  197,  202,  203,
    204,  205,  206,  211,  212,  213,  214,  215,  220,  221,  222,  223,
    224,  229,  230,  231,  232,  233,  258,  263,  264,  265,  266,  267,
    272,  273,  274,  275,  276,  281,  282,  283,  284,  285,  290,  291,
    292,  293,  294,  319,  344,  349,  350,  351,  352,  353,  358,  359,
    360,  361,  362,  367,  368,  369,  370,  371,  376,  377,  378,  379,
    380,  405,  410,  411,  412,  413,  414,  419,  420,  421,  422,  423,
    428,  429,  430,  431,  432,  437,  438,  439,  440,  441,  466,  471,
    472,  473,  474,  475,  480,  481,  482,  483,  484,  489,  490,  491,
    492,  493,  498,  499,  500,  501,  502,  527,  532,  533,  534,  535,
    536,  541,  542,  543,  544,  545,  550,  551,  552,  553,  554,  559,
    560,  561,  562,  563,  588,  613,  618,  619,  620,  621,  622,  627,
    628,  629,  630,  631,  636,  637,  638,  639,  640,  645,  646,  647,
    648,  649,  674,  679,  680,  681,  682,  683,  688,  689,  690,  691,
    692,  697,  698,  699,  700,  701,  706,  707,  708,  709,  710,  735,
    740,  741,  742,  743,  744,  749,  750,  751,  752,  753,  758,  759,
    760,  761,  762,  767,  768,  769,  770,  771,  796,  801,  802,  803,
    804,  805,  810,  811,  812,  813,  814,  819,  820,  821,  822,  823,
    828,  829,  830,  831,  832,  857,  882,  887,  888,  889,  890,  891,
    896,  897,  898,  899,  900,  905,  906,  907,  908,  909,  914,  915,
    916,  917,  918,  943,  948,  949,  950,  951,  952,  957,  958,  959,
    960,  961,  966,  967,  968,  969,  970,  975,  976,  977,  978,  979,
    1004, 1009, 1010, 1011, 1012, 1013, 1018, 1019, 1020, 1021, 1022, 1027,
    1028, 1029, 1030, 1031, 1036, 1037, 1038, 1039, 1040, 1065, 1070, 1071,
    1072, 1073, 1074, 1079, 1080, 1081, 1082, 1083, 1088, 1089, 1090, 1091,
    1092, 1097, 1098, 1099, 1100};
void init_sq_nsq_block(SequenceControlSet *sequence_control_set_ptr,
                       ModeDecisionContext *context_ptr) {
    uint32_t blk_idx = 0;
    do {
...
void init_sq_non4_block(SequenceControlSet *sequence_control_set_ptr,
                        ModeDecisionContext *context_ptr) {
    for (uint32_t blk_idx = 0; blk_idx < TOTAL_SQ_BLOCK_COUNT; blk_idx++) {
        context_ptr->md_cu_arr_nsq[sq_block_index[blk_idx]].part =
            PARTITION_SPLIT;
        context_ptr->md_local_cu_unit[sq_block_index[blk_idx]].tested_cu_flag =
            EB_FALSE;
    }
    for (uint32_t blk_idx = 0;
         blk_idx < sequence_control_set_ptr->max_block_cnt;
         ++blk_idx) {
        context_ptr->md_local_cu_unit[blk_idx].avail_blk_flag = EB_FALSE;
    }
}
 ...
 *********************************/
void picture_addition_kernel(uint8_t *pred_ptr, uint32_t pred_stride,
    ...
    while (row_index < height) {
        ...
        while (columnIndex < width) {
            // recon_ptr[columnIndex] = (uint8_t)CLIP3(0, maxValue,
            // ((int32_t)residual_ptr[columnIndex]) +
            // ((int32_t)pred_ptr[columnIndex]));
            uint16_t rec = (uint16_t)pred_ptr[columnIndex];
            recon_ptr[columnIndex] = (uint8_t)highbd_clip_pixel_add(
                rec, (TranLow)residual_ptr[columnIndex], bd);
...
void picture_addition_kernel16_bit(uint16_t *pred_ptr, uint32_t pred_stride,
    ...
    while (row_index < height) {
        ...
        while (columnIndex < width) {
            ...
            // ((int32_t)pred_ptr[columnIndex]));
            uint16_t rec = (uint16_t)pred_ptr[columnIndex];
            recon_ptr[columnIndex] = highbd_clip_pixel_add(
                rec, (TranLow)residual_ptr[columnIndex], bd);
...
void AV1PerformInverseTransformRecon(
    PictureControlSet *picture_control_set_ptr,
    ModeDecisionContext *context_ptr,
    ModeDecisionCandidateBuffer *candidate_buffer, CodingUnit *cu_ptr,
    ...
    if (picture_control_set_ptr->intra_md_open_loop_flag == EB_FALSE) {
        ...
        do {
            ...
            recLumaOffset = context_ptr->blk_geom->tx_org_x[tx_depth][txb_itr] +
                            context_ptr->blk_geom->tx_org_y[tx_depth][txb_itr] *
                                candidate_buffer->recon_ptr->stride_y;
            recCbOffset =
                ((((context_ptr->blk_geom->tx_org_x[tx_depth][txb_itr] >> 3)
                   << 3) +
                  ((context_ptr->blk_geom->tx_org_y[tx_depth][txb_itr] >> 3)
                   << 3) *
            ...
            if (tx_depth == 0 || txb_itr == 0) {
                if (context_ptr->chroma_level <= CHROMA_MODE_1) {
                    uint32_t chroma_tu_width =
                        tx_size_wide[context_ptr->blk_geom
                                         ->txsize_uv[tx_depth][txb_itr]];
                    uint32_t chroma_tu_height =
                        tx_size_high[context_ptr->blk_geom
                                         ->txsize_uv[tx_depth][txb_itr]];
                    uint32_t cbTuChromaOriginIndex =
                        ((((txb_origin_x >> 3) << 3) +
                          ((txb_origin_y >> 3) << 3) *
                              candidate_buffer->recon_coeff_ptr->stride_cb) >>
                         1);
                    uint32_t crTuChromaOriginIndex =
                        ((((txb_origin_x >> 3) << 3) +
                          ((txb_origin_y >> 3) << 3) *
                    ...
                    if (context_ptr->blk_geom->has_uv && txb_ptr->v_has_coeff)
                        inv_transform_recon_wrapper(
                            ...
                            candidate_buffer->recon_ptr->stride_cr,
                            (int32_t *)
                                candidate_buffer->recon_coeff_ptr->buffer_cr,
                            txb_1d_offset_uv,
                            context_ptr->hbd_mode_decision,
                            context_ptr->blk_geom->txsize_uv[tx_depth][txb_itr],
                            candidate_buffer->candidate_ptr->transform_type_uv,
                    ...
                    if (context_ptr->blk_geom->has_uv)
                        txb_1d_offset_uv +=
                            context_ptr->blk_geom
                                ->tx_width_uv[tx_depth][txb_itr] *
                            context_ptr->blk_geom
                                ->tx_height_uv[tx_depth][txb_itr];
                }
            }
            txb_1d_offset +=
                context_ptr->blk_geom->tx_width[tx_depth][txb_itr] *
...
void fast_loop_core(ModeDecisionCandidateBuffer *candidate_buffer,
                    PictureControlSet *picture_control_set_ptr,
                    ModeDecisionContext *context_ptr,
                    EbPictureBufferDesc *input_picture_ptr,
                    uint32_t input_origin_index, uint32_t input_cb_origin_index,
                    uint32_t input_cr_origin_index, CodingUnit *cu_ptr,
                    uint32_t cu_origin_index, uint32_t cu_chroma_origin_index,
                    EbBool use_ssd) {
    ...
    candidate_buffer->candidate_ptr->interp_filters =
        (context_ptr->md_staging_use_bilinear)
            ...
            : 0;
    ProductPredictionFunTable[candidate_buffer->candidate_ptr->use_intrabc
                                  ? INTER_MODE
                                  : candidate_ptr->type](
        context_ptr->hbd_mode_decision,
        context_ptr,
    ...
    if (use_ssd) {
        ...
        candidate_buffer->candidate_ptr->luma_fast_distortion =
            (uint32_t)(lumaFastDistortion = spatial_full_dist_type_fun(
        ...
        if (!context_ptr->hbd_mode_decision) {
            candidate_buffer->candidate_ptr->luma_fast_distortion =
                (uint32_t)(lumaFastDistortion = nxm_sad_kernel_sub_sampled(
            ...
            candidate_buffer->candidate_ptr->luma_fast_distortion = (uint32_t)(
                lumaFastDistortion = sad_16b_kernel(
...
void set_md_stage_counts(PictureControlSet *picture_control_set_ptr,
                         ModeDecisionContext *context_ptr,
                         uint32_t fastCandidateTotalCount) {
    SequenceControlSet *scs =
        (SequenceControlSet *)(picture_control_set_ptr
                                   ->sequence_control_set_wrapper_ptr
                                   ->object_ptr);
    ...
    if (context_ptr->md_staging_count_level == 0) {
        ...
        uint8_t is_intra =
            (picture_control_set_ptr->slice_type == I_SLICE) ? 1 : 0;
        ...
        context_ptr->md_stage_1_count[CAND_CLASS_6] =
            (picture_control_set_ptr->temporal_layer_index == 0) ? 10 : 5;
#endif
#if PAL_CLASS
        context_ptr->md_stage_1_count[CAND_CLASS_7] = 12;
#endif
        context_ptr->md_stage_1_count[CAND_CLASS_8] =
            (picture_control_set_ptr->temporal_layer_index == 0) ? 5 : 4;
    ...
    if (picture_control_set_ptr->enc_mode >= ENC_M2) {
        ...
        context_ptr->md_stage_2_count[CAND_CLASS_0] =
            (picture_control_set_ptr->slice_type == I_SLICE)
                ...
                : (picture_control_set_ptr->parent_pcs_ptr
                       ->is_used_as_reference_flag)
                      ? ((scs->input_resolution >= INPUT_SIZE_1080i_RANGE) ? 7
                                                                           : 10)
                      : 4;
#if FILTER_INTRA_FLAG
        context_ptr->md_stage_2_count[CAND_CLASS_6] =
            ...
            context_ptr->md_stage_2_count[CAND_CLASS_7] =
                (picture_control_set_ptr->temporal_layer_index == 0)
                    ...
                    : (picture_control_set_ptr->parent_pcs_ptr
                          ...
                          ? 4
                          : 4;
        else if (picture_control_set_ptr->parent_pcs_ptr->palette_mode == 2 ||
                 picture_control_set_ptr->parent_pcs_ptr->palette_mode == 3)
            context_ptr->md_stage_2_count[CAND_CLASS_7] =
                (picture_control_set_ptr->temporal_layer_index == 0)
                    ? 7
                    : (picture_control_set_ptr->parent_pcs_ptr
                          ...
                          ? 2
                          : 2;
        else if (picture_control_set_ptr->parent_pcs_ptr->palette_mode == 4 ||
                 picture_control_set_ptr->parent_pcs_ptr->palette_mode == 5)
            context_ptr->md_stage_2_count[CAND_CLASS_7] =
                (picture_control_set_ptr->temporal_layer_index == 0)
                    ? 4
                    : (picture_control_set_ptr->parent_pcs_ptr
...
if (picture_control_set_ptr->enc_mode >= ENC_M1)
...
#if PRESETS_TUNE
        if (picture_control_set_ptr->enc_mode >= ENC_M3 &&
            picture_control_set_ptr->enc_mode <= ENC_M4) {
#else
    if (picture_control_set_ptr->enc_mode >= ENC_M2 &&
        picture_control_set_ptr->enc_mode <= ENC_M4) {
#endif
    ...
    context_ptr->md_stage_2_count[CAND_CLASS_0] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_0]
            ? context_ptr->md_stage_1_count[CAND_CLASS_0]
            : context_ptr->md_stage_2_count[CAND_CLASS_0];
    context_ptr->md_stage_2_count[CAND_CLASS_1] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_1]
            ? context_ptr->md_stage_1_count[CAND_CLASS_1]
            : context_ptr->md_stage_2_count[CAND_CLASS_1];
    context_ptr->md_stage_2_count[CAND_CLASS_2] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_2]
            ? context_ptr->md_stage_1_count[CAND_CLASS_2]
            : context_ptr->md_stage_2_count[CAND_CLASS_2];
    context_ptr->md_stage_2_count[CAND_CLASS_3] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_3]
            ? context_ptr->md_stage_1_count[CAND_CLASS_3]
            : context_ptr->md_stage_2_count[CAND_CLASS_3];
    context_ptr->md_stage_2_count[CAND_CLASS_4] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_4]
            ? context_ptr->md_stage_1_count[CAND_CLASS_4]
            : context_ptr->md_stage_2_count[CAND_CLASS_4];
    context_ptr->md_stage_2_count[CAND_CLASS_5] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_5]
            ? context_ptr->md_stage_1_count[CAND_CLASS_5]
            : context_ptr->md_stage_2_count[CAND_CLASS_5];
    context_ptr->md_stage_2_count[CAND_CLASS_6] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_6]
            ? context_ptr->md_stage_1_count[CAND_CLASS_6]
            : context_ptr->md_stage_2_count[CAND_CLASS_6];
    context_ptr->md_stage_2_count[CAND_CLASS_8] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_8]
            ? context_ptr->md_stage_1_count[CAND_CLASS_8]
            : context_ptr->md_stage_2_count[CAND_CLASS_8];
    ...
    context_ptr->md_stage_2_count[CAND_CLASS_7] =
        context_ptr->bypass_md_stage_1[CAND_CLASS_7]
...
void set_md_stage_counts(PictureControlSet *picture_control_set_ptr,
    ...
    if (context_ptr->md_staging_mode) {
        ...
        context_ptr->bypass_stage1[CAND_CLASS_3] =
            context_ptr->combine_class12 ? EB_TRUE : EB_FALSE;
#if II_COMP_FLAG
    ...
    if (context_ptr->md_staging_mode) {
        ...
        if (context_ptr->md_staging_mode == MD_STAGING_MODE_2 ||
            ...
            context_ptr->bypass_stage2[CAND_CLASS_3] =
                context_ptr->combine_class12 ? EB_TRUE : EB_FALSE;
        } else {
    ...
    context_ptr->md_stage_1_count[CAND_CLASS_4] =
        (picture_control_set_ptr->slice_type == I_SLICE)
            ...
            : (picture_control_set_ptr->parent_pcs_ptr
                   ->is_used_as_reference_flag)
                  ? 14
                  : 6;  // INTER_PRED_NFL: (INTER_PRED_NFL >> 1);
#endif
#if OBMC_FLAG
    if (picture_control_set_ptr->parent_pcs_ptr->pic_obmc_mode == 1)
        context_ptr->md_stage_1_count[CAND_CLASS_5] = 16;
...
#if PAL_CLASS
    context_ptr->md_stage_2_count[CAND_CLASS_7] =
        14;  // context_ptr->bypass_stage1[CAND_CLASS_7] ?
             // context_ptr->md_stage_1_count[CAND_CLASS_7] : 14;
#endif
    context_ptr->md_stage_2_count[CAND_CLASS_1] =
        (picture_control_set_ptr->slice_type == I_SLICE)
...
#if OBMC_FLAG
    context_ptr->md_stage_2_count[CAND_CLASS_5] =
        (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : 16;
#endif
    context_ptr->md_stage_2_count[CAND_CLASS_8] =
        (picture_control_set_ptr->temporal_layer_index == 0) ? 5 : 4;
    ...
    if (context_ptr->combine_class12) {
        context_ptr->md_stage_3_count[CAND_CLASS_1] =
            context_ptr->md_stage_3_count[CAND_CLASS_1] * 2;
    } else {
    ...
    if (picture_control_set_ptr->enc_mode >= ENC_M2 &&
        ...
        if (context_ptr->md_staging_mode == MD_STAGING_MODE_0 &&
            picture_control_set_ptr->enc_mode <= ENC_M6) {
            context_ptr->md_stage_1_count[CAND_CLASS_0] =
                (picture_control_set_ptr->slice_type == I_SLICE)
                    ? 8
                    : (picture_control_set_ptr->parent_pcs_ptr
    ...
    // setting should be done beyond this point)
    context_ptr->md_stage_2_count[CAND_CLASS_0] =
        context_ptr->bypass_stage1[CAND_CLASS_0]
            ? context_ptr->md_stage_1_count[CAND_CLASS_0]
            : context_ptr->md_stage_2_count[CAND_CLASS_0];
    context_ptr->md_stage_2_count[CAND_CLASS_1] =
        context_ptr->bypass_stage1[CAND_CLASS_1]
            ? context_ptr->md_stage_1_count[CAND_CLASS_1]
            : context_ptr->md_stage_2_count[CAND_CLASS_1];
    context_ptr->md_stage_2_count[CAND_CLASS_2] =
        context_ptr->bypass_stage1[CAND_CLASS_2]
            ? context_ptr->md_stage_1_count[CAND_CLASS_2]
            : context_ptr->md_stage_2_count[CAND_CLASS_2];
    context_ptr->md_stage_2_count[CAND_CLASS_3] =
        context_ptr->bypass_stage1[CAND_CLASS_3]
    ...
    context_ptr->md_stage_3_count[CAND_CLASS_0] =
        context_ptr->bypass_stage2[CAND_CLASS_0]
            ? context_ptr->md_stage_2_count[CAND_CLASS_0]
            : context_ptr->md_stage_3_count[CAND_CLASS_0];
    context_ptr->md_stage_3_count[CAND_CLASS_1] =
        context_ptr->bypass_stage2[CAND_CLASS_1]
            ? context_ptr->md_stage_2_count[CAND_CLASS_1]
            : context_ptr->md_stage_3_count[CAND_CLASS_1];
    context_ptr->md_stage_3_count[CAND_CLASS_2] =
        context_ptr->bypass_stage2[CAND_CLASS_2]
            ? context_ptr->md_stage_2_count[CAND_CLASS_2]
            : context_ptr->md_stage_3_count[CAND_CLASS_2];
    context_ptr->md_stage_3_count[CAND_CLASS_3] =
        context_ptr->bypass_stage2[CAND_CLASS_3]
    ...
    // TODO: use actual number of stages on the setting section and update using
    // the following logic.
    // stage2_cand_count[CAND_CLASS_i] = bypass_stage2 ?
    // stage3_cand_count[CAND_CLASS_i] : stage2_cand_count[CAND_CLASS_i];
    // stage1_cand_count[CAND_CLASS_i] = bypass_stage1 ?
    // stage2_cand_count[CAND_CLASS_i] : stage1_cand_count[CAND_CLASS_i];

#if PAL_CLASS  // THIS SHOULD BE rEMOVED AFTER REBAS~~~
    context_ptr->md_stage_2_count[CAND_CLASS_7] =
        context_ptr->bypass_stage1[CAND_CLASS_7]
            ? context_ptr->md_stage_1_count[CAND_CLASS_7]
            : context_ptr->md_stage_2_count[CAND_CLASS_7];
    context_ptr->md_stage_3_count[CAND_CLASS_7] =
        context_ptr->bypass_stage2[CAND_CLASS_7]
            ? context_ptr->md_stage_2_count[CAND_CLASS_7]
            : context_ptr->md_stage_3_count[CAND_CLASS_7];
#endif
...
void sort_stage0_fast_candidates(
    ...
    uint32_t input_buffer_count,  // how many cand buffers to sort. one of the
                                  // buffer can have max cost.
    uint32_t *cand_buff_indices) {
    ModeDecisionCandidateBuffer **buffer_ptr_array =
        context_ptr->candidate_buffer_ptr_array;
#if !SPEED_OPT
    //  fill cand_buff_indices with surviving buffer indices ; move the scratch
    //  candidates (MAX_CU_COST) to the last spots (if any)
    uint32_t ordered_start_idx = 0;
    uint32_t ordered_end_idx = input_buffer_count - 1;
#endif

    uint32_t input_buffer_end_idx =
        input_buffer_start_idx + input_buffer_count - 1;
#if SPEED_OPT
    uint32_t buffer_index, i, j;
    uint32_t k = 0;
    for (buffer_index = input_buffer_start_idx;
         buffer_index <= input_buffer_end_idx;
         buffer_index++, k++) {
...
#else
    for (uint32_t buffer_index = input_buffer_start_idx;
         buffer_index <= input_buffer_end_idx;
         buffer_index++) {
...
static INLINE void heap_sort_stage_max_node_fast_cost_ptr(
    ModeDecisionCandidateBuffer **buffer_ptr, uint32_t *sort_index, uint32_t i,
    uint32_t num) {
...
static void qsort_stage_max_node_fast_cost_ptr(
    ModeDecisionCandidateBuffer **buffer_ptr_array, uint32_t *dst, uint32_t *a,
    uint32_t *b, int num) {
    if (num < 4) {
        if (num < 2) {
            if (num) {
        ...
        /* bacuse a and dst can point on this same array, copy temporary
         * values*/
        uint32_t tmp_a = a[0];
        uint32_t tmp_b = a[1];
        if (*(buffer_ptr_array[tmp_a]->fast_cost_ptr) <
            *(buffer_ptr_array[tmp_b]->fast_cost_ptr)) {
            dst[0] = tmp_a;
            dst[1] = tmp_b;
        } else {
            dst[0] = tmp_b;
    ...
    qsort_stage_max_node_fast_cost_ptr(
        buffer_ptr_array, dst, b, a, sorted_down);
...
static INLINE void sort_array_index_fast_cost_ptr(
    ModeDecisionCandidateBuffer **buffer_ptr, uint32_t *sort_index,
    uint32_t num) {
    if (num <= 60) {
        ...
        qsort_stage_max_node_fast_cost_ptr(
            buffer_ptr, sort_index, sort_index, sorted_tmp, num);
        return;
    }

    // For big arrays uses 'heap sort', not need allocate memory
    // For small array less that 40 elements heap sort work slower than
    // 'insertion sort'
    uint32_t i;
    for (i = (num - 1) / 2; i > 0; i--) {
...
void sort_stage1_fast_candidates(struct ModeDecisionContext *context_ptr,
...
void sort_stage1_fast_candidates(struct ModeDecisionContext *context_ptr,
    ...
    // sorted best:
    // *(buffer_ptr_array[sorted_candidate_index_array[?]]->fast_cost_ptr)
    sort_array_index_fast_cost_ptr(
        buffer_ptr_array, cand_buff_indices, num_of_cand_to_sort);
}
#endif
#if REMOVE_MD_STAGE_1
...
#endif
    struct ModeDecisionContext *context_ptr, uint32_t num_of_cand_to_sort,
    uint32_t *cand_buff_indices) {
    uint32_t i, j, index;
    ModeDecisionCandidateBuffer **buffer_ptr_array =
        context_ptr->candidate_buffer_ptr_array;
...
void construct_best_sorted_arrays_md_stage_2(
    ...
    uint32_t *sorted_candidate_index_array, uint64_t *ref_fast_cost) {
    // best = union from all classes
    uint32_t best_candi = 0;
    for (CAND_CLASS class_i = CAND_CLASS_0; class_i < CAND_CLASS_TOTAL;
         class_i++)
#if REMOVE_MD_STAGE_1
        for (uint32_t candi = 0; candi < context_ptr->md_stage_1_count[class_i];
             candi++)
#else
        for (uint32_t candi = 0; candi < context_ptr->md_stage_2_count[class_i];
             candi++)
#endif
...
void construct_best_sorted_arrays_md_stage_3(
    ...
    for (CAND_CLASS class_i = CAND_CLASS_0; class_i < CAND_CLASS_TOTAL;
        ...
        for (uint32_t candi = 0; candi < context_ptr->md_stage_2_count[class_i];
             candi++)
#else
        for (uint32_t candi = 0; candi < context_ptr->md_stage_3_count[class_i];
             candi++)
#endif
            sorted_candidate_index_array[best_candi++] =
                context_ptr->cand_buff_indices[class_i][candi];
...
void md_stage_0(
    ...
    int32_t fast_candidate_start_index, int32_t fast_candidate_end_index,
    EbPictureBufferDesc *input_picture_ptr, uint32_t inputOriginIndex,
    uint32_t inputCbOriginIndex, uint32_t inputCrOriginIndex,
    CodingUnit *cu_ptr, uint32_t cuOriginIndex, uint32_t cuChromaOriginIndex,
    uint32_t candidate_buffer_start_index, uint32_t maxBuffers,
    EbBool scratch_buffer_pesent_flag, EbBool use_ssd) {
    ...
    int32_t bestFirstFastCostSearchCandidateIndex =
        INVALID_FAST_CANDIDATE_INDEX;
    ...
    context_ptr->md_staging_skip_interpolation_search =
        (context_ptr->md_staging_mode == MD_STAGING_MODE_1)
            ? EB_TRUE
            : context_ptr->interpolation_search_level >=
    ...
    context_ptr->md_staging_skip_inter_chroma_pred =
        (context_ptr->md_staging_mode == MD_STAGING_MODE_1 &&
         context_ptr->target_class != CAND_CLASS_0 &&
         context_ptr->target_class != CAND_CLASS_6 &&
...
#if REMOVE_MD_STAGE_1
    context_ptr->md_staging_use_bilinear =
        (context_ptr->md_staging_mode == MD_STAGING_MODE_1) ? EB_TRUE
                                                            : EB_FALSE;
#else
    context_ptr->md_staging_use_bilinear =
        (context_ptr->md_staging_mode) ? EB_TRUE : EB_FALSE;
#endif
    ...
    while (fastLoopCandidateIndex >= fast_candidate_start_index) {
        if (fast_candidate_array[fastLoopCandidateIndex].cand_class ==
            ...
            ModeDecisionCandidate *candidate_ptr =
                candidate_buffer->candidate_ptr =
                    &fast_candidate_array[fastLoopCandidateIndex];
            // Initialize tx_depth
            ...
            if (candidate_ptr->distortion_ready) {
                ...
                *(candidate_buffer->fast_cost_ptr) =
                    Av1ProductFastCostFuncTable[candidate_ptr->type](
                        ...
                        picture_control_set_ptr,
                        &(context_ptr
                              ->md_local_cu_unit[context_ptr->blk_geom
                                                     ->blkidx_mds]
                              .ed_ref_mv_stack[candidate_ptr->ref_frame_type]
                                              [0]),
                        context_ptr->blk_geom,
                        context_ptr->cu_origin_y >> MI_SIZE_LOG2,
                        context_ptr->cu_origin_x >> MI_SIZE_LOG2,
                ...
                // Keep track of the candidate index of the best  (src - src)
                // candidate
                if (*(candidate_buffer->fast_cost_ptr) <=
                    bestFirstFastCostSearchCandidateCost) {
                ...
                // Initialize Fast Cost - to do not interact with the second
                // Fast-Cost Search
    ...
    while (fastLoopCandidateIndex >= fast_candidate_start_index) {
        if (fast_candidate_array[fastLoopCandidateIndex].cand_class ==
            context_ptr->target_class) {
            ModeDecisionCandidateBuffer *candidate_buffer =
                candidate_buffer_ptr_array_base[highestCostIndex];
            ModeDecisionCandidate *candidate_ptr =
                candidate_buffer->candidate_ptr =
                    &fast_candidate_array[fastLoopCandidateIndex];
            // Initialize tx_depth
            ...
            if (fastLoopCandidateIndex || scratch_buffer_pesent_flag) {
                // maxCost is volatile to prevent the compiler from loading
                // 0xFFFFFFFFFFFFFF
                //   as a const at the early-out. Loading a large constant on
                //   intel x64 processors clogs the i-cache/intstruction decode.
                //   This still reloads the variable from the stack each pass,
                //   so a better solution would be to register the variable, but
                //   this might require asm.
    ...
    // Set the cost of the scratch canidate to max to get discarded @ the
    // sorting phase
...
#if !REMOVE_MD_STAGE_1
void md_stage_1(PictureControlSet *picture_control_set_ptr,
                ModeDecisionContext *context_ptr,
                ModeDecisionCandidateBuffer **candidate_buffer_ptr_array_base,
                uint32_t num_of_candidates,
                EbPictureBufferDesc *input_picture_ptr,
                uint32_t inputOriginIndex, uint32_t inputCbOriginIndex,
                uint32_t inputCrOriginIndex, CodingUnit *cu_ptr,
                uint32_t cuOriginIndex, uint32_t cuChromaOriginIndex,
                EbBool use_ssd) {
    // Set MD Staging fast_loop_core settings
    context_ptr->md_staging_skip_interpolation_search =
        (context_ptr->md_staging_mode == MD_STAGING_MODE_3)
    ...
    for (uint32_t cand_idx = 0; cand_idx < num_of_candidates; ++cand_idx) {
        uint32_t candidateIndex =
            context_ptr->cand_buff_indices[context_ptr->target_class][cand_idx];
        ModeDecisionCandidateBuffer *candidate_buffer =
            candidate_buffer_ptr_array_base[candidateIndex];
        ModeDecisionCandidate *candidate_ptr = candidate_buffer->candidate_ptr;
...
void predictive_me_full_pel_search(
    ...
    uint32_t inputOriginIndex, EbBool use_ssd, uint8_t list_idx, int8_t ref_idx,
    ...
    EbReferenceObject *refObj =
        picture_control_set_ptr->ref_pic_ptr_array[list_idx][ref_idx]
            ->object_ptr;
    EbPictureBufferDesc *ref_pic = hbd_mode_decision
                                       ? refObj->reference_picture16bit
                                       : refObj->reference_picture;
    for (int32_t refinement_pos_x = search_position_start_x;
        ...
        for (int32_t refinement_pos_y = search_position_start_y;
            ...
            uint32_t ref_origin_index =
                ref_pic->origin_x +
                (context_ptr->cu_origin_x + (mvx >> 3) + refinement_pos_x) +
                (context_ptr->cu_origin_y + (mvy >> 3) + ref_pic->origin_y +
                 refinement_pos_y) *
                    ref_pic->stride_y;
            if (use_ssd) {
                EbSpatialFullDistType spatial_full_dist_type_fun =
...
void predictive_me_sub_pel_search(
    PictureControlSet *picture_control_set_ptr,
    ModeDecisionContext *context_ptr, EbPictureBufferDesc *input_picture_ptr,
    uint32_t inputOriginIndex, uint32_t cuOriginIndex, EbBool use_ssd,
    uint8_t list_idx, int8_t ref_idx, int16_t mvx, int16_t mvy,
    ...
    uint32_t *best_distortion, uint8_t search_pattern) {
#if HBD2_PME
    uint8_t hbd_mode_decision = context_ptr->hbd_mode_decision == EB_DUAL_BIT_MD
                                    ? EB_8_BIT_MD
                                    : context_ptr->hbd_mode_decision;
    ...
    for (int32_t refinement_pos_x = search_position_start_x;
        ...
        for (int32_t refinement_pos_y = search_position_start_y;
            ...
            if (search_pattern == 1 && refinement_pos_x != 0 &&
                refinement_pos_y != 0)
                continue;
            ...
            candidate_ptr->ref_frame_type =
                svt_get_ref_frame_type(list_idx, ref_idx);
            ...
            ProductPredictionFunTable[INTER_MODE](hbd_mode_decision,
                                                  context_ptr,
                                                  picture_control_set_ptr,
                                                  candidate_buffer);
...
#endif
#endif
void predictive_me_search(PictureControlSet *picture_control_set_ptr,
                          ModeDecisionContext *context_ptr,
                          EbPictureBufferDesc *input_picture_ptr,
                          uint32_t inputOriginIndex, uint32_t cuOriginIndex) {
    ...
    // Update input origin
    inputOriginIndex =
        (context_ptr->cu_origin_y + input_picture_ptr->origin_y) *
            input_picture_ptr->stride_y +
        (context_ptr->cu_origin_x + input_picture_ptr->origin_x);
    ...
    for (uint32_t refIt = 0;
        ...
        if (rf[1] == NONE_FRAME) {
            ...
            if (list_idx == 0) {
                me_mv_x =
                    (me_results
                         ->me_mv_array[context_ptr->me_block_offset][ref_idx]
                         .x_mv)
                    << 1;
                me_mv_y =
                    (me_results
                ...
                me_mv_x =
                    (me_results
                         ->me_mv_array
                             ...
                             [((sequence_control_set_ptr->mrp_mode == 0) ? 4
                                                                         : 2) +
                              ref_idx]
                         .x_mv)
                    << 1;
            ...
            uint32_t ref_origin_index =
                ref_pic->origin_x +
                (context_ptr->cu_origin_x + (me_mv_x >> 3)) +
                (context_ptr->cu_origin_y + (me_mv_y >> 3) +
                 ref_pic->origin_y) *
                    ref_pic->stride_y;
            if (use_ssd) {
                EbSpatialFullDistType spatial_full_dist_type_fun =
            ...
            if (pa_me_distortion != 0 ||
                ...
                // NEAR
                maxDrlIndex =
                    GetMaxDrlIndex(xd->ref_mv_count[frame_type], NEARMV);
                ...
                for (int8_t mvp_index = 0; mvp_index < mvp_count; mvp_index++) {
                    ...
                    EbPictureBufferDesc *ref_pic =
                        hbd_mode_decision ? refObj->reference_picture16bit
                                          : refObj->reference_picture;

                    uint32_t ref_origin_index =
                        ref_pic->origin_x +
                        (context_ptr->cu_origin_x +
                         (mvp_x_array[mvp_index] >> 3)) +
                        (context_ptr->cu_origin_y +
                ...
                if (pa_me_distortion == 0)
                    ...
                    exit_predictive_me_sub_pel =
                        ((((best_search_distortion - pa_me_distortion) * 100) /
                          pa_me_distortion) < PREDICTIVE_ME_DEVIATION_TH)
                            ? EB_FALSE
                            : EB_TRUE;
                }
                ...
                if (exit_predictive_me_sub_pel == EB_FALSE ||
                    ...
                    if (context_ptr->predictive_me_level >= 2) {
                        ...
                        // 0: all possible position(s): horizontal, vertical,
                        // diagonal 1: horizontal, vertical 2: horizontal only
                        // 3: vertical only

                        // Step 3: perform half pel search around the best full
                        // pel position
                        search_pattern =
                            (context_ptr->predictive_me_level >= 4) ? 0 : 1;
                        ...
                        // Step 4: perform quarter pel search around the best
                        // half pel position
                        search_pattern =
                            (context_ptr->predictive_me_level >= 4) ? 0 : 1;
                        predictive_me_sub_pel_search(
...
#if EIGHT_PEL_PREDICTIVE_ME
                    // Step 5: perform eigh pel search around the best quarter
                    // pel position
                    if (picture_control_set_ptr->parent_pcs_ptr->frm_hdr
...
void AV1CostCalcCfl(PictureControlSet *picture_control_set_ptr,
                    ModeDecisionCandidateBuffer *candidate_buffer,
                    SuperBlock *sb_ptr, ModeDecisionContext *context_ptr,
                    uint32_t component_mask,
                    EbPictureBufferDesc *input_picture_ptr,
                    uint32_t inputCbOriginIndex, uint32_t cuChromaOriginIndex,
                    uint64_t full_distortion[DIST_CALC_TOTAL],
                    uint64_t *coeffBits, EbBool check_dc) {
    ModeDecisionCandidate *candidate_ptr = candidate_buffer->candidate_ptr;
    ...
    *coeffBits = 0;
    ...
    if (component_mask == COMPONENT_CHROMA_CB ||
        component_mask == COMPONENT_CHROMA || component_mask == COMPONENT_ALL) {
        ...
        if (!context_ptr->hbd_mode_decision) {
            eb_cfl_predict_lbd(
                ...
                &(candidate_buffer->prediction_ptr
                      ->buffer_cb[cuChromaOriginIndex]),
                candidate_buffer->prediction_ptr->stride_cb,
                &(candidate_buffer->cfl_temp_prediction_ptr
                      ->buffer_cb[cuChromaOriginIndex]),
        ...
        // Create new function
        cu_full_distortion_fast_tu_mode_r(sb_ptr,
                                          candidate_buffer,
                                          context_ptr,
                                          candidate_ptr,
                                          picture_control_set_ptr,
        ...
        full_distortion[DIST_CALC_RESIDUAL] +=
            cbFullDistortion[DIST_CALC_RESIDUAL];
        full_distortion[DIST_CALC_PREDICTION] +=
            cbFullDistortion[DIST_CALC_PREDICTION];
        *coeffBits += cb_coeff_bits;
    }
    if (component_mask == COMPONENT_CHROMA_CR ||
        component_mask == COMPONENT_CHROMA || component_mask == COMPONENT_ALL) {
        cbFullDistortion[DIST_CALC_RESIDUAL] = 0;
        crFullDistortion[DIST_CALC_RESIDUAL] = 0;
        cbFullDistortion[DIST_CALC_PREDICTION] = 0;
        ...
        candidate_ptr->v_has_coeff =
            *count_non_zero_coeffs[2] ? EB_TRUE : EB_FALSE;
        ...
        full_distortion[DIST_CALC_RESIDUAL] +=
            crFullDistortion[DIST_CALC_RESIDUAL];
        full_distortion[DIST_CALC_PREDICTION] +=
            crFullDistortion[DIST_CALC_PREDICTION];
...
/*************************Pick the best alpha for cfl mode  or Choose
 * DC******************************************************/
void cfl_rd_pick_alpha(PictureControlSet *picture_control_set_ptr,
                       ModeDecisionCandidateBuffer *candidate_buffer,
                       SuperBlock *sb_ptr, ModeDecisionContext *context_ptr,
                       EbPictureBufferDesc *input_picture_ptr,
    ...
    const int64_t mode_rd =
        RDCOST(context_ptr->full_lambda,
               (uint64_t)candidate_buffer->candidate_ptr->md_rate_estimation_ptr
                   ->intra_uv_mode_fac_bits[CFL_ALLOWED]
                                           [candidate_buffer->candidate_ptr
    ...
    for (int32_t plane = 0; plane < CFL_PRED_PLANES; plane++) {
        coeffBits = 0;
        full_distortion[DIST_CALC_RESIDUAL] = 0;
        for (int32_t joint_sign = 0; joint_sign < CFL_JOINT_SIGNS;
             joint_sign++) {
        ...
        for (int32_t pn_sign = CFL_SIGN_NEG; pn_sign < CFL_SIGNS; pn_sign++) {
            ...
            for (int32_t c = 0; c < CFL_ALPHABET_SIZE; c++) {
                ...
                for (int32_t i = 0; i < CFL_SIGNS; i++) {
                    const int32_t joint_sign =
                        PLANE_SIGN_TO_JOINT_SIGN(plane, pn_sign, i);
                    if (i == 0) {
                        ...
                        AV1CostCalcCfl(picture_control_set_ptr,
                                       candidate_buffer,
                                       sb_ptr,
                                       context_ptr,
                                       (plane == 0) ? COMPONENT_CHROMA_CB
                                                    : COMPONENT_CHROMA_CR,
                                       input_picture_ptr,
                                       inputCbOriginIndex,
                                       cuChromaOriginIndex,
    ...
    int64_t dc_rd = RDCOST(context_ptr->full_lambda,
                           coeffBits,
                           full_distortion[DIST_CALC_RESIDUAL]);
...
static void CflPrediction(PictureControlSet *picture_control_set_ptr,
                          ModeDecisionCandidateBuffer *candidate_buffer,
                          SuperBlock *sb_ptr, ModeDecisionContext *context_ptr,
                          EbPictureBufferDesc *input_picture_ptr,
                          uint32_t inputCbOriginIndex,
    ...
    if (context_ptr->blk_geom->has_uv) {
        ...
        AV1PerformInverseTransformReconLuma(
            picture_control_set_ptr, context_ptr, candidate_buffer);

        uint32_t recLumaOffset = ((context_ptr->blk_geom->origin_y >> 3) << 3) *
                                     candidate_buffer->recon_ptr->stride_y +
        ...
        // 3: Loop over alphas and find the best or choose DC
        cfl_rd_pick_alpha(picture_control_set_ptr,
                          candidate_buffer,
                          sb_ptr,
                          context_ptr,
                          input_picture_ptr,
        ...
        if (candidate_buffer->candidate_ptr->intra_chroma_mode == UV_CFL_PRED) {
            ...
            if (!context_ptr->hbd_mode_decision) {
                eb_cfl_predict_lbd(context_ptr->pred_buf_q3,
                                   &(candidate_buffer->prediction_ptr
                                         ->buffer_cb[cuChromaOriginIndex]),
                                   candidate_buffer->prediction_ptr->stride_cb,
                                   &(candidate_buffer->prediction_ptr
...
uint8_t get_skip_tx_search_flag(int32_t sq_size, uint64_t ref_fast_cost,
                                uint64_t cu_cost, uint64_t weight) {
    // NM: Skip tx search when the fast cost of the current mode candidate is
    // substansially
    // Larger than the best fast_cost (
    uint8_t tx_search_skip_flag =
        cu_cost >= ((ref_fast_cost * weight) / 100) ? 1 : 0;
...
void check_best_indepedant_cfl(
    PictureControlSet *picture_control_set_ptr,
    EbPictureBufferDesc *input_picture_ptr, ModeDecisionContext *context_ptr,
    uint32_t inputCbOriginIndex, uint32_t cuChromaOriginIndex,
    ModeDecisionCandidateBuffer *candidate_buffer, uint8_t cb_qp, uint8_t cr_qp,
    uint64_t *cbFullDistortion, uint64_t *crFullDistortion,
    uint64_t *cb_coeff_bits, uint64_t *cr_coeff_bits) {
#if FILTER_INTRA_FLAG
    if (candidate_buffer->candidate_ptr->filter_intra_mode !=
        FILTER_INTRA_MODES)
    ...
    if (candidate_buffer->candidate_ptr->intra_chroma_mode == UV_CFL_PRED) {
        chromaRate +=
            candidate_buffer->candidate_ptr->md_rate_estimation_ptr
                ->cfl_alpha_fac_bits[candidate_buffer->candidate_ptr
                                         ->cfl_alpha_signs][CFL_PRED_U]
                                    [CFL_IDX_U(candidate_buffer->candidate_ptr
                                                   ->cfl_alpha_idx)] +
            candidate_buffer->candidate_ptr->md_rate_estimation_ptr
                ->cfl_alpha_fac_bits
                    [candidate_buffer->candidate_ptr->cfl_alpha_signs]
    ...
    int distortion = (int)(cbFullDistortion[DIST_CALC_RESIDUAL] +
                           crFullDistortion[DIST_CALC_RESIDUAL]);
    int rate = (int)(coeff_rate + chromaRate +
                     candidate_buffer->candidate_ptr->fast_luma_rate);
    uint64_t cfl_uv_cost = RDCOST(context_ptr->full_lambda, rate, distortion);

    // cfl vs. best independant
    if (context_ptr
            ->best_uv_cost[candidate_buffer->candidate_ptr->intra_luma_mode]
                          [3 + candidate_buffer->candidate_ptr
                                   ->angle_delta[PLANE_TYPE_Y]] < cfl_uv_cost) {
        // Update the current candidate
        candidate_buffer->candidate_ptr->intra_chroma_mode =
            context_ptr->best_uv_mode
                [candidate_buffer->candidate_ptr->intra_luma_mode]
                [MAX_ANGLE_DELTA +
                 candidate_buffer->candidate_ptr->angle_delta[PLANE_TYPE_Y]];
        candidate_buffer->candidate_ptr->angle_delta[PLANE_TYPE_UV] =
            context_ptr->best_uv_angle
                [candidate_buffer->candidate_ptr->intra_luma_mode]
                [MAX_ANGLE_DELTA +
                 candidate_buffer->candidate_ptr->angle_delta[PLANE_TYPE_Y]];
        ...
        // check if candidate_buffer->candidate_ptr->fast_luma_rate =
        // context_ptr->fast_luma_rate[candidate_buffer->candidate_ptr->intra_luma_mode];
        candidate_buffer->candidate_ptr->fast_chroma_rate =
            context_ptr->fast_chroma_rate
                [candidate_buffer->candidate_ptr->intra_luma_mode]
                [MAX_ANGLE_DELTA +
                 candidate_buffer->candidate_ptr->angle_delta[PLANE_TYPE_Y]];
        ...
        *cr_coeff_bits = 0;
        ...
        cu_full_distortion_fast_tu_mode_r(context_ptr->sb_ptr,
                                          candidate_buffer,
                                          context_ptr,
                                          candidate_buffer->candidate_ptr,
                                          picture_control_set_ptr,
...
EbErrorType av1_intra_luma_prediction(
    ...
    uint8_t tx_width =
        md_context_ptr->blk_geom
            ->tx_width[md_context_ptr->tx_depth][md_context_ptr->txb_itr];
    uint8_t tx_height =
        md_context_ptr->blk_geom
            ->tx_height[md_context_ptr->tx_depth][md_context_ptr->txb_itr];
    ...
    uint32_t intraLumaModeLeftNeighborIndex =
    ...
    md_context_ptr->intra_luma_left_mode = (uint32_t)(
        (md_context_ptr->mode_type_neighbor_array
             ->left_array[modeTypeLeftNeighborIndex] != INTRA_MODE)
            ? DC_PRED /*EB_INTRA_DC*/
            : (uint32_t)md_context_ptr->intra_luma_mode_neighbor_array
                  ->left_array[intraLumaModeLeftNeighborIndex]);

    md_context_ptr->intra_luma_top_mode = (uint32_t)(
        (md_context_ptr->mode_type_neighbor_array
             ->top_array[modeTypeTopNeighborIndex] != INTRA_MODE)
            ? DC_PRED /*EB_INTRA_DC*/
            : (uint32_t)
                  md_context_ptr->intra_luma_mode_neighbor_array->top_array
                      [intraLumaModeTopNeighborIndex]);  //   use DC. This seems
                                                         //   like we could use
                                                         //   a LCU-width

    TxSize tx_size =
        md_context_ptr->blk_geom
            ->txsize[md_context_ptr->tx_depth][md_context_ptr->txb_itr];
    ...
    if (!md_context_ptr->hbd_mode_decision) {
        ...
        if (txb_origin_y != 0)
            memcpy(
                topNeighArray + 1,
                md_context_ptr->tx_search_luma_recon_neighbor_array->top_array +
                    txb_origin_x,
                tx_width * 2);
        if (txb_origin_x != 0)
        ...
        if (txb_origin_y != 0 && txb_origin_x != 0)
            topNeighArray[0] = leftNeighArray[0] =
                md_context_ptr->tx_search_luma_recon_neighbor_array
                    ->top_left_array[MAX_PICTURE_HEIGHT_SIZE + txb_origin_x -
                                     txb_origin_y];
        ...
        eb_av1_predict_intra_block(
...
#if PAL_SUP
            candidate_buffer_ptr->candidate_ptr->palette_info.pmi
                    .palette_size[0] > 0,
            ...
            md_context_ptr->blk_geom->tx_boff_x[md_context_ptr->tx_depth]
                                               [md_context_ptr->txb_itr] >>
                2,  // int32_t col_off,
            md_context_ptr->blk_geom->tx_boff_y[md_context_ptr->tx_depth]
                                               [md_context_ptr->txb_itr] >>
                2,         // int32_t row_off,
            ...
            md_context_ptr->blk_geom
                ->tx_org_x[md_context_ptr->tx_depth]
                          [md_context_ptr->txb_itr],  // uint32_t cuOrgX used
                                                      // only for prediction Ptr
            md_context_ptr->blk_geom
                ->tx_org_y[md_context_ptr->tx_depth]
                          [md_context_ptr->txb_itr]  // uint32_t cuOrgY used
                                                     // only for prediction Ptr
        );
    } else {
        ...
        if (txb_origin_y != 0)
            memcpy(topNeighArray + 1,
                   (uint16_t *)(md_context_ptr
                                    ->tx_search_luma_recon_neighbor_array16bit
                                    ->top_array) +
                       txb_origin_x,
...
static void tx_search_update_recon_sample_neighbor_array(
    ...
    EbPictureBufferDesc *recon_buffer, uint32_t tu_origin_x,
    uint32_t tu_origin_y, uint32_t input_origin_x, uint32_t input_origin_y,
...
uint8_t get_end_tx_depth(BlockSize bsize, uint8_t btype) {
    ...
    else if (bsize == BLOCK_8X8 || bsize == BLOCK_64X16 ||
             bsize == BLOCK_16X64 || bsize == BLOCK_32X8 ||
...
void tx_initialize_neighbor_arrays(PictureControlSet *picture_control_set_ptr,
                                   ModeDecisionContext *context_ptr,
                                   EbBool is_inter) {
            ...
            context_ptr->tx_search_luma_recon_neighbor_array16bit =
                (context_ptr->tx_depth)
                    ? picture_control_set_ptr
                          ->md_tx_depth_1_luma_recon_neighbor_array16bit
                              [MD_NEIGHBOR_ARRAY_INDEX]
                    : picture_control_set_ptr->md_luma_recon_neighbor_array16bit
                          [MD_NEIGHBOR_ARRAY_INDEX];
        else
#endif
            context_ptr->tx_search_luma_recon_neighbor_array =
                (context_ptr->tx_depth)
                    ? picture_control_set_ptr
                          ->md_tx_depth_1_luma_recon_neighbor_array
                              [MD_NEIGHBOR_ARRAY_INDEX]
                    : picture_control_set_ptr->md_luma_recon_neighbor_array
                          [MD_NEIGHBOR_ARRAY_INDEX];
    }
        ...
        (context_ptr->tx_depth == 1)
            ? picture_control_set_ptr
                  ->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array
                      [MD_NEIGHBOR_ARRAY_INDEX]
            : picture_control_set_ptr
                  ->md_luma_dc_sign_level_coeff_neighbor_array
                      [MD_NEIGHBOR_ARRAY_INDEX];
}

void tx_update_neighbor_arrays(PictureControlSet *picture_control_set_ptr,
                               ModeDecisionContext *context_ptr,
                               ModeDecisionCandidateBuffer *candidate_buffer,
                               EbBool is_inter) {
    if (context_ptr->tx_depth) {
        if (!is_inter)
            tx_search_update_recon_sample_neighbor_array(
                ...
                candidate_buffer->recon_ptr,
                context_ptr->blk_geom
                    ->tx_org_x[context_ptr->tx_depth][context_ptr->txb_itr],
                context_ptr->blk_geom
                    ->tx_org_y[context_ptr->tx_depth][context_ptr->txb_itr],
                context_ptr->sb_origin_x +
                    context_ptr->blk_geom
                        ->tx_org_x[context_ptr->tx_depth][context_ptr->txb_itr],
                context_ptr->sb_origin_y +
...
void tx_reset_neighbor_arrays(PictureControlSet *picture_control_set_ptr,
                              ModeDecisionContext *context_ptr, EbBool is_inter,
    ...
    if (end_tx_depth) {
        if (!is_inter) {
...
#if ATB_FIX
            {
                copy_neigh_arr(
                    picture_control_set_ptr
                        ->md_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX],
                    picture_control_set_ptr
                        ->md_tx_depth_1_luma_recon_neighbor_array
                            [MD_NEIGHBOR_ARRAY_INDEX],
                    context_ptr->sb_origin_x + context_ptr->blk_geom->origin_x,
...
#endif
        copy_neigh_arr(
            picture_control_set_ptr->md_luma_dc_sign_level_coeff_neighbor_array
                [MD_NEIGHBOR_ARRAY_INDEX],
            picture_control_set_ptr
                ->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array
                    [MD_NEIGHBOR_ARRAY_INDEX],
...
void tx_type_search(SequenceControlSet *sequence_control_set_ptr,
                    PictureControlSet *picture_control_set_ptr,
                    ModeDecisionContext *context_ptr,
                    ModeDecisionCandidateBuffer *candidate_buffer,
                    uint32_t qp) {
...
#endif
    int32_t seg_qp =
        picture_control_set_ptr->parent_pcs_ptr->frm_hdr.segmentation_params
                .segmentation_enabled
            ? picture_control_set_ptr->parent_pcs_ptr->frm_hdr
                  .segmentation_params
                  .feature_data[context_ptr->cu_ptr->segment_id][SEG_LVL_ALT_Q]
            : 0;
    ...
    int32_t tx_type;
    TxSize txSize = context_ptr->blk_geom
    ...
    const TxSetType tx_set_type = get_ext_tx_set_type(
        txSize, is_inter, context_ptr->tx_search_reduced_set);
#else
        const TxSetType tx_set_type = get_ext_tx_set_type(
            txSize,
            is_inter,
            picture_control_set_ptr->parent_pcs_ptr->tx_search_reduced_set);
#endif
    ...
    uint32_t input_tu_origin_index = (context_ptr->sb_origin_x + txb_origin_x +
    ...
    get_txb_ctx(sequence_control_set_ptr,
                COMPONENT_LUMA,
                context_ptr->full_loop_luma_dc_sign_level_coeff_neighbor_array,
    ...
    for (tx_type = txk_start; tx_type < txk_end; ++tx_type) {
            ...
            if (is_inter) {
...
#if MULTI_PASS_PD
                const TxSetType tx_set_type = get_ext_tx_set_type(
                    max_tx_size, is_inter, context_ptr->tx_search_reduced_set);
                int32_t eset = get_ext_tx_set(
                    ...
                    const TxSetType tx_set_type = get_ext_tx_set_type(
                        ...
                        is_inter,
                        picture_control_set_ptr->parent_pcs_ptr
                            ->tx_search_reduced_set);
                    int32_t eset =
                        get_ext_tx_set(max_tx_size,
                                       is_inter,
                                       picture_control_set_ptr->parent_pcs_ptr
                                           ->tx_search_reduced_set);
#endif
                // eset == 0 should correspond to a set with only DCT_DCT and
                // there is no need to send the tx_type
                if (eset <= 0)
                    continue;
                else if (av1_ext_tx_used[tx_set_type][tx_type] == 0)
                    continue;
                else if (context_ptr->blk_geom
                                 ->tx_height[context_ptr->tx_depth]
                                            [context_ptr->txb_itr] > 32 ||
                         context_ptr->blk_geom->tx_width[context_ptr->tx_depth]
                                                        [context_ptr->txb_itr] >
                             32)
                    continue;
            }
...
#endif
            if (!allowed_tx_set_a[context_ptr->blk_geom
                                      ->txsize[context_ptr->tx_depth]
                                              [context_ptr->txb_itr]][tx_type])
                continue;

        // For Inter blocks, transform type of chroma follows luma transfrom
        // type
        if (is_inter)
            candidate_buffer->candidate_ptr->transform_type_uv =
                (context_ptr->txb_itr == 0)
                    ? candidate_buffer->candidate_ptr
                          ->transform_type[context_ptr->txb_itr]
                    : candidate_buffer->candidate_ptr->transform_type_uv;

        // Y: T Q iQ
        av1_estimate_transform(
            &(((int16_t *)
                   candidate_buffer->residual_ptr->buffer_y)[tu_origin_index]),
            candidate_buffer->residual_ptr->stride_y,
            &(((int32_t *)context_ptr->trans_quant_buffers_ptr
                   ->tu_trans_coeff2_nx2_n_ptr
            ...
            context_ptr->hbd_mode_decision ? BIT_INCREMENT_10BIT
                                           : BIT_INCREMENT_8BIT,
            tx_type,
            PLANE_TYPE_Y,
            DEFAULT_SHAPE);
        ...
        av1_quantize_inv_quantize(
            ...
            &(((int32_t *)candidate_buffer->residual_quant_coeff_ptr
                   ->buffer_y)[context_ptr->txb_1d_offset]),
            &(((int32_t *)candidate_buffer->recon_coeff_ptr
                   ->buffer_y)[context_ptr->txb_1d_offset]),
            qp,
            seg_qp,
            context_ptr->blk_geom
        ...
        // tx_type not equal to DCT_DCT and no coeff is not an acceptable option
        // in AV1.
        ...
        if (y_has_coeff)
            inv_transform_recon_wrapper(
                ...
                tx_type,
                PLANE_TYPE_Y,
                (uint16_t)candidate_buffer->candidate_ptr
                    ->eob[0][context_ptr->txb_itr]);
        else
            picture_copy(
                candidate_buffer->prediction_ptr,
                tu_origin_index,
        ...
        av1_tu_estimate_coeff_bits(
            ...
            context_ptr->blk_geom
                ->txsize[context_ptr->tx_depth][context_ptr->txb_itr],
            context_ptr->blk_geom
                ->txsize_uv[context_ptr->tx_depth][context_ptr->txb_itr],
            tx_type,
            candidate_buffer->candidate_ptr->transform_type_uv,
            COMPONENT_LUMA);
        ...
        uint64_t cost = RDCOST(context_ptr->full_lambda,
                               y_tu_coeff_bits,
                               tuFullDistortion[0][DIST_CALC_RESIDUAL]);
...
static INLINE int get_vartx_max_txsize(
    /*const MbModeInfo *xd,*/ BlockSize bsize, int plane) {
...
static INLINE void txfm_partition_update(TXFM_CONTEXT *above_ctx,
                                         TXFM_CONTEXT *left_ctx, TxSize tx_size,
                                         TxSize txb_size) {
...
static INLINE int txfm_partition_context(TXFM_CONTEXT *above_ctx,
    ...
    if (max_tx_size >= TX_8X8) {
        category = (txsize_sqr_up_map[tx_size] != max_tx_size &&
                    max_tx_size > TX_8X8) +
...
static uint64_t cost_tx_size_vartx(
    MacroBlockD *xd, const MbModeInfo *mbmi, TxSize tx_size, int depth,
    int blk_row, int blk_col, MdRateEstimationContext *md_rate_estimation_ptr) {
    uint64_t bits = 0;
...
static INLINE int get_tx_size_context(const MacroBlockD *xd) {
    ...
    if (has_above)
        if (is_inter_block(&above_mbmi->block_mi))
            above =
                block_size_wide[above_mbmi->block_mi.sb_type] >= max_tx_wide;
...
static uint64_t cost_selected_tx_size(
    const MacroBlockD *xd, MdRateEstimationContext *md_rate_estimation_ptr) {
    ...
    if (block_signals_txsize(bsize)) {
        ...
        bits += md_rate_estimation_ptr
                    ->tx_size_fac_bits[tx_size_cat][tx_size_ctx][depth];
    }

    return bits;
}

static uint64_t tx_size_bits(MdRateEstimationContext *md_rate_estimation_ptr,
                             MacroBlockD *xd, const MbModeInfo *mbmi,
                             TxMode tx_mode, BlockSize bsize, uint8_t skip) {
    uint64_t bits = 0;

    int is_inter_tx =
        is_inter_block(&mbmi->block_mi) || is_intrabc_block(&mbmi->block_mi);
    if (tx_mode == TX_MODE_SELECT && block_signals_txsize(bsize) &&
        ...
        set_txfm_ctxs(mbmi->tx_size,
                      xd->n8_w,
                      xd->n8_h,
                      skip && is_inter_block(&mbmi->block_mi),
                      xd);
    }
    return bits;
}

void set_mi_row_col(PictureControlSet *picture_control_set_ptr, MacroBlockD *xd,
                    TileInfo *tile, int mi_row, int bh, int mi_col, int bw,
                    uint32_t mi_stride, int mi_rows, int mi_cols);

uint64_t estimate_tx_size_bits(
    PictureControlSet *pcsPtr, ModeDecisionContext *context_ptr,
    ModeDecisionCandidate *candidate_ptr, EbBool skip_flag,
    uint32_t cu_origin_x, uint32_t cu_origin_y, CodingUnit *cu_ptr,
    ...
    uint32_t txfm_context_left_index =
        get_neighbor_array_unit_left_index(txfm_context_array, cu_origin_y);
    uint32_t txfm_context_above_index =
        get_neighbor_array_unit_top_index(txfm_context_array, cu_origin_x);
    ...
    uint64_t bits = tx_size_bits(
        md_rate_estimation_ptr, xd, mbmi, tx_mode, bsize, skip_flag);
...
uint64_t get_tx_size_bits(ModeDecisionCandidateBuffer *candidateBuffer,
    ...
    tx_size_bits = estimate_tx_size_bits(picture_control_set_ptr,
                                         context_ptr,
...
void tx_partitioning_path(ModeDecisionCandidateBuffer *candidate_buffer,
                          ModeDecisionContext *context_ptr,
                          PictureControlSet *picture_control_set_ptr,
                          uint64_t ref_fast_cost, uint8_t end_tx_depth,
                          uint32_t qp, uint32_t *y_count_non_zero_coeffs,
...
void tx_partitioning_path(ModeDecisionCandidateBuffer *candidate_buffer,
    ...
    if (is_inter) {
        ...
        if (context_ptr->hbd_mode_decision) {
            // Copy pred
            {
                uint16_t *src = &(((uint16_t *)candidate_buffer->prediction_ptr
                                       ->buffer_y)[block_index]);
                uint16_t *dst =
                    &(((uint16_t *)context_ptr->scratch_candidate_buffer
                           ->prediction_ptr->buffer_y)[block_index]);

                for (int i = 0; i < context_ptr->blk_geom->bheight; i++) {
                    memcpy(dst,
                           src,
                           context_ptr->blk_geom->bwidth * sizeof(uint16_t));
                    src += candidate_buffer->prediction_ptr->stride_y;
                    dst += context_ptr->scratch_candidate_buffer->prediction_ptr
                               ->stride_y;
                }
            }
            ...
            // Copy residual
            {
                int16_t *src = &(((int16_t *)candidate_buffer->residual_ptr
                                      ->buffer_y)[block_index]);
                int16_t *dst =
                    &(((int16_t *)context_ptr->scratch_candidate_buffer
                           ->residual_ptr->buffer_y)[block_index]);

                for (int i = 0; i < context_ptr->blk_geom->bheight; i++) {
                    memcpy(dst, src, context_ptr->blk_geom->bwidth << 1);
                    src += candidate_buffer->residual_ptr->stride_y;
                    dst += context_ptr->scratch_candidate_buffer->residual_ptr
                               ->stride_y;
                }
            }
        } else
#endif
        {
            ...
            {
                EbByte src =
                    &(candidate_buffer->prediction_ptr->buffer_y[block_index]);
                EbByte dst = &(context_ptr->scratch_candidate_buffer
                                   ->prediction_ptr->buffer_y[block_index]);
                for (int i = 0; i < context_ptr->blk_geom->bheight; i++) {
    ...
    if (context_ptr->md_staging_tx_search == 0)
        ...
        tx_search_skip_flag =
            context_ptr->tx_search_level == TX_SEARCH_FULL_LOOP ? EB_FALSE
                                                                : EB_TRUE;
#else
        if (context_ptr->md_staging_tx_search == 0)
            tx_search_skip_flag = EB_TRUE;
            ...
            tx_search_skip_flag =
                picture_control_set_ptr->parent_pcs_ptr->tx_search_level ==
                        TX_SEARCH_FULL_LOOP
                    ? get_skip_tx_search_flag(
                          context_ptr->blk_geom->sq_size,
                          ref_fast_cost,
                          *candidate_buffer->fast_cost_ptr,
                          picture_control_set_ptr->parent_pcs_ptr->tx_weight)
                    : EB_TRUE;
        else
            tx_search_skip_flag =
                picture_control_set_ptr->parent_pcs_ptr->tx_search_level ==
    ...
    tx_reset_neighbor_arrays(
        picture_control_set_ptr, context_ptr, is_inter, end_tx_depth);
    ...
    for (context_ptr->tx_depth = 0; context_ptr->tx_depth <= end_tx_depth;
        ...
        tx_initialize_neighbor_arrays(
            picture_control_set_ptr, context_ptr, is_inter);
        ...
        uint16_t txb_count =
            context_ptr->blk_geom->txb_count[context_ptr->tx_depth];
        ...
        for (context_ptr->txb_itr = 0; context_ptr->txb_itr < txb_count;
            ...
            if (!is_inter) {
                av1_intra_luma_prediction(
                    context_ptr, picture_control_set_ptr, tx_candidate_buffer);
                ...
                residual_kernel(
                    ...
                    context_ptr->blk_geom
                        ->tx_width[context_ptr->tx_depth][context_ptr->txb_itr],
                    context_ptr->blk_geom->tx_height[context_ptr->tx_depth]
                                                    [context_ptr->txb_itr]);
                    ...
                    residual_kernel8bit(
                        ...
                        input_picture_ptr->stride_y,
                        &(tx_candidate_buffer->prediction_ptr
                              ->buffer_y[tu_origin_index]),
                        tx_candidate_buffer->prediction_ptr->stride_y,
                        &(((int16_t *)tx_candidate_buffer->residual_ptr
                               ->buffer_y)[tu_origin_index]),
                        tx_candidate_buffer->residual_ptr->stride_y,
                        context_ptr->blk_geom->tx_width[context_ptr->tx_depth]
                                                       [context_ptr->txb_itr],
            ...
            if (!tx_search_skip_flag) {
                tx_type_search(sequence_control_set_ptr,
                               picture_control_set_ptr,
                               context_ptr,
                               tx_candidate_buffer,
                               qp);
            ...
            uint32_t y_has_coeff =
                tx_y_count_non_zero_coeffs[context_ptr->txb_itr] > 0;

            tx_update_neighbor_arrays(picture_control_set_ptr,
                                      context_ptr,
                                      tx_candidate_buffer,
                                      is_inter);
        ...
        uint64_t tx_size_bits = 0;
        if (picture_control_set_ptr->parent_pcs_ptr->frm_hdr.tx_mode ==
            TX_MODE_SELECT)
    ...
    if (best_tx_depth == 1) {
        ...
        if (context_ptr->hbd_mode_decision) {
            ...
            uint16_t *src = &(((uint16_t *)context_ptr->scratch_candidate_buffer
                                   ->prediction_ptr->buffer_y)[block_index]);
            uint16_t *dst = &(((uint16_t *)candidate_buffer->prediction_ptr
                                   ->buffer_y)[block_index]);
            for (int i = 0; i < context_ptr->blk_geom->bheight; i++) {
                memcpy(
                    dst, src, context_ptr->blk_geom->bwidth * sizeof(uint16_t));
                src += context_ptr->scratch_candidate_buffer->prediction_ptr
                           ->stride_y;
...
#endif
        {
            EbByte src = &(context_ptr->scratch_candidate_buffer->prediction_ptr
                               ->buffer_y[block_index]);
            EbByte dst =
                &(candidate_buffer->prediction_ptr->buffer_y[block_index]);
            for (int i = 0; i < context_ptr->blk_geom->bheight; i++) {
...
void perform_intra_tx_partitioning(
    ModeDecisionCandidateBuffer *candidate_buffer,
    ModeDecisionContext *context_ptr,
    PictureControlSet *picture_control_set_ptr, uint64_t ref_fast_cost,
    uint8_t end_tx_depth, uint32_t qp, uint32_t *y_count_non_zero_coeffs,
    uint64_t *y_coeff_bits, uint64_t *y_full_distortion) {
    EbPictureBufferDesc *input_picture_ptr =
    ...
    TxType best_tx_type_depth_0 =
        DCT_DCT;  // Track the best tx type @ depth 0 to be used @ the final
                  // stage (i.e. avoid redoing the tx type search).
    uint8_t tx_search_skip_flag;
    if (context_ptr->md_staging_tx_search == 0)
        tx_search_skip_flag = EB_TRUE;
    else if (context_ptr->md_staging_tx_search == 1)
    ...
    for (context_ptr->tx_depth = 0; context_ptr->tx_depth <= end_tx_depth;
        ...
        *y_coeff_bits = 0;
        ...
        for (context_ptr->txb_itr = 0; context_ptr->txb_itr < txb_count;
            ...
            // Y Prediction
            av1_intra_luma_prediction(
                context_ptr, picture_control_set_ptr, candidate_buffer);
            ...
            if (!tx_search_skip_flag) {
                ...
                for (int32_t tx_type = txk_start; tx_type < txk_end;
                     ++tx_type) {
                    y_tu_coeff_bits = 0;
                    ...
                    int32_t eset = get_ext_tx_set(
                        ...
                        0,
                        0);  // assumes INTRA
                    // eset == 0 should correspond to a set with only DCT_DCT
                    // and there is no need to send the tx_type
                    if (eset <= 0)
                        continue;
                    else if (av1_ext_tx_used[tx_set_type][tx_type] == 0)
                        continue;
                    ...
                    int32_t seg_qp =
                        picture_control_set_ptr->parent_pcs_ptr->frm_hdr
                            ...
                            ? picture_control_set_ptr->parent_pcs_ptr->frm_hdr
                                  .segmentation_params
                                  .feature_data[context_ptr->cu_ptr->segment_id]
                                               [SEG_LVL_ALT_Q]
                            : 0;
                    // Y: T Q iQ
                    av1_estimate_transform(
                        &(((int16_t *)candidate_buffer->residual_ptr
                               ->buffer_y)[tu_origin_index]),
                        candidate_buffer->residual_ptr->stride_y,
                        &(((int32_t *)context_ptr->trans_quant_buffers_ptr
                               ->tu_trans_coeff2_nx2_n_ptr
                    ...
                    uint32_t y_has_coeff =
                        y_count_non_zero_coeffs[context_ptr->txb_itr] > 0;

                    // tx_type not equal to DCT_DCT and no coeff is not an
                    // acceptable option in AV1.
                    if (y_has_coeff == 0 && tx_type != DCT_DCT)
                        continue;
                    if (y_has_coeff)
                        inv_transform_recon_wrapper(
                            ...
                            candidate_buffer->recon_ptr->stride_y,
                            (int32_t *)
                                candidate_buffer->recon_coeff_ptr->buffer_y,
                            txb_1d_offset,
                            picture_control_set_ptr->hbd_mode_decision,
                            context_ptr->blk_geom->txsize[context_ptr->tx_depth]
                                                         [context_ptr->txb_itr],
                    ...
                    tuFullDistortion[0][DIST_CALC_PREDICTION] =
                    ...
                    av1_tu_estimate_coeff_bits(
                        ...
                        context_ptr->blk_geom->txsize[context_ptr->tx_depth]
                                                     [context_ptr->txb_itr],
                        context_ptr->blk_geom->txsize_uv[context_ptr->tx_depth]
                                                        [context_ptr->txb_itr],
                        tx_type,
                        candidate_buffer->candidate_ptr->transform_type_uv,
                        COMPONENT_LUMA);
                ...
                best_tx_type_depth_0 = (context_ptr->tx_depth == 0)
                                           ? best_tx_type
                                           : best_tx_type_depth_0;
            }
            //  Best Tx Type Pass
            candidate_buffer->candidate_ptr
                ->transform_type[context_ptr->txb_itr] = best_tx_type;
            ...
            av1_estimate_transform(
                ...
                candidate_buffer->residual_ptr->stride_y,
                &(((int32_t *)context_ptr->trans_quant_buffers_ptr
                       ->tu_trans_coeff2_nx2_n_ptr->buffer_y)[txb_1d_offset]),
                NOT_USED_VALUE,
                context_ptr->blk_geom
                    ->txsize[context_ptr->tx_depth][context_ptr->txb_itr],
                &context_ptr->three_quad_energy,
                context_ptr->transform_inner_array_ptr,
                picture_control_set_ptr->hbd_mode_decision ? BIT_INCREMENT_10BIT
                                                           : BIT_INCREMENT_8BIT,
                candidate_buffer->candidate_ptr
                    ->transform_type[context_ptr->txb_itr],
                PLANE_TYPE_Y,
            ...
            candidate_buffer->candidate_ptr->quantized_dc
                [0][context_ptr->txb_itr] = av1_quantize_inv_quantize(
                picture_control_set_ptr,
                context_ptr,
                &(((int32_t *)context_ptr->trans_quant_buffers_ptr
                       ->tu_trans_coeff2_nx2_n_ptr->buffer_y)[txb_1d_offset]),
            ...
            if (context_ptr->tx_depth) {
                ...
                tx_search_update_recon_sample_neighbor_array(
                    ...
                    context_ptr->blk_geom
                        ->tx_org_y[context_ptr->tx_depth][context_ptr->txb_itr],
                    context_ptr->sb_origin_x +
                        context_ptr->blk_geom->tx_org_x[context_ptr->tx_depth]
                                                       [context_ptr->txb_itr],
                    context_ptr->sb_origin_y +
                        context_ptr->blk_geom->tx_org_y[context_ptr->tx_depth]
                                                       [context_ptr->txb_itr],
                    context_ptr->blk_geom
                        ->tx_width[context_ptr->tx_depth][context_ptr->txb_itr],
                    context_ptr->blk_geom->tx_height[context_ptr->tx_depth]
    ...
    if (context_ptr->tx_depth == 0) {
        ...
        *y_coeff_bits = 0;
        ...
        for (context_ptr->txb_itr = 0; context_ptr->txb_itr < txb_count;
            ...
            candidate_buffer->candidate_ptr
                ->transform_type[context_ptr->txb_itr] = best_tx_type_depth_0;
...
void full_loop_core(PictureControlSet *picture_control_set_ptr,
                    SuperBlock *sb_ptr, CodingUnit *cu_ptr,
                    ModeDecisionContext *context_ptr,
                    ModeDecisionCandidateBuffer *candidate_buffer,
                    ModeDecisionCandidate *candidate_ptr,
                    EbPictureBufferDesc *input_picture_ptr,
    ...
    if (candidate_ptr->type != INTRA_MODE) {
        ...
        if (picture_control_set_ptr->parent_pcs_ptr
                ->interpolation_search_level > IT_SEARCH_OFF)
            if (picture_control_set_ptr->parent_pcs_ptr
                        ->interpolation_search_level == IT_SEARCH_FULL_LOOP ||
...
#if MULTI_PASS_PD
    if (context_ptr->md_atb_mode &&
        context_ptr->md_staging_skip_atb == EB_FALSE && end_tx_depth &&
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->atb_mode &&
        ...
        if (!tx_search_skip_flag) {
            product_full_loop_tx_search(
                candidate_buffer, context_ptr, picture_control_set_ptr);
    ...
    if (context_ptr->md_staging_skip_full_chroma == EB_FALSE) {
        ...
        if (candidate_ptr->type == INTRA_MODE &&
            ...
            // 3: Loop over alphas and find the best or choose DC
            // 4: Recalculate the residual for chroma
            CflPrediction(picture_control_set_ptr,
                          candidate_buffer,
                          sb_ptr,
                          context_ptr,
                          input_picture_ptr,
        ...
        if (context_ptr->blk_geom->has_uv &&
            ...
            if (candidate_buffer->candidate_ptr->type == INTRA_MODE &&
                (candidate_buffer->candidate_ptr->intra_chroma_mode ==
                 ...
                 candidate_buffer->candidate_ptr->intra_chroma_mode ==
                     UV_DC_PRED)) {
                check_best_indepedant_cfl(picture_control_set_ptr,
                                          input_picture_ptr,
                                          context_ptr,
                                          inputCbOriginIndex,
                                          cuChromaOriginIndex,
    ...
    candidate_ptr->block_has_coeff =
        (candidate_ptr->y_has_coeff | candidate_ptr->u_has_coeff |
         candidate_ptr->v_has_coeff)
            ? EB_TRUE
            : EB_FALSE;
    ...
    candidate_buffer->cb_distortion[DIST_CALC_RESIDUAL] =
        cbFullDistortion[DIST_CALC_RESIDUAL];
    candidate_buffer->cb_distortion[DIST_CALC_PREDICTION] =
        cbFullDistortion[DIST_CALC_PREDICTION];
    candidate_buffer->cb_coeff_bits = cb_coeff_bits;

    candidate_buffer->cr_distortion[DIST_CALC_RESIDUAL] =
        crFullDistortion[DIST_CALC_RESIDUAL];
    candidate_buffer->cr_distortion[DIST_CALC_PREDICTION] =
        crFullDistortion[DIST_CALC_PREDICTION];
    candidate_buffer->cr_coeff_bits = cr_coeff_bits;
    candidate_buffer->candidate_ptr->full_distortion =
        (uint32_t)(y_full_distortion[0]);
...
void md_stage_2(
#endif
    PictureControlSet *picture_control_set_ptr, SuperBlock *sb_ptr,
    CodingUnit *cu_ptr, ModeDecisionContext *context_ptr,
    EbPictureBufferDesc *input_picture_ptr, uint32_t inputOriginIndex,
    uint32_t inputCbOriginIndex, uint32_t cuOriginIndex,
    uint32_t cuChromaOriginIndex, uint64_t ref_fast_cost) {
    ModeDecisionCandidateBuffer **candidate_buffer_ptr_array_base =
        context_ptr->candidate_buffer_ptr_array;
    ...
    for (fullLoopCandidateIndex = 0;
         ...
         context_ptr->md_stage_1_count[context_ptr->target_class];
         ++fullLoopCandidateIndex) {
#else
    ...
    for (fullLoopCandidateIndex = 0;
         fullLoopCandidateIndex <
         context_ptr->md_stage_2_count[context_ptr->target_class];
         ++fullLoopCandidateIndex) {
#endif
        candidateIndex =
            context_ptr->cand_buff_indices[context_ptr->target_class]
                                          [fullLoopCandidateIndex];
...
#endif
        full_loop_core(picture_control_set_ptr,
                       sb_ptr,
                       cu_ptr,
                       context_ptr,
                       candidate_buffer,
...
void md_stage_3(
    ...
    EbPictureBufferDesc *input_picture_ptr, uint32_t inputOriginIndex,
    uint32_t inputCbOriginIndex, uint32_t cuOriginIndex,
    uint32_t cuChromaOriginIndex, uint32_t fullCandidateTotalCount,
    uint64_t ref_fast_cost) {
    ModeDecisionCandidateBuffer **candidate_buffer_ptr_array_base =
        context_ptr->candidate_buffer_ptr_array;
    ModeDecisionCandidateBuffer **candidate_buffer_ptr_array =
    ...
    for (fullLoopCandidateIndex = 0;
         fullLoopCandidateIndex < fullCandidateTotalCount;
         ++fullLoopCandidateIndex) {
        candidateIndex =
            (context_ptr->full_loop_escape == 2)
                ? context_ptr
                      ->sorted_candidate_index_array[fullLoopCandidateIndex]
                : context_ptr
                      ->best_candidate_index_array[fullLoopCandidateIndex];
        candidate_buffer = candidate_buffer_ptr_array[candidateIndex];
        candidate_ptr = candidate_buffer->candidate_ptr;
        ...
        context_ptr->md_staging_skip_full_pred =
...
#else
        context_ptr->md_staging_skip_full_pred =
            (context_ptr->md_staging_mode == MD_STAGING_MODE_3) ? EB_FALSE
                                                                : EB_TRUE;
#endif
...
#else
        context_ptr->md_staging_tx_search =
            candidate_ptr->cand_class == CAND_CLASS_0 ? 2 : 1;
#endif
...
void move_cu_data(
#if PAL_SUP
    PictureControlSet *pcs, EncDecContext *context_ptr,
#endif
    CodingUnit *src_cu, CodingUnit *dst_cu) {
#if PAL_SUP
    memcpy(&dst_cu->palette_info.pmi,
           &src_cu->palette_info.pmi,
           sizeof(PaletteModeInfo));
    if (svt_av1_allow_palette(pcs->parent_pcs_ptr->palette_mode,
                              context_ptr->blk_geom->bsize)) {
        dst_cu->palette_info.color_idx_map =
            (uint8_t *)malloc(MAX_PALETTE_SQUARE);
        assert(dst_cu->palette_info.color_idx_map != NULL &&
               "palette:Not-Enough-Memory");
    ...
    dst_cu->interintra_wedge_index =
        src_cu->interintra_wedge_index;             // inter_intra wedge index
    dst_cu->ii_wedge_sign = src_cu->ii_wedge_sign;  // inter_intra wedge sign=-1
#endif
    // CHKN TransformUnit transform_unit_array[TRANSFORM_UNIT_MAX_COUNT]; //
    // 2-bytes * 21 = 42-bytes
    memcpy(dst_cu->transform_unit_array,
           src_cu->transform_unit_array,
           TRANSFORM_UNIT_MAX_COUNT * sizeof(TransformUnit));

    // CHKN PredictionUnit prediction_unit_array[MAX_NUM_OF_PU_PER_CU];    //
    // 35-bytes * 4 = 140 bytes
    ...
    // CHKN
    // CHKN     signed                         delta_qp : 8; // can be signed
    // 8bits CHKN     signed                         org_delta_qp : 8; CHKN
    // #endif CHKN CHKN #if ADD_DELTA_QP_SUPPORT CHKN     uint16_t qp; CHKN
    // uint16_t                       ref_qp; CHKN CHKN     int16_t delta_qp; //
    // can be signed 8bits CHKN     int16_t org_delta_qp; CHKN #endif
    ...
    memcpy(dst_cu->inter_mode_ctx,
           src_cu->inter_mode_ctx,
           MODE_CTX_REF_FRAMES * sizeof(int16_t));

    // CHKN IntMv ref_mvs[MODE_CTX_REF_FRAMES][MAX_MV_REF_CANDIDATES]; //used
    // only for nonCompound modes.
    ...
    dst_cu->compoud_reference_type_context =
        src_cu->compoud_reference_type_context;
    dst_cu->segment_id = src_cu->segment_id;

    memcpy(dst_cu->quantized_dc,
           src_cu->quantized_dc,
           3 * MAX_TXB_COUNT * sizeof(int32_t));
...
void move_cu_data_redund(
#if PAL_SUP
    PictureControlSet *pcs, ModeDecisionContext *context_ptr,
#endif
    CodingUnit *src_cu, CodingUnit *dst_cu) {
#if PAL_SUP
    dst_cu->segment_id = src_cu->segment_id;
    ...
    dst_cu->filter_intra_mode = src_cu->filter_intra_mode;
#endif
    // CHKN TransformUnit_t transform_unit_array[TRANSFORM_UNIT_MAX_COUNT]; //
    // 2-bytes * 21 = 42-bytes
    memcpy(dst_cu->transform_unit_array,
           src_cu->transform_unit_array,
           TRANSFORM_UNIT_MAX_COUNT * sizeof(TransformUnit));

    // CHKN PredictionUnit_t prediction_unit_array[MAX_NUM_OF_PU_PER_CU];    //
    // 35-bytes * 4 = 140 bytes
    memcpy(dst_cu->prediction_unit_array,
           src_cu->prediction_unit_array,
           MAX_NUM_OF_PU_PER_CU * sizeof(PredictionUnit));
...
void check_redundant_block(const BlockGeom *blk_geom,
                           ModeDecisionContext *context_ptr,
                           uint8_t *redundant_blk_avail,
                           uint16_t *redundant_blk_mds) {
    if (blk_geom->redund) {
        for (int it = 0; it < blk_geom->redund_list.list_size; it++) {
            if (context_ptr
                    ->md_local_cu_unit[blk_geom->redund_list.blk_mds_table[it]]
                    .avail_blk_flag) {
...
#endif
    EbBool is_nsq_table_used, uint8_t nsq_max_shapes_md,
        ...
        if (!mdc_depth_level) {
            ...
            if (context_ptr->blk_geom->shape != PART_N) {
                ...
                for (int i = 0; i < nsq_max_shapes_md; i++) {
                    if (context_ptr->blk_geom->shape ==
                        context_ptr->nsq_table[i])
                        ret = 1;
                }
            }
        } else {
...
void init_candidate_buffer(
    ...
    memset(
        count_non_zero_coeffs[0], 0, sizeof(uint32_t) * MAX_NUM_OF_TU_PER_CU);

    candidate_ptr->chroma_distortion = 0;
    candidate_ptr->chroma_distortion_inter_depth = 0;
    memset(candidate_ptr->eob[1], 0, sizeof(uint16_t) * MAX_TXB_COUNT);
    memset(
        count_non_zero_coeffs[1], 0, sizeof(uint32_t) * MAX_NUM_OF_TU_PER_CU);
    memset(candidate_ptr->eob[2], 0, sizeof(uint16_t) * MAX_TXB_COUNT);
    memset(
        count_non_zero_coeffs[2], 0, sizeof(uint32_t) * MAX_NUM_OF_TU_PER_CU);
}
#if !MULTI_PASS_PD
void inter_depth_tx_search(PictureControlSet *picture_control_set_ptr,
                           ModeDecisionCandidateBuffer *candidate_buffer,
                           CodingUnit *cu_ptr, ModeDecisionContext *context_ptr,
                           EbPictureBufferDesc *input_picture_ptr,
                           uint64_t ref_fast_cost) {
    // Hsan: if Transform Search ON and INTRA, then Tx Type search is performed
    // @ the full loop
    uint8_t tx_search_skip_flag =
        (picture_control_set_ptr->parent_pcs_ptr->tx_search_level ==
             TX_SEARCH_INTER_DEPTH &&
         (picture_control_set_ptr->parent_pcs_ptr->atb_mode == 0 ||
          candidate_buffer->candidate_ptr->type == INTER_MODE))
            ? get_skip_tx_search_flag(
                  context_ptr->blk_geom->sq_size,
                  ref_fast_cost,
    ...
    if (!tx_search_skip_flag) {
        ...
        init_candidate_buffer(candidate_ptr, count_non_zero_coeffs);
        ...
        context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost =
            (context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost -
             candidate_buffer->candidate_ptr->chroma_distortion) +
            candidate_buffer->candidate_ptr->chroma_distortion_inter_depth;

        if (candidate_ptr->type == INTRA_MODE)
            context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost_luma =
                candidate_buffer->full_cost_luma;

        context_ptr->md_ep_pipe_sb[cu_ptr->mds_idx].merge_cost =
            *candidate_buffer->full_cost_merge_ptr;
        context_ptr->md_ep_pipe_sb[cu_ptr->mds_idx].skip_cost =
            *candidate_buffer->full_cost_skip_ptr;

        if (candidate_ptr->type == INTER_MODE &&
            candidate_ptr->merge_flag == EB_TRUE)
            context_ptr->md_ep_pipe_sb[cu_ptr->leaf_index].chroma_distortion =
                candidate_buffer->candidate_ptr->chroma_distortion;
        context_ptr->md_local_cu_unit[cu_ptr->mds_idx].full_distortion =
            candidate_buffer->candidate_ptr->full_distortion;

        context_ptr->md_local_cu_unit[cu_ptr->mds_idx].chroma_distortion =
            (uint32_t)candidate_buffer->candidate_ptr->chroma_distortion;
        ...
        tuTotalCount =
            context_ptr->blk_geom
                ->txb_count[candidate_buffer->candidate_ptr->tx_depth];
        tu_index = 0;
        txb_itr = 0;
        ...
        do {
            ...
            txb_ptr->u_has_coeff = (EbBool)(
                ((candidate_ptr->u_has_coeff) & (1 << (tu_index))) > 0);
            txb_ptr->v_has_coeff = (EbBool)(
                ((candidate_ptr->v_has_coeff) & (1 << (tu_index))) > 0);
            ...
            // copy coeff
            {
                uint32_t bwidth = context_ptr->blk_geom->tx_width[txb_itr] < 64
                                      ...
                                      : 32;
                uint32_t bheight =
                    context_ptr->blk_geom->tx_height[txb_itr] < 64
 ...
 * left partitions
 ****************************************************/
PART get_partition_shape(PartitionContextType above, PartitionContextType left,
 ...
 ****************************************************/
void adjust_nsq_rank(PictureControlSet *picture_control_set_ptr,
                     ModeDecisionContext *context_ptr,
                     const SequenceControlSet *sequence_control_set_ptr,
                     SuperBlock *sb_ptr,
                     NeighborArrayUnit *leaf_partition_neighbor_array) {
    ...
    me_2Nx2N_table_offset = (context_ptr->blk_geom->bwidth == 4 ||
    ...
    uint32_t partition_left_neighbor_index = get_neighbor_array_unit_left_index(
        leaf_partition_neighbor_array, context_ptr->cu_origin_y);
    uint32_t partition_above_neighbor_index = get_neighbor_array_unit_top_index(
        leaf_partition_neighbor_array, context_ptr->cu_origin_x);
    const PartitionContextType above_ctx =
        (((PartitionContext *)leaf_partition_neighbor_array
              ->top_array)[partition_above_neighbor_index]
    ...
    const PartitionContextType left_ctx =
        (((PartitionContext *)leaf_partition_neighbor_array
              ->left_array)[partition_left_neighbor_index]
    ...
    PART neighbor_part = get_partition_shape(above_ctx,
                                             left_ctx,
                                             context_ptr->blk_geom->bwidth,
                                             context_ptr->blk_geom->bheight);
    ...
    if (neighbor_part == PART_S && me_part_0 == PART_S && me_part_1 == PART_S) {
        ...
        if (neighbor_part != PART_N && neighbor_part != PART_S &&
            neighbor_part != me_part_0 && neighbor_part != me_part_1) {
            ...
            context_ptr->nsq_table[5] =
                neighbor_part != PART_N && neighbor_part != PART_S
                    ? neighbor_part
                    : me_part_0;
    }
#if MDC_ADAPTIVE_LEVEL
    if (picture_control_set_ptr->parent_pcs_ptr
            ->enable_adaptive_ol_partitioning) {
#else
        if (picture_control_set_ptr->parent_pcs_ptr->mdc_depth_level <
            MAX_MDC_LEVEL) {
#endif
        context_ptr->nsq_table[2] =
            context_ptr->nsq_table[0] != ol_part1 &&
                    context_ptr->nsq_table[1] != ol_part1
                ? ol_part1
                : context_ptr->nsq_table[0] != ol_part2 &&
                          context_ptr->nsq_table[1] != ol_part2
                      ? ol_part2
                      : ol_part3 != PART_N ? ol_part3
                                           : context_ptr->nsq_table[2];
        context_ptr->nsq_table[3] =
            context_ptr->nsq_table[0] != ol_part1 &&
                    context_ptr->nsq_table[1] != ol_part1 &&
                    context_ptr->nsq_table[2] != ol_part1
                ? ol_part1
                : context_ptr->nsq_table[0] != ol_part2 &&
                          context_ptr->nsq_table[1] != ol_part2 &&
                          context_ptr->nsq_table[2] != ol_part2
                      ? ol_part2
                      : context_ptr->nsq_table[0] != ol_part3 &&
                                context_ptr->nsq_table[1] != ol_part3 &&
                                context_ptr->nsq_table[2] != ol_part3
                            ? ol_part3
                            : ol_part4 != PART_N ? ol_part4
                                                 : context_ptr->nsq_table[3];
        context_ptr->nsq_table[4] =
            context_ptr->nsq_table[0] != ol_part1 &&
                    context_ptr->nsq_table[1] != ol_part1 &&
                    context_ptr->nsq_table[2] != ol_part1 &&
                    context_ptr->nsq_table[3] != ol_part1
                ? ol_part1
                : context_ptr->nsq_table[0] != ol_part2 &&
                          context_ptr->nsq_table[1] != ol_part2 &&
                          context_ptr->nsq_table[2] != ol_part2 &&
                          context_ptr->nsq_table[3] != ol_part2
                      ? ol_part2
                      : context_ptr->nsq_table[0] != ol_part3 &&
                                context_ptr->nsq_table[1] != ol_part3 &&
                                context_ptr->nsq_table[2] != ol_part3 &&
                                context_ptr->nsq_table[3] != ol_part3
                            ? ol_part3
                            : context_ptr->nsq_table[0] != ol_part4 &&
                                      context_ptr->nsq_table[1] != ol_part4 &&
                                      context_ptr->nsq_table[2] != ol_part4 &&
                                      context_ptr->nsq_table[3] != ol_part4
                                  ? ol_part4
                                  : ol_part5 != PART_N
                                        ? ol_part5
                                        : context_ptr->nsq_table[4];
        context_ptr->nsq_table[5] =
            context_ptr->nsq_table[0] != ol_part1 &&
                    ...
                    context_ptr->nsq_table[2] != ol_part1 &&
                    context_ptr->nsq_table[3] != ol_part1 &&
                    context_ptr->nsq_table[4] != ol_part1
                ? ol_part1
                : context_ptr->nsq_table[0] != ol_part2 &&
                          context_ptr->nsq_table[1] != ol_part2 &&
                          context_ptr->nsq_table[2] != ol_part2 &&
                          context_ptr->nsq_table[3] != ol_part2 &&
                          context_ptr->nsq_table[4] != ol_part2
                      ? ol_part2
                      : context_ptr->nsq_table[0] != ol_part3 &&
                                context_ptr->nsq_table[1] != ol_part3 &&
                                context_ptr->nsq_table[2] != ol_part3 &&
                                context_ptr->nsq_table[3] != ol_part3 &&
                                context_ptr->nsq_table[4] != ol_part3
                            ? ol_part3
                            : context_ptr->nsq_table[0] != ol_part4 &&
                                      context_ptr->nsq_table[1] != ol_part4 &&
                                      context_ptr->nsq_table[2] != ol_part4 &&
                                      context_ptr->nsq_table[3] != ol_part4 &&
                                      context_ptr->nsq_table[4] != ol_part4
                                  ? ol_part4
                                  : context_ptr->nsq_table[0] != ol_part5 &&
                                            context_ptr->nsq_table[1] !=
                                                ol_part5 &&
                                            context_ptr->nsq_table[2] !=
                                                ol_part5 &&
                                            context_ptr->nsq_table[3] !=
                                        ...
                                        : ol_part6 != PART_N
                                              ? ol_part6
                                              : context_ptr->nsq_table[5];
        context_ptr->nsq_table[6] =
            context_ptr->nsq_table[0] != ol_part1 &&
                    context_ptr->nsq_table[1] != ol_part1 &&
                    context_ptr->nsq_table[2] != ol_part1 &&
                    context_ptr->nsq_table[3] != ol_part1 &&
                    context_ptr->nsq_table[4] != ol_part1 &&
                    context_ptr->nsq_table[5] != ol_part1
                ? ol_part1
                : context_ptr->nsq_table[0] != ol_part2 &&
                          context_ptr->nsq_table[1] != ol_part2 &&
                          context_ptr->nsq_table[2] != ol_part2 &&
                          context_ptr->nsq_table[3] != ol_part2 &&
                          context_ptr->nsq_table[4] != ol_part2 &&
                          context_ptr->nsq_table[5] != ol_part2
                      ? ol_part2
                      : context_ptr->nsq_table[0] != ol_part3 &&
                                context_ptr->nsq_table[1] != ol_part3 &&
                                context_ptr->nsq_table[2] != ol_part3 &&
                                context_ptr->nsq_table[3] != ol_part3 &&
                                context_ptr->nsq_table[4] != ol_part3 &&
                                context_ptr->nsq_table[5] != ol_part3
                            ? ol_part3
                            : context_ptr->nsq_table[0] != ol_part4 &&
                                      context_ptr->nsq_table[1] != ol_part4 &&
                                      context_ptr->nsq_table[2] != ol_part4 &&
                                      context_ptr->nsq_table[3] != ol_part4 &&
                                      context_ptr->nsq_table[4] != ol_part4 &&
                                      context_ptr->nsq_table[5] != ol_part4
                                  ? ol_part4
                                  : context_ptr->nsq_table[0] != ol_part5 &&
                                            context_ptr->nsq_table[1] !=
                                                ol_part5 &&
 ...
 * probable Shape to be selected in the lowest index
 ****************************************************/
void order_nsq_table(PictureControlSet *picture_control_set_ptr,
                     ModeDecisionContext *context_ptr,
                     const SequenceControlSet *sequence_control_set_ptr,
                     SuperBlock *sb_ptr,
                     NeighborArrayUnit *leaf_partition_neighbor_array) {
    ...
    EbBool isCompoundEnabled =
        (frm_hdr->reference_mode == SINGLE_REFERENCE) ? 0 : 1;
    uint32_t me_sb_addr;
...
uint8_t check_skip_sub_blks(PictureControlSet *picture_control_set_ptr,
                            ModeDecisionContext *context_ptr,
                            CodingUnit *cu_ptr, uint8_t is_complete_sb,
                            uint32_t sb_index) {
    uint8_t skip_sub_blocks = 0;
    if (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode ==
            PIC_OPEN_LOOP_DEPTH_MODE ||
        (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode ==
             PIC_SB_SWITCH_DEPTH_MODE &&
         picture_control_set_ptr->parent_pcs_ptr
                 ->sb_depth_mode_array[sb_index] >= SB_OPEN_LOOP_DEPTH_MODE))
        if (is_complete_sb)
            if ((context_ptr->md_local_cu_unit[cu_ptr->mds_idx]
                     .top_neighbor_depth == context_ptr->blk_geom->bsize) &&
                (context_ptr->md_local_cu_unit[cu_ptr->mds_idx]
                     .left_neighbor_depth == context_ptr->blk_geom->bsize)) {
                skip_sub_blocks = 1;
                context_ptr->md_cu_arr_nsq[context_ptr->blk_geom->sqi_mds]
                    .split_flag = 0;
            }
...
#if ENHANCED_M0_SETTINGS
void search_best_independent_uv_mode(PictureControlSet *picture_control_set_ptr,
                                     EbPictureBufferDesc *input_picture_ptr,
    ...
    for (uv_mode = UV_DC_PRED; uv_mode <= UV_PAETH_PRED; uv_mode++) {
        uint8_t uv_angleDeltaCandidateCount =
            (use_angle_delta &&
        ...
        for (uint8_t uv_angleDeltaCounter = 0;
             uv_angleDeltaCounter < uv_angleDeltaCandidateCount;
             ++uv_angleDeltaCounter) {
            int32_t uv_angle_delta =
                CLIP(uv_angle_delta_shift *
                         (uv_angleDeltaCandidateCount == 1
                              ? 0
                              : uv_angleDeltaCounter -
                                    (uv_angleDeltaCandidateCount >> 1)),
                     -MAX_ANGLE_DELTA,
            ...
            candidate_array[uv_mode_total_count]
            ...
            candidate_array[uv_mode_total_count]
                .palette_info.pmi.palette_size[1] = 0;
            candidate_array[uv_mode_total_count].filter_intra_mode =
                FILTER_INTRA_MODES;
            ...
            candidate_array[uv_mode_total_count].motion_mode =
                SIMPLE_TRANSLATION;
    ...
    for (uint8_t uv_mode_count = 0; uv_mode_count < uv_mode_total_count;
         uv_mode_count++) {
        ModeDecisionCandidateBuffer *candidate_buffer =
            context_ptr->candidate_buffer_ptr_array[uv_mode_count];
        candidate_buffer->candidate_ptr =
            &context_ptr->fast_candidate_array[uv_mode_count];
    ...
    sort_fast_candidates(
        context_ptr,
        0,
        uv_mode_total_count,  // how many cand buffers to sort. one of the
                              // buffers can have max cost.
    ...
    for (uint8_t uv_mode_count = 0;
        ...
        ModeDecisionCandidateBuffer *candidate_buffer =
            context_ptr->candidate_buffer_ptr_array
                [uv_cand_buff_indices[uv_mode_count]];
        candidate_buffer->candidate_ptr =
            &context_ptr
        ...
        coeff_rate[candidate_buffer->candidate_ptr->intra_chroma_mode]
                  [MAX_ANGLE_DELTA + candidate_buffer->candidate_ptr
                                         ->angle_delta[PLANE_TYPE_UV]] =
                      (int)(cb_coeff_bits + cr_coeff_bits);
        distortion[candidate_buffer->candidate_ptr->intra_chroma_mode]
                  [MAX_ANGLE_DELTA + candidate_buffer->candidate_ptr
                                         ->angle_delta[PLANE_TYPE_UV]] =
                      (int)(cbFullDistortion[DIST_CALC_RESIDUAL] +
                            crFullDistortion[DIST_CALC_RESIDUAL]);
    }

    // Loop over all intra mode, then over all uv move to derive the best uv
    // mode for a given intra mode in term of rate

    // intra_mode loop (luma mode loop)
    for (uint8_t intra_mode = DC_PRED; intra_mode <= PAETH_PRED; ++intra_mode) {
        uint8_t angleDeltaCandidateCount =
            (use_angle_delta &&
        ...
        for (uint8_t angleDeltaCounter = 0;
            ...
            int32_t angle_delta = CLIP(
                angle_delta_shift *
                    (angleDeltaCandidateCount == 1
                         ? 0
                         : angleDeltaCounter - (angleDeltaCandidateCount >> 1)),
                -MAX_ANGLE_DELTA,
                MAX_ANGLE_DELTA);

            // uv mode loop
            context_ptr
                ->best_uv_cost[intra_mode][MAX_ANGLE_DELTA + angle_delta] =
                (uint64_t)~0;
            ...
            for (uint8_t uv_mode_count = 0;
                ...
                if (uv_cost <
                    context_ptr->best_uv_cost[intra_mode]
                                             [MAX_ANGLE_DELTA + angle_delta]) {
                    context_ptr->best_uv_mode[intra_mode]
                                             [MAX_ANGLE_DELTA + angle_delta] =
                        candidate_ptr->intra_chroma_mode;
                    context_ptr->best_uv_angle[intra_mode]
                                              [MAX_ANGLE_DELTA + angle_delta] =
                        candidate_ptr->angle_delta[PLANE_TYPE_UV];
                    ...
                    context_ptr->best_uv_cost[intra_mode]
                                             [MAX_ANGLE_DELTA + angle_delta] =
                        uv_cost;
                    context_ptr->fast_luma_rate[intra_mode]
                                               [MAX_ANGLE_DELTA + angle_delta] =
                        candidate_ptr->fast_luma_rate;
                    context_ptr->fast_chroma_rate[intra_mode][MAX_ANGLE_DELTA +
                                                              angle_delta] =
                        candidate_ptr->fast_chroma_rate;
                }
            }
...
void search_best_independent_uv_mode(PictureControlSet *picture_control_set_ptr,
    ...
    // Use the 1st spot of the candidate buffer to hold cfl settings to use same
    // kernel as MD for coef cost estimation
    ModeDecisionCandidateBuffer *candidate_buffer =
        &(context_ptr->candidate_buffer_ptr_array[0][0]);
    candidate_buffer->candidate_ptr = &(context_ptr->fast_candidate_array[0]);
    candidate_buffer->candidate_ptr->type = INTRA_MODE;
    ...
    for (uv_mode = uv_mode_start; uv_mode <= uv_mode_end; uv_mode++) {
        ...
        for (uint8_t uv_angleDeltaCounter = 0;
            ...
            coeff_rate[uv_mode][MAX_ANGLE_DELTA + uv_angle_delta] =
                (int)(cb_coeff_bits + cr_coeff_bits);
            distortion[uv_mode][MAX_ANGLE_DELTA + uv_angle_delta] =
                (int)(cbFullDistortion[DIST_CALC_RESIDUAL] +
                      crFullDistortion[DIST_CALC_RESIDUAL]);
        }
    }
    ...
    // Loop over all intra mode, then over all uv move to derive the best uv
    // mode for a given intra mode in term of rate
    for (uint8_t intra_mode = intra_mode_start; intra_mode <= intra_mode_end;
         ++intra_mode) {
        uint8_t angleDeltaCandidateCount =
            (use_angle_delta &&
             av1_is_directional_mode((PredictionMode)intra_mode))
        ...
        for (uint8_t angleDeltaCounter = 0;
            ...
            candidate_buffer->candidate_ptr->type = INTRA_MODE;
#if PAL_SUP
            candidate_buffer->candidate_ptr->palette_info.pmi.palette_size[0] =
                0;
            candidate_buffer->candidate_ptr->palette_info.pmi.palette_size[1] =
                0;
#endif
            candidate_buffer->candidate_ptr->intra_luma_mode = intra_mode;
            candidate_buffer->candidate_ptr->distortion_ready = 0;
            candidate_buffer->candidate_ptr->use_intrabc = 0;
            candidate_buffer->candidate_ptr->is_directional_mode_flag =
            ...
            // int32_t  p_angle =
            // mode_to_angle_map[(PredictionMode)openLoopIntraCandidate] +
            // angle_delta * ANGLE_STEP; if (!disable_z2_prediction || (p_angle
            // <= 90 || p_angle >= 180)) {
            // uv mode loop
            context_ptr
                ->best_uv_cost[intra_mode][MAX_ANGLE_DELTA + angle_delta] =
            ...
            for (uv_mode = uv_mode_start; uv_mode <= uv_mode_end; uv_mode++) {
                ...
                for (uint8_t uv_angleDeltaCounter = 0;
                    ...
                    candidate_buffer->candidate_ptr
                        ->angle_delta[PLANE_TYPE_UV] = uv_angle_delta;

                    candidate_buffer->candidate_ptr->transform_type_uv =
                        av1_get_tx_type(context_ptr->blk_geom->bsize,
                                        0,
                                        (PredictionMode)candidate_buffer
                                            ->candidate_ptr->intra_luma_mode,
                                        (UvPredictionMode)candidate_buffer
                                            ->candidate_ptr->intra_chroma_mode,
                                        PLANE_TYPE_UV,
                                        0,
                                        0,
                    ...
                    // Fast Cost
                    *(candidate_buffer->fast_cost_ptr) =
                        Av1ProductFastCostFuncTable[candidate_buffer
                                                        ->candidate_ptr->type](
                            context_ptr->cu_ptr,
                            candidate_buffer->candidate_ptr,
                            context_ptr->qp,
                            ...
                            &(context_ptr
                                  ->md_local_cu_unit[context_ptr->blk_geom
                                                         ->blkidx_mds]
                                  .ed_ref_mv_stack[candidate_buffer
                                                       ->candidate_ptr
                                                       ->ref_frame_type][0]),
                            context_ptr->blk_geom,
                            context_ptr->cu_origin_y >> MI_SIZE_LOG2,
                    ...
                    if (uv_cost <
                        context_ptr->best_uv_cost[intra_mode][MAX_ANGLE_DELTA +
                                                              angle_delta]) {
                        context_ptr->best_uv_mode[intra_mode][MAX_ANGLE_DELTA +
                                                              angle_delta] =
                            uv_mode;
                        context_ptr->best_uv_angle[intra_mode][MAX_ANGLE_DELTA +
                                                               angle_delta] =
                            uv_angle_delta;
                        ...
                        context_ptr->best_uv_cost[intra_mode][MAX_ANGLE_DELTA +
                            ...
                            uv_cost;
                        context_ptr
                            ->fast_luma_rate[intra_mode]
                                            [MAX_ANGLE_DELTA + angle_delta] =
                            candidate_buffer->candidate_ptr->fast_luma_rate;
                        context_ptr
                            ->fast_chroma_rate[intra_mode]
                                              [MAX_ANGLE_DELTA + angle_delta] =
                            candidate_buffer->candidate_ptr->fast_chroma_rate;
                    }
                }
...
#if SPEED_OPT
#if !REMOVE_MD_STAGE_1
void inter_class_decision_count_1(struct ModeDecisionContext *context_ptr) {
    ModeDecisionCandidateBuffer **buffer_ptr_array =
        context_ptr->candidate_buffer_ptr_array;
    // Distortion-based NIC pruning not applied to INTRA clases: CLASS_0 and
    // CLASS
    for (CAND_CLASS cand_class_it = CAND_CLASS_1; cand_class_it <= CAND_CLASS_3;
         cand_class_it++) {
        if (context_ptr->md_stage_0_count[cand_class_it] > 0 &&
            ...
            if (*(buffer_ptr_array[cand_buff_indices[0]]->fast_cost_ptr) <
                *(buffer_ptr_array[context_ptr
                                       ->cand_buff_indices[CAND_CLASS_0][0]]
                      ->fast_cost_ptr)) {
                uint32_t fast1_cand_count = 1;
                while (
                    fast1_cand_count <
                        context_ptr->md_stage_1_count[cand_class_it] &&
                    ((((*(buffer_ptr_array[cand_buff_indices[fast1_cand_count]]
                              ->fast_cost_ptr) -
                        *(buffer_ptr_array[cand_buff_indices[0]]
                              ->fast_cost_ptr)) *
                       100) /
                      (*(buffer_ptr_array[cand_buff_indices[0]]
                             ->fast_cost_ptr))) <
                     context_ptr->dist_base_md_stage_0_count_th)) {
                    fast1_cand_count++;
                }
...
unsigned int eb_av1_get_sby_perpixel_variance(
    const aom_variance_fn_ptr_t *fn_ptr, const uint8_t *src, int stride,
    BlockSize bs);
#endif

#if INTER_INTRA_CLASS_PRUNING

void interintra_class_pruning_1(ModeDecisionContext *context_ptr,
                                uint64_t best_md_stage_cost) {
    for (CAND_CLASS cand_class_it = CAND_CLASS_0;
         cand_class_it < CAND_CLASS_TOTAL;
         cand_class_it++) {
#if MULTI_PASS_PD
        if (context_ptr->md_stage_1_cand_prune_th != (uint64_t)~0 ||
            ...
            if (context_ptr->md_stage_0_count[cand_class_it] > 0 &&
                ...
                uint64_t class_best_cost =
                    *(context_ptr
                          ->candidate_buffer_ptr_array[cand_buff_indices[0]]
                          ->fast_cost_ptr);

                // inter class pruning
#if ENHANCED_M0_SETTINGS
                if (best_md_stage_cost && class_best_cost &&
                    ((((class_best_cost - best_md_stage_cost) * 100) /
                      best_md_stage_cost) >
                     context_ptr->md_stage_1_class_prune_th)) {
#else
                if ((((class_best_cost - best_md_stage_cost) * 100) /
                     best_md_stage_cost) >
                    context_ptr->md_stage_1_class_prune_th) {
#endif
                    context_ptr->md_stage_1_count[cand_class_it] = 0;
        ...
        context_ptr->md_stage_1_total_count +=
            context_ptr->md_stage_1_count[cand_class_it];
    }
}

void interintra_class_pruning_2(ModeDecisionContext *context_ptr,
                                uint64_t best_md_stage_cost) {
    for (CAND_CLASS cand_class_it = CAND_CLASS_0;
         cand_class_it < CAND_CLASS_TOTAL;
         cand_class_it++) {
...
void md_encode_block(SequenceControlSet *sequence_control_set_ptr,
                     PictureControlSet *picture_control_set_ptr,
                     ModeDecisionContext *context_ptr,
                     EbPictureBufferDesc *input_picture_ptr,
                     uint8_t *skip_sub_blocks, uint32_t lcuAddr,
    ...
    const uint32_t inputOriginIndex =
        (context_ptr->cu_origin_y + input_picture_ptr->origin_y) *
            input_picture_ptr->stride_y +
        (context_ptr->cu_origin_x + input_picture_ptr->origin_x);

    const uint32_t inputCbOriginIndex = ((context_ptr->round_origin_y >> 1) +
                                         (input_picture_ptr->origin_y >> 1)) *
                                            input_picture_ptr->stride_cb +
                                        ((context_ptr->round_origin_x >> 1) +
                                         (input_picture_ptr->origin_x >> 1));
    const uint32_t cuOriginIndex =
        blk_geom->origin_x + blk_geom->origin_y * SB_STRIDE_Y;
    const uint32_t cuChromaOriginIndex =
        ROUND_UV(blk_geom->origin_x) / 2 +
    ...
    if (is_nsq_table_used) {
        if (context_ptr->blk_geom->shape == PART_N) {
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->mdc_depth_level <
                MAX_MDC_LEVEL) {
#endif
                adjust_nsq_rank(picture_control_set_ptr,
                                context_ptr,
                                sequence_control_set_ptr,
                                context_ptr->sb_ptr,
                                context_ptr->leaf_partition_neighbor_array);
            } else {
                order_nsq_table(picture_control_set_ptr,
                                context_ptr,
                                sequence_control_set_ptr,
                                context_ptr->sb_ptr,
                                context_ptr->leaf_partition_neighbor_array);
    ...
    uint8_t is_complete_sb =
        sequence_control_set_ptr->sb_geom[lcuAddr].is_complete_sb;
    ...
    if (allowed_ns_cu(
        ...
        const aom_variance_fn_ptr_t *fn_ptr =
            &mefn_ptr[context_ptr->blk_geom->bsize];
        ...
        cu_ptr->av1xd->tile.mi_col_start =
            context_ptr->sb_ptr->tile_info.mi_col_start;
        cu_ptr->av1xd->tile.mi_col_end =
            context_ptr->sb_ptr->tile_info.mi_col_end;
        cu_ptr->av1xd->tile.mi_row_start =
            context_ptr->sb_ptr->tile_info.mi_row_start;
        cu_ptr->av1xd->tile.mi_row_end =
            context_ptr->sb_ptr->tile_info.mi_row_end;
        ...
        ProductCodingLoopInitFastLoop(
            ...
            context_ptr->leaf_depth_neighbor_array,
            context_ptr->leaf_partition_neighbor_array);
        // Skip sub blocks if the current block has the same depth as the left
        // block and above block
        if (picture_control_set_ptr->parent_pcs_ptr->skip_sub_blks)
        ...
        if (context_ptr->chroma_level == CHROMA_MODE_0) {
            if (context_ptr->blk_geom->sq_size < 128) {
                if (context_ptr->blk_geom->has_uv) {
#if ENHANCED_M0_SETTINGS
                    search_best_independent_uv_mode(picture_control_set_ptr,
                                                    input_picture_ptr,
                                                    inputCbOriginIndex,
                                                    inputCbOriginIndex,
                                                    cuChromaOriginIndex,
        ...
        context_ptr->me_block_offset =
            (context_ptr->blk_geom->bwidth == 4 ||
             ...
             context_ptr->blk_geom->bheight == 128)
                ? 0
                : get_me_info_index(picture_control_set_ptr->parent_pcs_ptr
                                        ->max_number_of_pus_per_sb,
                                    context_ptr->blk_geom,
        ...
        if (!context_ptr->md_skip_mvp_generation) {
#endif
            if (frm_hdr->allow_intrabc)  // picture_control_set_ptr->slice_type
                                         // == I_SLICE
            ...
            mvp_bypass_init(picture_control_set_ptr, context_ptr);
        }
#endif

        // Perform ME search around the best MVP
        if (context_ptr->predictive_me_level)
            predictive_me_search(picture_control_set_ptr,
                                 context_ptr,
                                 input_picture_ptr,
                                 inputOriginIndex,
                                 cuOriginIndex);
...
#if II_COMP_FLAG
        // for every CU, perform Luma DC/V/H/S intra prediction to be used later
        // in inter-intra search

        int allow_ii =
            is_interintra_allowed_bsize(context_ptr->blk_geom->bsize);
#if MULTI_PASS_PD
        if (context_ptr->md_enable_inter_intra && allow_ii)
#else
            if (picture_control_set_ptr->parent_pcs_ptr->enable_inter_intra &&
                allow_ii)
#endif
            precompute_intra_pred_for_inter_intra(picture_control_set_ptr,
                                                  context_ptr);
#endif

        generate_md_stage_0_cand(context_ptr->sb_ptr,
                                 context_ptr,
                                 &fast_candidate_total_count,
                                 picture_control_set_ptr);

        // MD Stages
        // The first stage(old fast loop) and the last stage(old full loop)
        // should remain at their locations, new stages could be created between
        // those two. a bypass mechanism should be added to skip one or all of
        // the intermediate stages, in a way to to be able to fall back to org
        // design (FastLoop->FullLoop)
        set_md_stage_counts(
            picture_control_set_ptr, context_ptr, fast_candidate_total_count);
...
#endif
        for (cand_class_it = CAND_CLASS_0; cand_class_it < CAND_CLASS_TOTAL;
             cand_class_it++) {
            // number of next level candidates could not exceed number of curr
            // level candidates
            context_ptr->md_stage_1_count[cand_class_it] =
            ...
            if (context_ptr->md_stage_0_count[cand_class_it] > 0 &&
                ...
                assert(buffer_total_count <= MAX_NFL_BUFF &&
                       "not enough cand buffers");

                // Input: md_stage_0_count[cand_class_it]  Output:
                // md_stage_1_count[cand_class_it]
                context_ptr->target_class = cand_class_it;

                md_stage_0(picture_control_set_ptr,
                           context_ptr,
                           ...
                           context_ptr->md_stage_0_count[cand_class_it] >
                               context_ptr->md_stage_1_count
                                   [cand_class_it],  // is there need to max the
                                                     // temp buffer
                           0);
                ...
                sort_stage0_fast_candidates(
                    ...
                    context_ptr,
                    buffer_start_idx,
                    buffer_count_for_curr_class,  // how many cand buffers to
                                                  // sort. one of the buffers can
                                                  // have max cost.
                    context_ptr->cand_buff_indices[cand_class_it]);
                    ...
                    if (context_ptr->md_stage_0_count[CAND_CLASS_0] > 0 &&
                        *(context_ptr
                        ...
                        while (
                            ...
                            ((((*(context_ptr
                                      ->candidate_buffer_ptr_array
                                          [cand_buff_indices[fast1_cand_count]]
                                      ->fast_cost_ptr) -
                                *(context_ptr
                                      ->candidate_buffer_ptr_array
                                          [cand_buff_indices[0]]
                ...
                buffer_start_idx +=
                    buffer_count_for_curr_class;  // for next iteration.
            }
...
#if !REMOVE_MD_STAGE_1
#if SPEED_OPT
        // after completing stage0, we might shorten cand count for some
        // classes.
        ...
        for (cand_class_it = CAND_CLASS_0; cand_class_it < CAND_CLASS_TOTAL;
            ...
            if (context_ptr->bypass_stage1[cand_class_it] == 0 &&
                context_ptr->md_stage_1_count[cand_class_it] > 0 &&
                context_ptr->md_stage_2_count[cand_class_it] > 0) {
                // Input: md_stage_1_count[cand_class_it]  Output:
                // full_cand_count[cand_class_it]
                context_ptr->target_class = cand_class_it;
                md_stage_1(picture_control_set_ptr,
                           context_ptr,
                           candidate_buffer_ptr_array_base,
                           context_ptr->md_stage_1_count[cand_class_it],
                           input_picture_ptr,
...
#endif
        memset(context_ptr->best_candidate_index_array,
               0xFFFFFFFF,
               MAX_NFL_BUFF * sizeof(uint32_t));
        memset(context_ptr->sorted_candidate_index_array,
               0xFFFFFFFF,
               MAX_NFL * sizeof(uint32_t));
        ...
        for (cand_class_it = CAND_CLASS_0; cand_class_it < CAND_CLASS_TOTAL;
            ...
            if (context_ptr->bypass_stage2[cand_class_it] == EB_FALSE &&
                context_ptr->md_stage_2_count[cand_class_it] > 0 &&
                ...
                // Sort the candidates of the target class based on the 1st full
                // loop cost
        ...
        md_stage_3(
            ...
            context_ptr->md_stage_3_total_count,
#endif
            ref_fast_cost);  // fullCandidateTotalCount to number of buffers to
                             // process
...
#if !MULTI_PASS_PD
        if (picture_control_set_ptr->parent_pcs_ptr
                ->interpolation_search_level == IT_SEARCH_INTER_DEPTH) {
            if (candidate_buffer->candidate_ptr->type != INTRA_MODE &&
                candidate_buffer->candidate_ptr->motion_mode ==
                    SIMPLE_TRANSLATION) {
                context_ptr->md_staging_skip_interpolation_search = EB_FALSE;
                context_ptr->md_staging_skip_inter_chroma_pred = EB_FALSE;
                ProductPredictionFunTable[candidate_buffer->candidate_ptr
                                              ->type](
                    context_ptr->hbd_mode_decision,
                    context_ptr,
                    picture_control_set_ptr,
        ...
        inter_depth_tx_search(picture_control_set_ptr,
                              candidate_buffer,
                              cu_ptr,
                              context_ptr,
                              input_picture_ptr,
        ...
        if (context_ptr->blk_geom->shape == PART_N) {
            context_ptr->parent_sq_type[sq_index] =
                candidate_buffer->candidate_ptr->type;
            ...
            context_ptr->parent_sq_pred_mode[sq_index] =
                candidate_buffer->candidate_ptr->pred_mode;
        }

        AV1PerformInverseTransformRecon(picture_control_set_ptr,
                                        context_ptr,
                                        candidate_buffer,
                                        cu_ptr,
                                        context_ptr->blk_geom);
        ...
        if (!context_ptr->blk_geom->has_uv) {
            ...
            if (context_ptr->hbd_mode_decision) {
                for (uint32_t j = 0; j < context_ptr->blk_geom->bheight; ++j)
                    memcpy(context_ptr->cfl_temp_luma_recon16bit +
                ...
                for (uint32_t j = 0; j < context_ptr->blk_geom->bheight; ++j)
                    memcpy(&context_ptr
                                ->cfl_temp_luma_recon[rec_luma_offset +
        ...
        // copy neigh recon data in cu_ptr
        {
            ...
            uint32_t recCbOffset =
                ((((context_ptr->blk_geom->origin_x >> 3) << 3) +
                  ((context_ptr->blk_geom->origin_y >> 3) << 3) *
                      candidate_buffer->recon_ptr->stride_cb) >>
                 1);
            uint32_t recCrOffset =
                ((((context_ptr->blk_geom->origin_x >> 3) << 3) +
                  ((context_ptr->blk_geom->origin_y >> 3) << 3) *
                      candidate_buffer->recon_ptr->stride_cr) >>
                 1);
            ...
            if (!context_ptr->hbd_mode_decision) {
                memcpy(cu_ptr->neigh_top_recon[0],
                       recon_ptr->buffer_y + recLumaOffset +
                           (context_ptr->blk_geom->bheight - 1) *
                               recon_ptr->stride_y,
                       context_ptr->blk_geom->bwidth);
                if (context_ptr->blk_geom->has_uv &&
                    context_ptr->chroma_level <= CHROMA_MODE_1) {
                    memcpy(cu_ptr->neigh_top_recon[1],
                           recon_ptr->buffer_cb + recCbOffset +
                               (context_ptr->blk_geom->bheight_uv - 1) *
                                   recon_ptr->stride_cb,
                           context_ptr->blk_geom->bwidth_uv);
                    memcpy(cu_ptr->neigh_top_recon[2],
                           recon_ptr->buffer_cr + recCrOffset +
                               (context_ptr->blk_geom->bheight_uv - 1) *
                                   recon_ptr->stride_cr,
                           context_ptr->blk_geom->bwidth_uv);
                }
                ...
                if (context_ptr->blk_geom->has_uv &&
                    ...
                    for (j = 0; j < context_ptr->blk_geom->bheight_uv; ++j) {
                        cu_ptr->neigh_left_recon[1][j] =
                            recon_ptr
                                ->buffer_cb[recCbOffset +
                                            context_ptr->blk_geom->bwidth_uv -
                                            1 + j * recon_ptr->stride_cb];
                        cu_ptr->neigh_left_recon[2][j] =
                            recon_ptr
                                ->buffer_cr[recCrOffset +
                                            context_ptr->blk_geom->bwidth_uv -
                ...
                memcpy(cu_ptr->neigh_top_recon_16bit[0],
                       recon_ptr->buffer_y +
                           sz * (recLumaOffset +
                                 (context_ptr->blk_geom->bheight - 1) *
                                     recon_ptr->stride_y),
                ...
                if (context_ptr->blk_geom->has_uv &&
                    context_ptr->chroma_level <= CHROMA_MODE_1) {
                    memcpy(cu_ptr->neigh_top_recon_16bit[1],
                           recon_ptr->buffer_cb +
                               sz * (recCbOffset +
                                     (context_ptr->blk_geom->bheight_uv - 1) *
                                         recon_ptr->stride_cb),
                           sz * context_ptr->blk_geom->bwidth_uv);
                    memcpy(cu_ptr->neigh_top_recon_16bit[2],
                           recon_ptr->buffer_cr +
                               sz * (recCrOffset +
                                     (context_ptr->blk_geom->bheight_uv - 1) *
                                         recon_ptr->stride_cr),
                ...
                for (j = 0; j < context_ptr->blk_geom->bheight; ++j)
                    cu_ptr->neigh_left_recon_16bit[0][j] =
                        ((uint16_t *)recon_ptr
                             ->buffer_y)[recLumaOffset +
                                         context_ptr->blk_geom->bwidth - 1 +
                                         j * recon_ptr->stride_y];
                ...
                if (context_ptr->blk_geom->has_uv &&
                    context_ptr->chroma_level <= CHROMA_MODE_1) {
                    for (j = 0; j < context_ptr->blk_geom->bheight_uv; ++j) {
                        cu_ptr->neigh_left_recon_16bit[1][j] =
                            ((uint16_t *)recon_ptr
                                 ->buffer_cb)[recCbOffset +
                                              context_ptr->blk_geom->bwidth_uv -
                                              1 + j * recon_ptr->stride_cb];
                        cu_ptr->neigh_left_recon_16bit[2][j] =
                            ((uint16_t *)recon_ptr
                                 ->buffer_cr)[recCrOffset +
                                              context_ptr->blk_geom->bwidth_uv -
                                              1 + j * recon_ptr->stride_cr];
...
#if NO_ENCDEC
        // copy recon
        uint32_t tu_origin_index = context_ptr->blk_geom->origin_x +
        ...
        if (!context_ptr->hbd_mode_decision) {
            uint8_t *src_ptr =
                &(((uint8_t *)
                       candidate_buffer->recon_ptr->buffer_y)[tu_origin_index]);
            uint8_t *dst_ptr =
                &(((uint8_t *)context_ptr->cu_ptr->recon_tmp->buffer_y)[0]);

            uint32_t j;
            for (j = 0; j < bheight; j++)
                memcpy(dst_ptr + j * 128,
                       src_ptr + j * 128,
            ...
            if (context_ptr->blk_geom->has_uv) {
                ...
                // Cb
                src_ptr = &(((uint8_t *)candidate_buffer->recon_ptr
                                 ->buffer_cb)[tu_origin_index]);
                dst_ptr = &(
                    ((uint8_t *)context_ptr->cu_ptr->recon_tmp->buffer_cb)[0]);
                ...
                // Cr
                src_ptr = &(((uint8_t *)candidate_buffer->recon_ptr
                                 ->buffer_cr)[tu_origin_index]);
                dst_ptr = &(
                    ((uint8_t *)context_ptr->cu_ptr->recon_tmp->buffer_cr)[0]);
            ...
            uint16_t *src_ptr =
                ((uint16_t *)candidate_buffer->recon_ptr->buffer_y) +
                tu_origin_index;
            uint16_t *dst_ptr =
                (uint16_t *)context_ptr->cu_ptr->recon_tmp->buffer_y;
            for (uint32_t j = 0; j < bheight; j++)
                memcpy(dst_ptr + j * 128,
                       src_ptr + j * 128,
                       bwidth * sizeof(uint16_t));
            ...
            if (context_ptr->blk_geom->has_uv) {
                ...
                // Cb
                src_ptr = ((uint16_t *)candidate_buffer->recon_ptr->buffer_cb) +
                          tu_origin_index;
                ...
                // Cr
                src_ptr = ((uint16_t *)candidate_buffer->recon_ptr->buffer_cr) +
                          tu_origin_index;
...
void update_skip_next_nsq_for_a_b_shapes(ModeDecisionContext *context_ptr,
    ...
    case 0:
        *sq_cost =
            context_ptr->md_local_cu_unit[context_ptr->cu_ptr->mds_idx].cost;
        *h_cost = 0;
        *v_cost = 0;
        break;
    ...
    case 4:
        *v_cost +=
            context_ptr->md_local_cu_unit[context_ptr->cu_ptr->mds_idx].cost;
        *skip_next_nsq = (*h_cost > ((*sq_cost * context_ptr->sq_weight) / 100))
                             ? 1
                             : *skip_next_nsq;
        break;
    ...
    case 14:
    case 15:
        *skip_next_nsq = (*v_cost > ((*sq_cost * context_ptr->sq_weight) / 100))
                             ? 1
                             : *skip_next_nsq;
        break;
    }
...
static uint32_t mds_idx_16x16[64] = {
    ...
    67,   128,  336,  397,  1168, 1229, 1437, 1498, 189,  250,  458,
    519,  1290, 1351, 1559, 1620, 605,  666,  874,  935,  1706, 1767,
    1975, 2036, 727,  788,  996,  1057, 1828, 1889, 2097, 2158, 2269,
    2330, 2538, 2599, 3370, 3431, 3639, 3700, 2391, 2452, 2660, 2721,
    3492, 3553, 3761, 3822, 2807, 2868, 3076, 3137, 3908, 3969, 4177,
    4238, 2929, 2990, 3198, 3259, 4030, 4091, 4299, 4360
...
void av1_get_max_min_partition_features(
    ...
    int f_idx = 0;
    uint32_t q_index = picture_control_set_ptr->parent_pcs_ptr->frm_hdr
    ...
    const int dc_q =
        ...
        (sequence_control_set_ptr->static_config.encoder_bit_depth - 8);
    ...
    for (uint32_t index = 0; index < 64; index++) {
        ...
        uint32_t me_block_offset = get_me_info_index(
            picture_control_set_ptr->parent_pcs_ptr->max_number_of_pus_per_sb,
            blk_geom,
            geom_offset_x,
            geom_offset_y);
        ...
        for (uint8_t me_candidate_index = 0; me_candidate_index < total_me_cnt;
            ...
            if (inter_direction == 0 && list0_ref_index == 0) {
                ...
                cu_ptr.av1xd->mb_to_top_edge =
                    -(int32_t)((mirow * MI_SIZE) * 8);
                cu_ptr.av1xd->mb_to_bottom_edge =
                    ((picture_control_set_ptr->parent_pcs_ptr->av1_cm->mi_rows -
                      bw - mirow) *
                     MI_SIZE) *
                    8;
                cu_ptr.av1xd->mb_to_left_edge =
                    -(int32_t)((micol * MI_SIZE) * 8);
                cu_ptr.av1xd->mb_to_right_edge =
                    ((picture_control_set_ptr->parent_pcs_ptr->av1_cm->mi_cols -
                      bh - micol) *
                     MI_SIZE) *
                    8;
                ...
                av1_inter_prediction_function_table[is_highbd](
                    ...
                    !is_highbd
                        ? ((EbReferenceObject *)picture_control_set_ptr
                               ->ref_pic_ptr_array[0][0]
                               ->object_ptr)
                              ->reference_picture
                        : ((EbReferenceObject *)picture_control_set_ptr
                               ->ref_pic_ptr_array[0][0]
                               ->object_ptr)
                              ->reference_picture16bit,  // use last = [List
                                                         // 0][Ref Index 0]
                    NULL,                                // ref_pic_list1,
                    prediction_ptr,
                    blk_geom->origin_x,
                    blk_geom->origin_y,
                    0,  // perform_chroma,
                    (uint8_t)sequence_control_set_ptr->static_config
                        .encoder_bit_depth);

                const aom_variance_fn_ptr_t *fn_ptr =
                    &mefn_ptr[blk_geom->bsize];

                const uint32_t input_origin_index =
                    (cu_origin_y + input_picture_ptr->origin_y) *
                        input_picture_ptr->stride_y +
                    (cu_origin_x + input_picture_ptr->origin_x);
        ...
        if (abs_mv_row < min_abs_mv_row)
            min_abs_mv_row = abs_mv_row;
        if (abs_mv_row > max_abs_mv_row)
            max_abs_mv_row = abs_mv_row;
        if (abs_mv_col < min_abs_mv_col)
            min_abs_mv_col = abs_mv_col;
        if (abs_mv_col > max_abs_mv_col)
            max_abs_mv_col = abs_mv_col;

        sum_log_sse_sq += log_sse * log_sse;
        sum_log_sse += log_sse;
        if (log_sse < min_log_sse)
            min_log_sse = log_sse;
        if (log_sse > max_log_sse)
            max_log_sse = log_sse;
    }
    ...
    const float var_log_sse =
        sum_log_sse_sq / 64.0f - avg_log_sse * avg_log_sse;
...
BlockSize av1_predict_max_partition(PictureControlSet *picture_control_set_ptr,
    ...
    assert(
        picture_control_set_ptr->sf.auto_max_partition_based_on_simple_motion !=
        NOT_IN_USE);
    ...
    if (picture_control_set_ptr->sf.auto_max_partition_based_on_simple_motion ==
        DIRECT_PRED) {
        result = 0;
    ...
    } else if (picture_control_set_ptr->sf
                   .auto_max_partition_based_on_simple_motion == RELAXED_PRED) {
        for (result = MAX_NUM_CLASSES_MAX_MIN_PART_PRED - 1; result >= 0;
            ...
            if (probs[result] > 0.2)
                break;
                   ...
                   .auto_max_partition_based_on_simple_motion == ADAPT_PRED) {
        const uint32_t input_origin_index =
            (sb_origin_y + input_picture_ptr->origin_y) *
                input_picture_ptr->stride_y +
            (sb_origin_x + input_picture_ptr->origin_x);
        const aom_variance_fn_ptr_t *fn_ptr = &mefn_ptr[BLOCK_128X128];
        const unsigned int source_variance = eb_av1_get_sby_perpixel_variance(
            fn_ptr,
            (input_picture_ptr->buffer_y + input_origin_index),
            input_picture_ptr->stride_y,
            BLOCK_128X128);
...
EB_EXTERN EbErrorType
mode_decision_sb(SequenceControlSet *sequence_control_set_ptr,
                 PictureControlSet *picture_control_set_ptr,
                 const MdcLcuData *const mdcResultTbPtr, SuperBlock *sb_ptr,
                 uint16_t sb_origin_x, uint16_t sb_origin_y, uint32_t lcuAddr,
                 ModeDecisionContext *context_ptr) {
    EbErrorType return_error = EB_ErrorNone;
    ...
    const EbMdcLeafData *const leaf_data_array =
        mdcResultTbPtr->leaf_data_array;
    ...
    EbBool all_cu_init =
        (picture_control_set_ptr->parent_pcs_ptr->pic_depth_mode <=
         PIC_SQ_DEPTH_MODE);
    if (all_cu_init) {
        init_sq_nsq_block(sequence_control_set_ptr, context_ptr);
    } else {
        init_sq_non4_block(sequence_control_set_ptr, context_ptr);
    }
    // Mode Decision Neighbor Arrays
    context_ptr->intra_luma_mode_neighbor_array =
        picture_control_set_ptr
            ->md_intra_luma_mode_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->intra_chroma_mode_neighbor_array =
        picture_control_set_ptr
            ->md_intra_chroma_mode_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->mv_neighbor_array =
        picture_control_set_ptr->md_mv_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->skip_flag_neighbor_array =
        picture_control_set_ptr
            ->md_skip_flag_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->mode_type_neighbor_array =
        picture_control_set_ptr
            ->md_mode_type_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->leaf_depth_neighbor_array =
        picture_control_set_ptr
            ->md_leaf_depth_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->leaf_partition_neighbor_array =
        picture_control_set_ptr
            ->mdleaf_partition_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    ...
    if (!context_ptr->hbd_mode_decision) {
        ...
        context_ptr->cb_recon_neighbor_array =
            picture_control_set_ptr
                ->md_cb_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
        context_ptr->cr_recon_neighbor_array =
            picture_control_set_ptr
                ->md_cr_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    } else {
        context_ptr->luma_recon_neighbor_array16bit =
            picture_control_set_ptr
                ->md_luma_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX];
        context_ptr->cb_recon_neighbor_array16bit =
            picture_control_set_ptr
                ->md_cb_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX];
        context_ptr->cr_recon_neighbor_array16bit =
            picture_control_set_ptr
                ->md_cr_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX];
    }
    context_ptr->skip_coeff_neighbor_array =
        picture_control_set_ptr
            ->md_skip_coeff_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->luma_dc_sign_level_coeff_neighbor_array =
        picture_control_set_ptr->md_luma_dc_sign_level_coeff_neighbor_array
            [MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->cb_dc_sign_level_coeff_neighbor_array =
        picture_control_set_ptr
            ->md_cb_dc_sign_level_coeff_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->cr_dc_sign_level_coeff_neighbor_array =
        picture_control_set_ptr
            ->md_cr_dc_sign_level_coeff_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->txfm_context_array =
        picture_control_set_ptr->md_txfm_context_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->inter_pred_dir_neighbor_array =
        picture_control_set_ptr
            ->md_inter_pred_dir_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->ref_frame_type_neighbor_array =
        picture_control_set_ptr
            ->md_ref_frame_type_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    context_ptr->interpolation_type_neighbor_array =
        picture_control_set_ptr
            ->md_interpolation_type_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX];
    ...
    if (context_ptr->hbd_mode_decision) {
        ...
        uint32_t sb_width = MIN(
            sequence_control_set_ptr->sb_size_pix,
            sequence_control_set_ptr->seq_header.max_frame_width - sb_origin_x);
        uint32_t sb_height =
            MIN(sequence_control_set_ptr->sb_size_pix,
        ...
        Store16bitInputSrc(context_ptr->input_sample16bit_buffer,
                           picture_control_set_ptr,
                           sb_origin_x,
                           sb_origin_y,
                           sb_width,
        ...
        if (picture_control_set_ptr->slice_type != I_SLICE &&
            ...
            if ((sb_origin_x +
                 sequence_control_set_ptr->static_config.super_block_size) <
                    picture_control_set_ptr->parent_pcs_ptr
                        ->sequence_control_set_ptr->seq_header
                        .max_frame_width &&
                (sb_origin_y +
                 sequence_control_set_ptr->static_config.super_block_size) <
                    picture_control_set_ptr->parent_pcs_ptr
                ...
                av1_get_max_min_partition_features(sequence_control_set_ptr,
                                                   picture_control_set_ptr,
                                                   context_ptr,
                                                   features,
                                                   input_picture_ptr,
                                                   ...
                                                   sb_origin_y);
                max_bsize =
                    MIN(av1_predict_max_partition(picture_control_set_ptr,
                                                  features,
                                                  input_picture_ptr,
                                                  sb_origin_x,
                                                  sb_origin_y),
    ...
    uint64_t nsq_cost[NUMBER_OF_SHAPES] = {MAX_CU_COST,
                                           ...
                                           MAX_CU_COST,
                                           MAX_CU_COST};
    PART nsq_shape_table[NUMBER_OF_SHAPES] = {PART_N,
                                              PART_H,
                                              PART_V,
                                              PART_HA,
                                              PART_HB,
    ...
    do {
        ...
        const BlockGeom *blk_geom = context_ptr->blk_geom =
            get_blk_geom_mds(blk_idx_mds);
        CodingUnit *cu_ptr = context_ptr->cu_ptr =
            &context_ptr->md_cu_arr_nsq[blk_idx_mds];
        ...
        const EbMdcLeafData *const leafDataPtr =
            &mdcResultTbPtr->leaf_data_array[cuIdx];
...
#endif
        cu_ptr->mds_idx = blk_idx_mds;
        context_ptr->md_cu_arr_nsq[blk_idx_mds].mdc_split_flag =
            (uint16_t)leafDataPtr->split_flag;
#if ADD_SUPPORT_TO_SKIP_PART_N
        context_ptr->md_cu_arr_nsq[blk_geom->sqi_mds].split_flag =
            (uint16_t)leafDataPtr->split_flag;
#endif
        cu_ptr->split_flag =
            (uint16_t)leafDataPtr->split_flag;  // mdc indicates smallest or non
                                                // valid CUs with split flag=
        ...
        if (leafDataPtr->tot_d1_blocks != 1) {
            ...
            if (blk_geom->shape == PART_N)
#endif
                copy_neighbour_arrays(  // save a clean neigh in [1], encode
                                        // uses [0], reload the clean in [0]
                                        // after done last ns block in a
                                        // partition
                    picture_control_set_ptr,
                    context_ptr,
                    0,
        ...
        int mi_stride =
            picture_control_set_ptr->parent_pcs_ptr->av1_cm->mi_stride;
        const int32_t offset = mi_row * mi_stride + mi_col;
        cu_ptr->av1xd->mi = picture_control_set_ptr->parent_pcs_ptr->av1_cm
                                ->pcs_ptr->mi_grid_base +
                            offset;
        ModeInfo *mi_ptr = *cu_ptr->av1xd->mi;
        cu_ptr->av1xd->up_available = (mi_row > sb_ptr->tile_info.mi_row_start);
        cu_ptr->av1xd->left_available =
            (mi_col > sb_ptr->tile_info.mi_col_start);
        ...
        uint16_t redundant_blk_mds;
        if (all_cu_init)
            check_redundant_block(blk_geom,
                                  context_ptr,
                                  &redundant_blk_avail,
                                  &redundant_blk_mds);
        ...
        if (redundant_blk_avail && context_ptr->redundant_blk) {
            ...
            move_cu_data_redund(
                picture_control_set_ptr, context_ptr, src_cu, dst_cu);
#else
            move_cu_data_redund(src_cu, dst_cu);
#endif
            memcpy(&context_ptr->md_local_cu_unit[cu_ptr->mds_idx],
                   &context_ptr->md_local_cu_unit[redundant_blk_mds],
                   sizeof(MdCodingUnit));

            if (!context_ptr->hbd_mode_decision) {
                memcpy(dst_cu->neigh_left_recon[0],
                       src_cu->neigh_left_recon[0],
                       128);
                memcpy(dst_cu->neigh_left_recon[1],
                       src_cu->neigh_left_recon[1],
                       128);
                memcpy(dst_cu->neigh_left_recon[2],
                       src_cu->neigh_left_recon[2],
                       128);
                memcpy(dst_cu->neigh_top_recon[0],
                       src_cu->neigh_top_recon[0],
                       128);
                memcpy(dst_cu->neigh_top_recon[1],
                       src_cu->neigh_top_recon[1],
                       128);
                memcpy(dst_cu->neigh_top_recon[2],
                       src_cu->neigh_top_recon[2],
                       128);
            } else {
                uint16_t sz = sizeof(uint16_t);
                memcpy(dst_cu->neigh_left_recon_16bit[0],
                       src_cu->neigh_left_recon_16bit[0],
                       128 * sz);
                memcpy(dst_cu->neigh_left_recon_16bit[1],
                       src_cu->neigh_left_recon_16bit[1],
                       128 * sz);
                memcpy(dst_cu->neigh_left_recon_16bit[2],
                       src_cu->neigh_left_recon_16bit[2],
                       128 * sz);
                memcpy(dst_cu->neigh_top_recon_16bit[0],
                       src_cu->neigh_top_recon_16bit[0],
                       128 * sz);
                memcpy(dst_cu->neigh_top_recon_16bit[1],
                       src_cu->neigh_top_recon_16bit[1],
                       128 * sz);
                memcpy(dst_cu->neigh_top_recon_16bit[2],
                       src_cu->neigh_top_recon_16bit[2],
                       128 * sz);
            }

            memcpy(&context_ptr->md_ep_pipe_sb[cu_ptr->mds_idx],
            ...
            if (context_ptr->blk_geom->shape == PART_N) {
                uint8_t sq_index = LOG2F(context_ptr->blk_geom->sq_size) - 2;
                context_ptr->parent_sq_type[sq_index] =
                    src_cu->prediction_mode_flag;
                context_ptr->parent_sq_has_coeff[sq_index] =
                    src_cu->block_has_coeff;
        ...
        if (blk_geom->quadi > 0 && blk_geom->shape == PART_N) {
                ...
                parent_depth_idx_mds =
                    (context_ptr->blk_geom->sqi_mds -
                     (context_ptr->blk_geom->quadi - 3) *
                         ns_depth_offset[sequence_control_set_ptr->seq_header
                                             .sb_size == BLOCK_128X128]
                                        [context_ptr->blk_geom->depth]) -
                    parent_depth_offset[sequence_control_set_ptr->seq_header
                                            .sb_size == BLOCK_128X128]
                                       [blk_geom->depth];

                if (picture_control_set_ptr->slice_type == I_SLICE &&
                    parent_depth_idx_mds == 0 &&
                ...
                if (!sequence_control_set_ptr->sb_geom[lcuAddr]
                         .block_is_allowed[parent_depth_idx_mds])
                    parent_depth_cost = MAX_MODE_COST;

                    // compare the cost of the parent to the cost of the already
                    // encoded child + an estimated cost for the remaining child
                    // @ the current depth if the total child cost is higher
                    // than the parent cost then skip the remaining  child @ the
                    // current depth when md_exit_th=0 the estimated cost for
                    // the remaining child is not taken into account and the
                    // action will be lossless compared to no exit MD_EXIT_THSL
                    // could be tuned toward a faster encoder but lossy
#if SPEED_OPT
                if (parent_depth_cost <=
                    current_depth_cost + (current_depth_cost *
                                          (4 - context_ptr->blk_geom->quadi) *
                                          context_ptr->md_exit_th /
                                          context_ptr->blk_geom->quadi / 100)) {
#else
            if (parent_depth_cost <=
                current_depth_cost +
                    (current_depth_cost * (4 - context_ptr->blk_geom->quadi) *
                     MD_EXIT_THSL / context_ptr->blk_geom->quadi / 100)) {
#endif
                    skip_next_sq = 1;
            ...
            if (cu_ptr->mds_idx >= next_non_skip_blk_idx_mds &&
                skip_next_sq == 1)
                skip_next_sq = 0;
            ...
            EbBool auto_max_partition_block_skip =
                ...
                (mdcResultTbPtr->leaf_data_array[cuIdx].split_flag == EB_TRUE);

            if (picture_control_set_ptr->parent_pcs_ptr
                    ->sequence_control_set_ptr->sb_geom[lcuAddr]
                    .block_is_allowed[cu_ptr->mds_idx] &&
                !skip_next_nsq && !skip_next_sq &&
                !auto_max_partition_block_skip) {
#else
        if (picture_control_set_ptr->parent_pcs_ptr->sequence_control_set_ptr
                ->sb_geom[lcuAddr]
                .block_is_allowed[cu_ptr->mds_idx] &&
            !skip_next_nsq && !skip_next_sq) {
#endif
                md_encode_block(sequence_control_set_ptr,
                                picture_control_set_ptr,
                                context_ptr,
                                input_picture_ptr,
                                &skip_sub_blocks,
                ...
                if (context_ptr->blk_geom->shape != PART_N)
                    context_ptr->md_local_cu_unit[context_ptr->cu_ptr->mds_idx]
                        .cost = (MAX_MODE_COST >> 4);
                else
                    context_ptr->md_local_cu_unit[context_ptr->cu_ptr->mds_idx]
                        .cost = (MAX_MODE_COST >> 10);
            }
#endif
                ...
                // - For square blocks, since the blocks can be further
                // splitted, they are considered in
                // d2_inter_depth_block_decision with cost of zero.
                // - For non square blocks, since they can not be splitted
                // further the cost is set to a large value (MAX_MODE_COST >> 4)
                // to make sure they are not selected.
                //   The value is set to MAX_MODE_COST >> 4 to make sure there
                //   is not overflow when adding costs.
                if (context_ptr->blk_geom->shape != PART_N)
                    context_ptr->md_local_cu_unit[context_ptr->cu_ptr->mds_idx]
                        .cost = (MAX_MODE_COST >> 4);
                else
        ...
        if (blk_geom->nsi + 1 == blk_geom->totns)
            ...
            uint32_t first_blk_idx =
                context_ptr->cu_ptr->mds_idx -
                (blk_geom->nsi);  // index of first block in this partition
            for (int blk_it = 0; blk_it < blk_geom->nsi + 1; blk_it++)
                tot_cost +=
                    context_ptr->md_local_cu_unit[first_blk_idx + blk_it].cost;
#if MULTI_PASS_PD
            nsq_cost[context_ptr->blk_geom->shape] = tot_cost;
#endif
#if SPEED_OPT
            if ((tot_cost + tot_cost * (blk_geom->totns - (blk_geom->nsi + 1)) *
                                context_ptr->md_exit_th / (blk_geom->nsi + 1) /
                                100) >
                context_ptr->md_local_cu_unit[context_ptr->blk_geom->sqi_mds]
                    .cost)
#else
            if ((tot_cost + tot_cost * (blk_geom->totns - (blk_geom->nsi + 1)) *
                                MD_EXIT_THSL / (blk_geom->nsi + 1) / 100) >
                context_ptr->md_local_cu_unit[context_ptr->blk_geom->sqi_mds]
                    .cost)
#endif
                skip_next_nsq = 1;
        ...
        if (blk_geom->shape != PART_N) {
            if (blk_geom->nsi + 1 < blk_geom->totns)
                ...
                copy_neighbour_arrays(  // restore [1] in [0] after done last ns
                                        // block
                    picture_control_set_ptr,
                    context_ptr,
                    1,
        ...
        d1_blocks_accumlated =
            d1_first_block == 1 ? 1 : d1_blocks_accumlated + 1;
#else
        d1_blocks_accumlated =
            blk_geom->shape == PART_N ? 1 : d1_blocks_accumlated + 1;
#endif
        ...
        if (d1_blocks_accumlated == leafDataPtr->tot_d1_blocks) {
            ...
            // Sorting
            {
                ...
                for (i = 0; i < NUMBER_OF_SHAPES - 1; ++i) {
                    for (j = i + 1; j < NUMBER_OF_SHAPES; ++j) {
                        if (nsq_cost[nsq_shape_table[j]] <
                            nsq_cost[nsq_shape_table[i]]) {
                ...
                depth_cost[sequence_control_set_ptr->static_config
                                       .super_block_size == 128
                               ? context_ptr->blk_geom->depth
                               : context_ptr->blk_geom->depth + 1] +=
                    nsq_cost[nsq_shape_table[0]];
            }
#endif
            ...
            context_ptr->coeff_based_skip_atb =
                picture_control_set_ptr->parent_pcs_ptr->coeff_based_skip_atb &&
                        context_ptr->md_local_cu_unit[lastCuIndex_mds]
                            .avail_blk_flag &&
                        context_ptr->md_cu_arr_nsq[lastCuIndex_mds]
                                .block_has_coeff == 0
                    ? 1
                    : 0;

            if (context_ptr->md_cu_arr_nsq[lastCuIndex_mds].split_flag ==
                EB_FALSE) {
        ...
        if (skip_sub_blocks && leaf_data_array[cuIdx].split_flag) {
            ...
            while (cuIdx < leaf_count) {
                const BlockGeom *next_blk_geom =
                    get_blk_geom_mds(leaf_data_array[cuIdx].mds_idx);
                if ((next_blk_geom->origin_x <
                     blk_geom->origin_x + blk_geom->bwidth) &&
                    (next_blk_geom->origin_y <
                     blk_geom->origin_y + blk_geom->bheight))
                    cuIdx++;
                else
                    break;
            }
...
#define MAX_TATAL_SEARCH_AREA_WIDTH \
    (MAX_SB_SIZE + MAX_SEARCH_POINT_WIDTH + ME_FILTER_TAP)
#define MAX_TATAL_SEARCH_AREA_HEIGHT \
    (MAX_SB_SIZE + MAX_SEARCH_POINT_HEIGHT + ME_FILTER_TAP)

#define MAX_SEARCH_AREA_SIZE \
    MAX_TATAL_SEARCH_AREA_WIDTH *MAX_TATAL_SEARCH_AREA_HEIGHT
#if PREDICT_NSQ_SHAPE
uint64_t spatial_full_distortion_helper(uint8_t *input, uint32_t input_offset,
                                        uint32_t input_stride, uint8_t *recon,
                                        uint32_t recon_offset,
                                        uint32_t recon_stride,
                                        uint32_t area_width,
                                        uint32_t area_height, uint8_t choice) {
    uint64_t sfd = 0;

    switch (choice) {
    case 0:
        sfd = spatial_full_distortion_kernel4x_n_sse2_intrin(input,
                                                             input_offset,
                                                             input_stride,
                                                             recon,
                                                             recon_offset,
        ...
        break;
    case 1:
        sfd = spatial_full_distortion_kernel8x_n_sse2_intrin(input,
                                                             input_offset,
                                                             input_stride,
                                                             recon,
                                                             recon_offset,
        ...
        break;
    case 2:
        sfd = spatial_full_distortion_kernel16x_n_sse2_intrin(input,
                                                              input_offset,
                                                              input_stride,
                                                              recon,
                                                              recon_offset,
        ...
        break;
    case 3:
        sfd = spatial_full_distortion_kernel32x_n_sse2_intrin(input,
                                                              input_offset,
                                                              input_stride,
                                                              recon,
                                                              recon_offset,
        ...
        break;
    case 4:
        sfd = spatial_full_distortion_kernel64x_n_sse2_intrin(input,
                                                              input_offset,
                                                              input_stride,
                                                              recon,
                                                              recon_offset,
        ...
        break;
    case 5:
        sfd = spatial_full_distortion_kernel128x_n_sse2_intrin(input,
                                                               input_offset,
                                                               input_stride,
                                                               recon,
                                                               recon_offset,
...
uint64_t spatial_full_distortion_avx2_helper(
    ...
    uint32_t area_width, uint32_t area_height, uint8_t choice) {
    uint64_t sfd = 0;

    switch (choice) {
    case 0:
        sfd = spatial_full_distortion_kernel4x_n_avx2_intrin(input,
                                                             input_offset,
                                                             input_stride,
                                                             recon,
                                                             recon_offset,
        ...
        break;
    case 1:
        sfd = spatial_full_distortion_kernel8x_n_avx2_intrin(input,
                                                             input_offset,
                                                             input_stride,
                                                             recon,
                                                             recon_offset,
        ...
        break;
    case 2:
        sfd = spatial_full_distortion_kernel16x_n_avx2_intrin(input,
                                                              input_offset,
                                                              input_stride,
                                                              recon,
                                                              recon_offset,
        ...
        break;
    case 3:
        sfd = spatial_full_distortion_kernel32x_n_avx2_intrin(input,
                                                              input_offset,
                                                              input_stride,
                                                              recon,
                                                              recon_offset,
        ...
        break;
    case 4:
        sfd = spatial_full_distortion_kernel64x_n_avx2_intrin(input,
                                                              input_offset,
                                                              input_stride,
                                                              recon,
                                                              recon_offset,
        ...
        break;
    case 5:
        sfd = spatial_full_distortion_kernel128x_n_avx2_intrin(input,
                                                               input_offset,
                                                               input_stride,
                                                               recon,
                                                               recon_offset,
